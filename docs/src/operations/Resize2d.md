...
- **result** (`Tensor`): Output tensor with resized spatial dimensions. The output shape is:
  - `(N, C, H_out, W_out)` if `channel_last=False`
  - `(N, H_out, W_out, C)` if `channel_last=True`
  
  where `H_out` and `W_out` are the values specified in the `sizes` parameter. The batch size `N` and number of channels `C` remain unchanged.
