    ...
    std::vector<SparseCOO> const& sparse_zs)
{
    ...
    if (env_as<bool>("FORGE_SPARSE_NO_MATH"))
...
static std::pair<std::vector<std::int32_t>, int> encode_strips(
    ...
    int t_factor_c)
{
    if (env_as<bool>("FORGE_SPARSE_MM_ENCODE_ALL_STRIPS"))
    {
        throw std::runtime_error("Encoding all strips for sparse matmul is unsupported currently.");  // svuckovic
    }

    // Remove when this is resolved: tenstorrent/forge#842
    bool allow_illegal_sparse_pars = env_as<bool>("FORGE_ALLOW_ILLEGAL_SPARSE_PARS");
    ...
    std::string const& visualize_sparse_path) const
{
    ...
    if (not visualize_sparse_path.empty())
    {
        ...
        auto sparse_utils_module = py::module_::import("forge.op.eval.sparse_utils");
...
int SparseFORGE::get_encoding_tiles_per_core_estimate(int grid_r, int t_factor_r, int u_rt, int u_kt) const
{
    ...
    bool encode_all_strips = env_as<bool>("FORGE_SPARSE_MM_ENCODE_ALL_STRIPS");
