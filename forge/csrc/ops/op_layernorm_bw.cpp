...
void decompose_post_autograd(const Op &op, DecomposingContext &dc, const std::vector<NodeContext> &inputs)
{
    ...
    if (operand == 2)
    {
        ...
        if (grad.shape.size() == 3 && grad.shape[0] != 1)
        {
            ...
            grad_reduced = dc.op(Op("reduce_sum", {{"dim_arg", std::vector<int>{0}}, {"keep_dim", true}}), {grad});
        }
        // Gradient w.r.t. bias (beta): dbeta = reduce_sum(grad, dim=-2, keep_dim=True)
        NodeContext dbeta =
            dc.op(Op("reduce_sum", {{"dim_arg", std::vector<int>{-2}}, {"keep_dim", true}}), {grad_reduced});
        dc.fuse(dbeta);
        return;
    }
    ...
    if (operand == 1)
    {
        ...
        NodeContext xhat_grad = dc.op(Op("multiply"), {xhat, grad});
        NodeContext xhat_grad_reduced = xhat_grad;
        if (xhat_grad.shape.size() == 3 && xhat_grad.shape[0] != 1)
        {
            // has to reduce over batch first
            xhat_grad_reduced =
                dc.op(Op("reduce_sum", {{"dim_arg", std::vector<int>{0}}, {"keep_dim", true}}), {xhat_grad});
        }
        NodeContext dgamma =
            dc.op(Op("reduce_sum", {{"dim_arg", std::vector<int>{-2}}, {"keep_dim", true}}), {xhat_grad_reduced});
        dc.fuse(dgamma);
        return;
    }

    // operand == 0: Gradient w.r.t. input
    NodeContext dxhat = dc.op(Op("multiply"), {grad, gamma});

    // sum_1 = reduce_sum(dxhat, dim, keep_dim=True)
    NodeContext sum_1 = dc.op(Op("reduce_sum", {{"dim_arg", std::vector<int>{dim}}, {"keep_dim", true}}), {dxhat});

    // sum_2 = reduce_sum(dxhat * xhat, dim, keep_dim=True)
    NodeContext dxhat_xhat = dc.op(Op("multiply"), {dxhat, xhat});
    NodeContext sum_2 = dc.op(Op("reduce_sum", {{"dim_arg", std::vector<int>{dim}}, {"keep_dim", true}}), {dxhat_xhat});

    // xhat_sum_2 = xhat * sum_2
    NodeContext xhat_sum_2 = dc.op(Op("multiply"), {xhat, sum_2});

    // sum_1_sum_2_add = sum_1 + xhat_sum_2
    NodeContext sum_1_sum_2_add = dc.op(Op("add"), {sum_1, xhat_sum_2});
    ...
    NodeContext N_recip_add = dc.op(Op("multiply"), {N_recip_node, sum_1_sum_2_add});

    // dxhat_add_sub = dxhat - N_recip_add
    NodeContext dxhat_add_sub = dc.op(Op("subtract"), {dxhat, N_recip_add});

    // dx = ivar * dxhat_add_sub
    NodeContext dx = dc.op(Op("multiply"), {ivar, dxhat_add_sub});
