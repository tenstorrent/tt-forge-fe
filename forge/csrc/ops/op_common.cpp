    ...
    tt::DecomposingContext &dc, const tt::graphlib::NodeContext &activation, std::vector<int> sizes, bool channel_last)
{
    ...
    auto repeat_if_needed = [&](const tt::graphlib::NodeContext &in, int repeats, int dim) -> tt::graphlib::NodeContext
    {
        if (repeats > 1)
        {
            return dc.op(ops::Op("repeat_interleave", {{"repeats", repeats}, {"dim", dim}}), {in});
        }
        return in;
    };
    ...
    if (rank == 3)
    {
        ...
        if (down_w != 1)
        {
            ...
            result = dc.op(ops::Op("unsqueeze", {{"dim", static_cast<int>(w_dim)}}), {result});
            ...
            result = dc.op(ops::Op("squeeze", {{"dim", static_cast<int>(w_dim)}}), {result});
        }
