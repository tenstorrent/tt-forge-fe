...
{
    TT_DBG_ASSERT(op.type() == OpType::Argsort, "Wrong op type.");
    TT_ASSERT(tensors.size() == 1, "Argsort should have one input");
    ...
    return torch::argsort(tensors[0], axis, descending);
}
...
{
    TT_DBG_ASSERT(op.type() == OpType::Argsort, "Wrong op type.");
    TT_ASSERT(in_shapes.size() == 1, "Argsort should have one input");
...
{
    TT_DBG_ASSERT(op.type() == OpType::Argsort, "Wrong op type.");
    TT_ASSERT(false, "Argsort does not have backward.");
    unreachable();
}
