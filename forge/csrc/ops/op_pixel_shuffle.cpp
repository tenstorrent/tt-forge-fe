...
void decompose_initial(const Op &op, DecomposingContext &dc, const std::vector<NodeContext> &inputs)
{
    ...
    auto x = dc.op(Op("reshape", {{"shape", reshape_dims1}}), {result});
    ...
    x = dc.op(Op("transpose", {{"dim0", base_dim + 1}, {"dim1", base_dim + 3}}), {x});

    // Second transpose: swap r and r -> [..., C, H, r, r, W] (positions changed after first transpose)
    x = dc.op(Op("transpose", {{"dim0", base_dim + 2}, {"dim1", base_dim + 3}}), {x});

    // Third transpose: swap the last two dimensions (r and W) -> [..., C, H, r, W, r]
    x = dc.op(Op("transpose", {{"dim0", base_dim + 3}, {"dim1", base_dim + 4}}), {x});
    ...
    x = dc.op(Op("reshape", {{"shape", final_dims}}), {x});
