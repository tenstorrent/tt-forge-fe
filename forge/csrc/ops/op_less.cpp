...
{
    TT_DBG_ASSERT(op.type() == OpType::Less, "Wrong op type.");
    TT_ASSERT(in_shapes.size() == 2, "Less should have two input shapes.");
    TT_ASSERT(op.attrs().size() == 0, "Less should not have any attrs.");
...
{
    TT_ASSERT(false, "Less does not have backward.");
    unreachable();
}

long initial_flops_estimate(const Op &op, const std::vector<std::vector<std::uint32_t>> &inputs)
{
    TT_ASSERT(inputs.size() == 2, "Less should have two inputs");

    auto shape_tuple = less::shape(op, inputs);
