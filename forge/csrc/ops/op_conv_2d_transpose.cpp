...
at::Tensor eval(const Op &op, const std::vector<at::Tensor> &tensors)
{
    ...
    TT_ASSERT(tensors.size() <= 3, "ConvTranspose ops should have up to three inputs (input, weight, bias)");
    TT_ASSERT(tensors.size() >= 2, "ConvTranspose ops should have at least two inputs (input, weight)");
    ...
    std::vector<int> output_padding = op.attr_as<std::vector<int>>("output_padding");
    ...
    std::vector<int64_t> torch_output_padding = {
        static_cast<int64_t>(output_padding[0]), static_cast<int64_t>(output_padding[1])};
...
{
    TT_DBG_ASSERT(op.type() == OpType::Conv2dTranspose, "Wrong op type.");
    TT_ASSERT(in_shapes.size() <= 3, "ConvTranspose ops should have up to three inputs (input, weight, bias)");
    TT_ASSERT(in_shapes.size() >= 2, "ConvTranspose ops should have at least two inputs (input, weight)");

    auto act_shape = in_shapes[0];
    auto weight_shape = in_shapes[1];
    int groups = op.attr_as<int>("groups");

    std::uint32_t batch_size = act_shape[0];
    std::uint32_t cout = weight_shape[1] * groups;
    ...
    if (channel_last)
        out_shape = {batch_size, h_out, w_out, cout};
    else
        out_shape = {batch_size, cout, h_out, w_out};
...
{
    TT_DBG_ASSERT(op.type() == OpType::Conv2dTranspose, "Wrong op type.");
    TT_ASSERT(false, "Conv2dTranspose op does not support backward pass yet.");
    return nullptr;
}
...
{
    TT_DBG_ASSERT(op.type() == OpType::Conv2dTranspose, "Wrong op type.");
    TT_ASSERT(inputs.size() <= 3, "ConvTranspose ops should have up to three inputs (input, weight, bias)");
    TT_ASSERT(inputs.size() >= 2, "ConvTranspose ops should have at least two inputs (input, weight)");
    ...
    // Only want to re-create the Conv2dTranspose op if something has changed. Otherwise the compiler will infinitely
    // decompose the same Conv2dTranspose over and over.
    if (!is_bias_unchanged || !is_channel_last)
    {
        ops::Attrs conv_transpose_attrs = {
            {"stride", stride},
            {"dilation", dilation},
            {"groups", groups},
            {"padding", padding},
        ...
        NodeContext result = dc.op(Op(OpType::Conv2dTranspose, conv_transpose_attrs), new_inputs);
