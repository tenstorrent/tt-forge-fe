    ...
    const tt::graphlib::NodeContext &gradient)
{
    ...
    // x * sqrt(1/2)
    auto x_scaled = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {inputs[0], sqrt1_2});

    // erf(x * sqrt(1/2))
    auto erf_result = ac.autograd->create_op(ac, graphlib::OpType("erf"), {x_scaled});

    // 1 + erf(x * sqrt(1/2))
    auto one_plus_erf = ac.autograd->create_op(ac, graphlib::OpType("add"), {one, erf_result});

    // 0.5 * (1 + erf(x * sqrt(1/2)))
    auto cdf = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {half, one_plus_erf});

    // x^2
    auto x_squared = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {inputs[0], inputs[0]});

    // -0.5 * x^2
    auto neg_half_x_squared = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {neg_half, x_squared});

    // exp(-0.5 * x^2)
    auto exp_result = ac.autograd->create_op(ac, graphlib::OpType("exp"), {neg_half_x_squared});

    // x * exp(-0.5 * x^2) * (0.5 * sqrt(1/2) * 2/sqrt(pi))
    auto x_exp = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {inputs[0], exp_result});
    auto pdf = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {x_exp, sqrt2pi_factor});

    // cdf + pdf
    auto derivative = ac.autograd->create_op(ac, graphlib::OpType("add"), {cdf, pdf});
    ...
    const tt::graphlib::NodeContext &gradient)
{
    ...
    // x^3
    auto x_squared = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {inputs[0], inputs[0]});
    auto x_cubed = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {inputs[0], x_squared});

    // 0.044715 * x^3
    auto cubic_term = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {cubic_factor, x_cubed});

    // x + 0.044715 * x^3
    auto inner = ac.autograd->create_op(ac, graphlib::OpType("add"), {inputs[0], cubic_term});

    // (M_2_SQRTPI / M_SQRT2) * (x + 0.044715 * x^3)
    auto tanh_input = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {tanh_factor, inner});

    // tanh((M_2_SQRTPI / M_SQRT2) * (x + 0.044715 * x^3))
    auto tanh_result = ac.autograd->create_op(ac, graphlib::OpType("tanh"), {tanh_input});

    // 1 + tanh(...)
    auto one_plus_tanh = ac.autograd->create_op(ac, graphlib::OpType("add"), {one, tanh_result});

    // intermediate_0 = 0.5 * (1 + tanh(...))
    auto intermediate_0 = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {half, one_plus_tanh});

    // === Calculate intermediate_1: x * exp(-0.5 * x * x) * (0.5 * M_2_SQRTPI / M_SQRT2) ===

    // x * x
    auto x_times_x = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {inputs[0], inputs[0]});

    // -0.5 * x * x
    auto neg_half_x_squared = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {neg_half, x_times_x});
    ...
    // intermediate_1 = x * exp(-0.5 * x * x) * (0.5 * M_2_SQRTPI / M_SQRT2)
    auto intermediate_1 = ac.autograd->create_op(ac, graphlib::OpType("multiply"), {x_exp, exp_factor});

    auto derivative = ac.autograd->create_op(ac, graphlib::OpType("add"), {intermediate_0, intermediate_1});
...
long initial_flops_estimate(const Op &op, const std::vector<std::vector<std::uint32_t>> &inputs)
{
    ...
    return op_common::initial_flops_estimate_output_dim(gelu::shape(op, inputs));
}
