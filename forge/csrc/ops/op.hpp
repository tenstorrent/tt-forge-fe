...
class Op
{
   public:
    ...
    virtual std::tuple<graphlib::Shape, std::vector<graphlib::DimBroadcastTrampoline>> shape(
...
{  // clang-format off
    if      constexpr (op_type == OpType::Abs) return std::make_unique<OpAbs>(std::forward(args)...);
    else if constexpr (op_type == OpType::Add) return std::make_unique<OpAdd>(std::forward(args)...);
    else                                       return std::make_unique<Op>   (std::forward(args)...);
}  // clang-format on
