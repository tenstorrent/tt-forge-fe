...
torch::Tensor OpAbs::eval(const std::vector<torch::Tensor> &tensors) const
{
...
Shape OpAbs::shape(const std::vector<std::vector<std::uint32_t>> &in_shapes) const
{
    TT_ASSERT(in_shapes.size() == 1, "OpAbs::shape should have single input shape.");
    return Shape::create(in_shapes[0]);
}

long OpAbs::initial_flops_stimate(const std::vector<std::vector<std::uint32_t>> &inputs) const
{
