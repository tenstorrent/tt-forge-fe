    ...
    const graphlib::OpType &old_op_type, const Op &op, DecomposingContext &dc, const std::vector<NodeContext> &inputs)
{
    ...
    if (mode == "nearest" && ((size_h % input_h != 0) || (size_w % input_w != 0) || (!is_upsampling_height && !is_upsampling_width) || (is_upsampling_height != is_upsampling_width))){
        tt::ops::op_common::decompose_nearest_interpolation(dc, result, sizes, channel_last);
        return;
    }

    TT_ASSERT(is_upsampling_height == is_upsampling_width, "OpType::Resize2d does not support one spatial dimension as upsample and another spatial dimension as downsample for {} interpolation mode", mode);

    if (is_upsampling_height && is_upsampling_width)
    {
        TT_ASSERT((size_h % input_h == 0) && (size_w % input_w == 0), "OpType::Resize2d only support upsampling with integer scale factor for {} interpolation mode", mode);
        TT_ASSERT(
