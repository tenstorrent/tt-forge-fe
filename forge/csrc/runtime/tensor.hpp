...
class TensorImpl : public std::enable_shared_from_this<TensorImpl>
{
   public:
    ...
    torch::Tensor to_torch() const
    {
        ...
        // If the tensor is on host, return new torch tensor with the same data.
        TT_ASSERT(host_storage.has_value(), "Since the tensor is on host, we expect the host storage to be set");
        std::memcpy(torch_tensor.data_ptr(), host_storage->borrow_data().get(), torch_tensor.numel() * desc.itemsize);
...
class Tensor
{
   public:
    ...
    // Creates a brand new torch tensor from this one (by copying the data).
