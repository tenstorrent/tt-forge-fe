...
struct WhisperPastCacheBase : testing::Test
{
    ...
    WhisperPastCacheBase()
    {
        ...
        auto weight =
            create_input(*graph, "weight", graphlib::Shape::create({768, 768}), graphlib::InputNodeType::Parameter);
        auto pkv_input_1 = create_input(
            *graph, "pkv_input_1", graphlib::Shape::create({1, 12, 416, 64}), tt::graphlib::InputNodeType::Activation);
        auto pkv_input_2 = create_input(
            *graph, "pkv_input_2", graphlib::Shape::create({1, 12, 416, 64}), tt::graphlib::InputNodeType::Activation);
        auto in_1 = create_input(
            *graph, "_input_1", graphlib::Shape::create({32, 768}), tt::graphlib::InputNodeType::Activation);
        auto in_2 = create_input(
            *graph, "_input_2", graphlib::Shape::create({32, 768}), tt::graphlib::InputNodeType::Activation);
...
TEST_F(WhisperPastCacheBase, whisper_past_cache_base)
{
    ...
    EXPECT_EQ(
        graph->node_by_id(graph->user_edges(output_nodes[0])[0].consumer_node_id),
        graph->node_by_id(graph->user_edges(output_nodes[1])[0].consumer_node_id));

    // check if original 'pkv_input_' nodes are removed
    for (auto input_node : input_nodes) EXPECT_EQ(input_node->name().find("pkv_input_"), std::string::npos);
...
struct WhisperPastCacheSubGraph : testing::Test
{
    ...
    WhisperPastCacheSubGraph()
    {
        ...
        // define input/weight nodes
        auto weight =
            create_input(*graph, "weight", graphlib::Shape::create({384, 384}), graphlib::InputNodeType::Parameter, 0);
        auto pkv_input_1 = create_input(
            *graph, "pkv_input_1", graphlib::Shape::create({1, 1536, 384}), tt::graphlib::InputNodeType::Activation, 0);
        auto pkv_input_2 = create_input(
            *graph,
            "pkv_input_2",
            graphlib::Shape::create({1, 6, 1536, 64}),
            tt::graphlib::InputNodeType::Activation,
            1);
        ...
        auto reshape_1 = add_node<graphlib::PyOpNode>(
            *graph,
            "reshape_1",
            graphlib::OpType("reshape", {}, {{"shape", std::vector{1536, 384}}}),
            {pkv_input_1},
            {},
            0);
        ...
        auto reshape_1_ = add_node<graphlib::PyOpNode>(
            *graph,
            "reshape_1_",
            graphlib::OpType("reshape", {}, {{"shape", std::vector{6, 1536, 64}}}),
            {hslice_1},
            {},
            0);
        ...
        auto reshape_2 = add_node<graphlib::PyOpNode>(
            *graph,
            "reshape_2",
            graphlib::OpType("reshape", {}, {{"shape", std::vector{12, 1536, 64}}}),
            {nop_2},
            {},
            1);
...
TEST_F(WhisperPastCacheSubGraph, whisper_past_cache_subgraph)
{
    ...
    // check if original 'pkv_input_' nodes are removed
    for (auto input_node : input_nodes) EXPECT_EQ(input_node->name().find("pkv_input_2"), std::string::npos);
...
struct T5PastCacheRotate : testing::Test
{
    ...
    T5PastCacheRotate()
    {
        ...
        // define input/weight nodes
        auto weight =
            create_input(*graph, "weight", graphlib::Shape::create({512, 512}), graphlib::InputNodeType::Parameter);
        auto in_1 =
            create_input(*graph, "in_1", graphlib::Shape::create({32, 512}), tt::graphlib::InputNodeType::Constant);
        auto in_2 =
            create_input(*graph, "in_2", graphlib::Shape::create({32, 512}), tt::graphlib::InputNodeType::Constant);
        auto pkv_input_1 = create_input(
            *graph, "pkv_input_1", graphlib::Shape::create({1, 8, 480, 64}), tt::graphlib::InputNodeType::Activation);
        auto pkv_input_2 = create_input(
            *graph, "pkv_input_2", graphlib::Shape::create({1, 8, 480, 64}), tt::graphlib::InputNodeType::Activation);
...
{
    setenv("FORGE_ROTATE_PAST_CACHE_PARAMS", "1", 1 /* overwrite */);
    tt::passes::link_past_cache_ios(graph);
    unsetenv("FORGE_ROTATE_PAST_CACHE_PARAMS");
    ...
    EXPECT_EQ(
        graph->node_by_id(graph->user_edges(output_nodes[0])[0].consumer_node_id),
        graph->node_by_id(graph->user_edges(output_nodes[2])[0].consumer_node_id));

    // check if original 'pkv_input_' nodes are removed
    for (auto input_node : input_nodes) EXPECT_EQ(input_node->name().find("pkv_input"), std::string::npos);
...
struct Falcon40bPastCache : testing::Test
{
    ...
    Falcon40bPastCache()
    {
        ...
        // define input/weight nodes
        auto weight =
            create_input(*graph, "weight", graphlib::Shape::create({64, 8192}), graphlib::InputNodeType::Parameter);
        auto in_1_1 =
            create_input(*graph, "in_1_1", graphlib::Shape::create({128, 8192}), tt::graphlib::InputNodeType::Constant);
        auto in_1_2 = create_input(
            *graph, "in_1_2", graphlib::Shape::create({1, 1, 128, 64}), tt::graphlib::InputNodeType::Constant);
        auto in_2_1 =
            create_input(*graph, "in_2_1", graphlib::Shape::create({32, 8192}), tt::graphlib::InputNodeType::Constant);
        auto in_2_2 = create_input(
            *graph, "in_2_2", graphlib::Shape::create({1, 1, 32, 64}), tt::graphlib::InputNodeType::Constant);
        auto in_2_3 = create_input(
            *graph, "in_2_3", graphlib::Shape::create({1, 32, 128, 64}), tt::graphlib::InputNodeType::Constant);
        auto in_2_4 = create_input(
            *graph, "in_2_4", graphlib::Shape::create({1, 32, 128, 64}), tt::graphlib::InputNodeType::Constant);
        auto pkv_input = create_input(
            *graph, "pkv_input", graphlib::Shape::create({1, 32, 128, 64}), tt::graphlib::InputNodeType::Activation);
        auto cos_0 = create_input(
            *graph, "cos_0", graphlib::Shape::create({1, 1, 128, 64}), tt::graphlib::InputNodeType::Activation);
        auto cos_1 = create_input(
            *graph, "cos_1", graphlib::Shape::create({1, 1, 32, 64}), tt::graphlib::InputNodeType::Activation);
        ...
        auto reshape_2 = add_node<graphlib::PyOpNode>(
            *graph, "reshape_2", graphlib::OpType("reshape", {}, {{"shape", std::vector{1, 1, 32, 64}}}), {matmul_2});
        ...
        auto reshape_3 = add_node<graphlib::PyOpNode>(
            *graph, "reshape_3", graphlib::OpType("reshape", {}, {{"shape", std::vector{32, 128, 64}}}), {pkv_input});
...
struct Fuyu8bPastCache : testing::Test
{
    ...
    Fuyu8bPastCache()
    {
        ...
        // define input/weight nodes
        auto weight =
            create_input(*graph, "weight", graphlib::Shape::create({4096, 4096}), graphlib::InputNodeType::Parameter);
        auto in_1 =
            create_input(*graph, "in_1", graphlib::Shape::create({32, 4096}), tt::graphlib::InputNodeType::Constant);
        auto in_2 =
            create_input(*graph, "in_2", graphlib::Shape::create({416, 4096}), tt::graphlib::InputNodeType::Constant);
        auto in_add = create_input(
            *graph, "in_add", graphlib::Shape::create({1, 64, 416, 32}), tt::graphlib::InputNodeType::Constant);
        auto pkv_input = create_input(
            *graph, "pkv_input", graphlib::Shape::create({1, 64, 416, 64}), tt::graphlib::InputNodeType::Activation);

        // path 1
        auto matmul_1 = add_node<graphlib::PyOpNode>(*graph, "matmul_1", "matmul", {}, {in_1, weight});
        auto reshape_1 = add_node<graphlib::PyOpNode>(
            *graph, "reshape_1", graphlib::OpType("reshape", {}, {{"shape", std::vector{1, 32, 64, 64}}}), {matmul_1});
        auto transpose_1 = add_node<graphlib::PyOpNode>(
            *graph, "transpose_1", graphlib::OpType("transpose", {}, {{"dim0", -3}, {"dim1", -2}}), {reshape_1});
        auto concat_1 = add_node<graphlib::PyOpNode>(
            *graph, "concat_1", graphlib::OpType("concatenate", {}, {{"dim", -2}}), {pkv_input, transpose_1});
        auto output_1 = create_output(*graph, "output_1", concat_1);
