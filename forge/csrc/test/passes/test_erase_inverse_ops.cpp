...
TEST_F(EraseInverseOpsSqueezeAndUnsqueezeTest, EraseInverseOpsSqueezeAndUnsqueeze)
{
    ...
    graphlib::OpNode *reshape_op = nodes[4]->as<graphlib::PyOpNode>();
    ASSERT_EQ(reshape_op->op_name(), "reshape");
    ASSERT_EQ(reshape_op->name(), "squeeze_operand_commute_clone2");

    auto reshape_attrs = reshape_op->named_attrs();
    ASSERT_TRUE(reshape_attrs.count("shape"));

    auto shape_vector = std::get<std::vector<int>>(reshape_attrs["shape"]);
    std::vector<std::uint32_t> shape_vector_uint(shape_vector.begin(), shape_vector.end());
    graphlib::Shape shape = graphlib::Shape::create(shape_vector_uint);
    EXPECT_EQ(shape, graphlib::Shape::create({1, 256, 256}));
}
