...
TEST_F(DecomposeNdReshapeSplitTest, basic_dimension_split_optimization)
{
    ...
    for (auto node : graph->nodes()) {
        if (node->node_type() == graphlib::kPyOp) {
            ...
            if (op_node->op_name() == "index") index_count_before++;
        }
    }
    ...
    auto reshape_it = std::find_if(graph->nodes().begin(), graph->nodes().end(),
        [](graphlib::Node* n) { return n->name() == "reshape"; });
    ...
    for (auto node : graph->nodes()) {
        if (node->node_type() == graphlib::kPyOp) {
            auto op_node = node->as<graphlib::PyOpNode>();
            if (op_node->op_name() == "index") index_count_after++;
        }
    }
