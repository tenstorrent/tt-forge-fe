...
std::vector<graphlib::Shape> generate_input_shapes()
{
    ...
    // Generate systematic shape ranges
    auto shapes_1d = shape_range({1}, {8});  // 1D: [1] to [8]
    shapes.insert(shapes.end(), shapes_1d.begin(), shapes_1d.end());

    auto shapes_2d = shape_range({1, 1}, {4, 4});  // 2D: [1,1] to [4,4]
    shapes.insert(shapes.end(), shapes_2d.begin(), shapes_2d.end());

    auto shapes_3d = shape_range({1, 1, 1}, {3, 3, 3});  // 3D: [1,1,1] to [3,3,3]
