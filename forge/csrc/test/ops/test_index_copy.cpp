        ...
        for (int dim = 0; dim < static_cast<int>(input_shape.size()); ++dim)
        {
            ...
            for (uint32_t num_indices = 1; num_indices <= std::min(dim_size, 3u); ++num_indices)
            {
                ...
                if (dim == 2 && input_shape.size() == 4)
                {
                    ...
                    std::vector<torch::Dtype> dtypes = {torch::kFloat, torch::kLong, torch::kFloat}; // input, index, source
                    if (valid_inputs(shapes, dim))
                    {
                        // Create sequential index tensor from 0 to num_indices-1
                        torch::Tensor sequential_indices = torch::arange(0, static_cast<int64_t>(index_shape[0]), torch::kLong);
                        std::vector<torch::Tensor> override_tensors = {{}, sequential_indices, {}}; // Only override index tensor
                ...
                else
                {
                    ...
                    for (uint32_t test_idx = 0; test_idx < num_test_indices; ++test_idx)
                    {
                        ...
                        if (valid_inputs(shapes, dim))
                        {
                            ...
                            std::vector<torch::Tensor> override_tensors = {{}, random_index_tensor, {}}; // Only override index tensor
            ...
            if (dim_size > 0)
            {
                ...
                if (neg_dim == -2 && input_shape.size() == 4)
                {
                    ...
                    if (valid_inputs(shapes, neg_dim))
                    {
                        ...
                        torch::Tensor sequential_indices = torch::arange(0, static_cast<int64_t>(input_shape[0]), torch::kLong);
                        std::vector<torch::Tensor> override_tensors = {{}, sequential_indices, {}}; // Only override index tensor
