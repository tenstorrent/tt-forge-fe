...
bool Shape::can_be_broadcasted_to(const Shape &other) const
{
         ...
         ++dim, ++other_dim)
    {
        if (*dim < *other_dim)
        {
            // If current dim is less than other dim, it can be broadcasted only if it is 1.
            if (*dim != 1)
            {
                return false;
            }
        }
        else if (*dim > *other_dim)
        {
            // If current dim is greater than other dim, it cannot be broadcasted.
            return false;
        }
    }
