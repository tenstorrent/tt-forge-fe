...
void configure_output_data_formats(graphlib::Graph *graph, std::optional<DataFormat> default_df_override)
{
    for (Node *node : graph->nodes())
    {
        ...
        if (node_is_input)
        {
            ...
            {
                TT_ASSERT(
                    node->output_df() == default_df_override || is_integer_data_format(node->output_df()),
...
{
    bool enable_stochastic_rounding = env_as<bool>("FORGE_ENABLE_STOCHASTIC_ROUNDING", false);
