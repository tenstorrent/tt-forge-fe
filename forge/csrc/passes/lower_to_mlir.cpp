...
class MLIRGenerator
{
   ...
   private:
    ...
    mlir::Value emit_mlir_ttforge_convolution_op(tt::graphlib::Graph *graph, tt::graphlib::OpNode *op_node)
    {
        ...
        if (is_transpose) skip_attrs.insert("output_padding");

        for (const auto &[name, value] : op_attrs)
        {
            if (skip_attrs.count(name)) continue;

            // Force manual check for stride to ensure it uses I64
            if (name == "stride") {
                 auto stride_vec = std::get<std::vector<int>>(value);
                 std::vector<int64_t> mlir_strides(stride_vec.begin(), stride_vec.end());
                 mlir_attributes.push_back(builder_.getNamedAttr("window_strides", builder_.getDenseI64ArrayAttr(mlir_strides)));
                 continue;
            }
        ...
        // Weight Dilation (Default 1,1)
        mlir_attributes.push_back(builder_.getNamedAttr("weight_dilation", builder_.getDenseI64ArrayAttr({1, 1})));

        mlir_attributes.push_back(builder_.getNamedAttr("window_reversal", builder_.getDenseBoolArrayAttr({false, false})));
        ...
        // Create Op 
        return builder_.create<mlir::tt::ttir::ConvolutionOp>(
            get_tt_forge_operation_location(graph, op_node),
            return_types,
            operands,
            mlir_attributes
