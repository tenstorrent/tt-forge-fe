...
void decompose_nd_reshape_split(graphlib::Graph *graph)
{
    for (auto node : graph->nodes())
    {
        ...
        for (uint32_t i = 0; i < consumers.size(); i++)
        {
            ...
            op->op_type().attr[0] = original_dim;                         // dim
            op->op_type().attr[1] = start * new_dim_size;                 // start
            op->op_type().attr[2] = start * new_dim_size + new_dim_size;  // stop
            op->op_type().attr[3] = 1;                                    // stride
