...
static bool find_path_from_concat_to_select(
    graphlib::Graph *graph,
    std::vector<graphlib::Node *> &operands,
    int num_operands,
    ...
    for (int i = 0; i < num_operands; i++)
    {
        ...
        while (can_fuse)
        {
            ...
            if (current_op->op_name() == "select")
            {
                ...
                (current_op->op_name() == "add" or current_op->op_name() == "multiply" or
                 current_op->op_name() == "subtract"))
            {
    ...
    std::vector<int> &merge_group_operand_indices)
{
    ...
    auto longest_path = std::max_element(
        ops_to_fuse.begin(),
        ops_to_fuse.end(),
    ...
    for (auto iter : ops_to_fuse)
    {
        ...
        for (long unsigned int i = 0; i < value.size(); i++)
        {
            ...
            for (long unsigned int j = merge_group_idx; j < longest_path->second.size(); j++)
            {
                ...
                if (reference_op->op_name() == op->op_name() and
...
static bool fuse_per_channel_concat(graphlib::Graph *graph, graphlib::OpNode *concat)
{
    ...
    for (auto iter : merge_groups)
    {
        ...
        else
        {
            ...
            auto const_concat =
                graph->add_node(concat->clone(const_name), graph->get_subgraph_id_for_node(concat->id()));
                ...
                else
                {
                    ...
                    auto const_node = graph->add_node(
                        graphlib::create_node<graphlib::ConstantInputNode>(
                            ...
                            const_tensor,
                            const_shape),
                        graph->get_subgraph_id_for_node(first_op_in_group->id()));
            ...
            auto eltwise =
                graph->add_node(eltwise_op->clone(eltwise_name), graph->get_subgraph_id_for_node(eltwise_op->id()));
