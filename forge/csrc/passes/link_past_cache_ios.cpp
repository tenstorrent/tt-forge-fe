...
bool shape_compatible(graphlib::OpNode *output_producer, graphlib::Node *input_consumer)
{
    ...
    {
        int dim = std::get<int>(output_producer->op_type().named_attrs_.at("dim"));
...
std::unordered_map<graphlib::Node *, graphlib::Node *> link_cache_outputs_to_parameters(graphlib::Graph *graph)
{
    ...
    // Search through output nodes, if producer chain is
    // param-><transpose>->MM-><reshape>-><add>->hslice-><concatonate>->output with the <> nodes being optional, the
    // output is linked with the parameter
    ...
    for (Node *output_node : graph->ordered_module_outputs())
    {
        ...
        if (operand->node_type() == graphlib::NodeType::kInput and
            operand->as<graphlib::InputNode>()->input_type() == graphlib::InputNodeType::Parameter)
        {
            param_to_output[output_node] = operand;
            log_debug(
                LogGraphCompiler,
                "Linking present cache output {} to parameter {}",
                output_node->name(),
                operand->name());
        }
    }
...
void rotate_params(graphlib::Graph *graph, std::vector<graphlib::Node *> params_to_rotate)
{
    ...
    for (graphlib::Node *param : params_to_rotate)
    {
        ...
        graphlib::Edge read_edge_right(
            param->id(), (graphlib::PortId)0, index_node_right->id(), (graphlib::PortId)0, graphlib::EdgeType::kData);
        ...
        graphlib::Edge read_edge_left(
            param->id(), (graphlib::PortId)0, index_node_left->id(), (graphlib::PortId)0, graphlib::EdgeType::kData);
        ...
        auto nop_node =
            graph->add_node(graphlib::create_node<graphlib::PyOpNode>(param->name() + "_nop", nop), subgraph_index);
        graphlib::Edge nop_edge(
            concat_node->id(), (graphlib::PortId)0, nop_node->id(), (graphlib::PortId)0, graphlib::EdgeType::kData);
        ...
        auto output_node = graph->add_node(
            graphlib::create_node<graphlib::OutputNode>(param->name() + "_rotate_output"), subgraph_index);
        graphlib::Edge output_edge(
            nop_node->id(), (graphlib::PortId)0, output_node->id(), (graphlib::PortId)0, graphlib::EdgeType::kData);
        graph->add_edge(output_edge);
        graphlib::Edge write_edge(
            output_node->id(),
            (graphlib::PortId)0,
            param->id(),
            (graphlib::PortId)0,
...
graphlib::Node *detect_inputs_to_convert(graphlib::Graph *graph, graphlib::Node *producer)
{
    ...
    {
        return (
            n->node_type() == graphlib::NodeType::kInput and
            n->as<graphlib::InputNode>()->input_type() == graphlib::InputNodeType::Activation);
    };
    ...
    for (auto producer_input : producer_input_nodes)
    {
        ...
        for (auto input_consumer : input_consumers)
        {
            ...
            {
                std::string input_consumer_span =
                    input_consumer->as<graphlib::TaggedNode>()->tag_value<std::string>("layer");
                ...
                // if span matches and node shape is compatible, the activation node maybe supposed to be linked by
                // past-cache
...
std::map<std::string, std::size_t> convert_inputs_to_params(
    graphlib::Graph *graph, std::unordered_map<graphlib::Node *, graphlib::Node *> param_to_output)
{
    ...
    for (graphlib::Node *output_node : graph->nodes_by_type(graphlib::NodeType::kOutput))
    {
        ...
        if (is_concat)
        {
            ...
            if (slice_factor > 1)
            {
                ...
                {
                    graphlib::Edge control_edge(
                        producers[0]->id(),
                        (graphlib::PortId)0,
                        stack_node->id(),
                        (graphlib::PortId)0,
                        graphlib::EdgeType::kControl);
        ...
        // replace input node with parameter if this is a new cache, otherwise remove input node and connect to existing
        // parameter
        ...
        {
            // cross-attention cache, no concatination, cache is consumed as is link_cache_outputs_to_parameters
            // couldn't find it for this subgraph
            unsigned int current_subgraph = graph->get_subgraph_id_for_node(output_node->id());
            std::vector<graphlib::Node *> current_subgraph_outputs =
                graph->ordered_module_outputs_by_subgraph_index(current_subgraph);
            int position = std::find(current_subgraph_outputs.begin(), current_subgraph_outputs.end(), output_node) -
                           current_subgraph_outputs.begin();
                ...
                if (subgraph_outputs.size() == current_subgraph_outputs.size())
                {
                    if (param_to_output.count(subgraph_outputs[position]))
                    {
                        if (subgraph_outputs[position]->shape() == output_node->shape())
                        {
                            ...
                            log_warning(
                                LogGraphCompiler,
                                "No paramater found to produce {}, linking to {}",
                                output_node->name(),
                                producer_param->name());
        ...
        graphlib::Edge new_input_edge(
            input_param->id(),
            (graphlib::PortId)0,
            graph->data_users(input_node)[0]->id(),
            (graphlib::PortId)0,
            graphlib::EdgeType::kData);
        graph->add_edge(new_input_edge);
        for (size_t i = 1; i < graph->data_users(input_node).size(); ++i)
        {
            graphlib::Edge new_input_edge_other(
                input_param->id(),
                (graphlib::PortId)i,
                graph->data_users(input_node)[i]->id(),
                (graphlib::PortId)0,
                graphlib::EdgeType::kData);
    ...
    for (graphlib::Node *output_node : graph->nodes_by_type(graphlib::NodeType::kOutput))
    {
        if (std::find(processed_outputs.begin(), processed_outputs.end(), output_node) != processed_outputs.end() or
            param_to_output.count(output_node) == 0)
        {
            continue;
        }
    ...
    if (env_as<bool>("FORGE_ROTATE_PAST_CACHE_PARAMS", false))
