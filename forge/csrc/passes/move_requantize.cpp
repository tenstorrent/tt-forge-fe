...
static std::vector<graphlib::Node *> find_path_to_requant(graphlib::Graph *graph, graphlib::OpNode *initial_op)
{
...
void commute_through_requant(graphlib::Graph *graph, std::vector<graphlib::Node *> const &path)
{
    ...
    for (std::size_t i = 1; i < path.size(); ++i)
    {
        ...
        for (graphlib::Edge operand_edge : consumer_operands)
        {
            ...
            graphlib::Node *clone =
                graph->add_node(first->clone(name), graph->get_subgraph_id_for_node(operand_edge.producer_node_id));
    ...
    for (auto requant_out_edge : graph->user_data_edges(last))
    {
        ...
        graphlib::Node *curr_node =
            graph->add_node(tm_->clone(name), graph->get_subgraph_id_for_node(requant_out_edge.consumer_node_id));
