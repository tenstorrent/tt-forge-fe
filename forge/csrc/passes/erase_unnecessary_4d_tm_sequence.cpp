...
static void commute_4d_tm_ops(graphlib::Graph *graph, std::vector<graphlib::Node *> path)
{
    ...
    for (int i = 0; i < fourth_dim; i++)
    {
        ...
        new_node->set_shape(
            graphlib::Shape::create({1, unsigned(third_dim), first_reshape_out_shape[2], first_reshape_out_shape[3]}));
    ...
    graphlib::PyOpNode *new_interleave_node = graph->add_node(
        graphlib::create_node<graphlib::PyOpNode>(op_name, op_type), graph->get_subgraph_id_for_node(first->id()));
    new_interleave_node->set_shape(
        graphlib::Shape::create({1, unsigned(orig_third_dim), first_reshape_out_shape[2], first_reshape_out_shape[3]}));
    ...
    for (int idx = 0; idx < fourth_dim; ++idx)
    {
        ...
        auto new_edge0 = graphlib::Edge(input_edge.producer_node_id, 0, select_node->id(), 0, input_edge.edge_type);
        graph->add_edge(new_edge0);
        graph->copy_edge_attributes(input_edge, new_edge0);

        auto new_edge1 = graphlib::Edge(select_node->id(), 0, new_interleave_node->id(), idx, input_edge.edge_type);
