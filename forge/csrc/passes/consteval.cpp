...
static bool input_can_consteval(graphlib::Graph *graph, graphlib::InputNode *input)
{
    ...
    auto is_broadcast_or_repeat = [](graphlib::Node *n)
    {
        ...
        if (not op)
            return false;
        return op->new_op_type() == ops::OpType::Broadcast or op->new_op_type() == ops::OpType::Repeat or
               op->new_op_type() == ops::OpType::RepeatInterleave;
    };
...
void run_consteval_graph_pass(graphlib::Graph *graph)
{
    ...
    while (not needs_visit.empty())
    {
        ...
        // Will consteval the user(s) of an input as long as each user is identical and have no operands other than the
        // input.
