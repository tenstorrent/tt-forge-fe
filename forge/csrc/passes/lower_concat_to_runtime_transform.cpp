...
{
    bool concat_on_host = env_as<bool>("FORGE_CONCAT_ON_HOST");
    ...
    for (graphlib::Node *output_node : graph->nodes_by_type(graphlib::NodeType::kOutput))
    {
        ...
        if (not producers[0]->as<graphlib::OpNode>() or
            producers[0]->as<graphlib::OpNode>()->op_name() != "concatenate")
            continue;
        ...
        for (size_t i = 1; i < concat_producers.size(); i++)
        {
            ...
            auto output = graph->add_node(
                graphlib::create_node<graphlib::OutputNode>(producer->name() + "_" + std::to_string(i)),
                subgraph_index);
            graphlib::Edge output_edge(
                producer->id(), (graphlib::PortId)0, output->id(), (graphlib::PortId)0, graphlib::EdgeType::kData);
        ...
        for (size_t index = 0; index < concat_outputs.size(); index++)
        {
            graphlib::RuntimeTensorTransform transform =
                graphlib::RuntimeTensorTransform ::ConcatenateOnHost(concat_group, index, dim);
