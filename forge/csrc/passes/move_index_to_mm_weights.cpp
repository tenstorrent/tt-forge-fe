    ...
    graphlib::OpNode *index_op)
{
    ...
    for (size_t i = 0; i < path.size(); ++i)
    {
        ...
        size_t port_id = (is_type(org_node, "matmul")) ? 1 - get_non_param_port(graph, org_node)
                                                       : get_non_param_port(graph, org_node);
        auto new_edge = graphlib::Edge(
            prev_node->id(), graph->data_users(prev_node).size(), clone->id(), port_id, graphlib::EdgeType::kData);
        ...
        if (is_type(clone, "add"))
        {
            new_edge = graphlib::Edge(
                bias_node->id(),
                graph->data_users(bias_node).size(),
                clone->id(),
                1 - port_id,
        ...
        else if (is_type(clone, "matmul"))
        {
            ...
            new_edge = graphlib::Edge(
                input_node->id(),
                graph->data_users(input_node).size(),
                clone->id(),
                1 - port_id,
                graphlib::EdgeType::kData);
            graph->add_edge(new_edge);
...
std::vector<graphlib::OpType::Attr> recalculate_index_attr(
    graphlib::Graph *graph, graphlib::Node *next_node, std::vector<graphlib::OpType::Attr> &old_attr)
{
...
bool reached_target_node(graphlib::Graph *graph, graphlib::Node *n, graphlib::Node *target_node)
{
...
graphlib::Node *insert_index_op(
    graphlib::Graph *graph, graphlib::OpNode *index_op, graphlib::Edge &user_edge, std::vector<graphlib::Node *> &path)
{
    // clone new select op
...
std::tuple<graphlib::Node *, graphlib::Edge> get_param_operand_info(graphlib::Graph *graph, graphlib::Node *node)
{
    ...
    for (auto e : node_user_edges)
    {
        ...
        if (user->node_type() == graphlib::NodeType::kInput and
            user->as<graphlib::InputNode>()->input_type() == graphlib::InputNodeType::Parameter)
