    ...
    graphlib::Node* terminal_node)
{
    log_debug(
        LogTMFusion,
        "Trying to replace pattern from {} to {}.",
        pattern_to_string(current_pattern),
        pattern_to_string(replace_pattern));
        ...
        if (users.size() > 1)
        {
            ...
            for (auto& user : users)
            {
                ...
                if (user->as<graphlib::OpNode>()->op_type().op != op_type.op)
                {
                    ...
                    log_debug(
                        LogTMFusion,
                        "There is a user at the end of the matched pattern which is different op - cannot fuse tms.");
                    return false;
                }
                ...
                if (user->shape() != terminal_node->shape())
                {
                    ...
                    log_debug(
                        LogTMFusion,
                        "There is a user at the end of the matched pattern which is same op but different shape - "
                        "cannot fuse tms.");
                    return false;
                }
            }
    ...
    for (uint i = 0; i < replace_pattern.size(); i++)
    {
        ...
        auto new_node = graph->add_node(
            std::make_unique<graphlib::PyOpNode>(name, op.as_op_type()),
            graph->get_subgraph_id_for_node(sequence_producer->id()));
        fuse_node = new_node;
    ...
    log_info(
        LogTMFusion,
        "Found replaceable TM sequence. Fuse from {} tms into {} tms.",
        current_pattern.size(),
        replace_pattern.size());
    return true;
}
        ...
        for (auto& pattern : pattern_map)
        {
            ...
            for (auto* node : graphlib::topological_sort(*graph))
            {
                ...
                else if (current_pattern.size() == search_pattern.size())
                {
                    ...
                    // Verify i/o shape by calling forge shape function
                    ...
                    if (same_pattern and same_shape)
                    {
                        ...
                        bool is_pattern_replaced = replace_pattern_with_new_pattern(
                            graph, current_pattern, replace_pattern, sequence_producer, node);
                        // Break and reset
                        current_pattern.clear();
                        sequence_producer = nullptr;
