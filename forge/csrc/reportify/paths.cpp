...
static std::string get_variant(std::string const& test_name)
{
    ...
    if (auto maybe = env_as_optional<std::string>("FORGE_REPORTIFY_POSTFIX"); maybe and not test_name.empty())
    {
        variant = "." + *maybe;
    }
...
std::string get_default_reportify_path_for_sage_reports(const std::string& test_name)
{
...
std::string get_pass_reports_path_from_reportify_dir(const std::string& reportify_dir, const std::string& test_name)
{
