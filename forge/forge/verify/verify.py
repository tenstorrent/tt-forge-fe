...
from forge.verify.compare import compare_tensor_to_golden, compute_pcc_and_atol
...
def verify(
    ...
    if forge_property_handler is not None:
        pcc, atol = compute_pcc_and_atol(fw_out, co_out)
        logger.info(f"calculated pcc = {pcc}")
        logger.info(f"calculated atol = {atol}")
        if pcc is not None:
