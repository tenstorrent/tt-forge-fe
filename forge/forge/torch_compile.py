...
_tt0 = None
_compile_cache = None
_compile_cache_dir = os.environ.get("FORGE_COMPILE_CACHE_DIR", "tt_build")
...
def _build_backend_compile_request(
    ...
    # Backend Compile
    bcfg = forge._C.backend_api.BackendConfig(
        device.type,
        device.arch,
        forge._C.backend_api.DeviceMode.CompileAndRun,
...
def compile_torch(
    ...
    with torch.no_grad():
        forge_decompositions = get_forge_decompositions()
...
def _torch_compile(
    ...
    cache &= not bool(os.environ.get("FORGE_DISABLE_COMPILE_CACHE", "0"))
