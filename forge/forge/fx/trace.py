...
class IOTracer:
        ...
        def trace(node: torch.fx.Node) -> Set[torch.fx.Node]:
            for user in node.users:
                if user.op == "output":
                    ...
                    user in node_to_output
                ):  # depth-first, so we should have already reached the outputs if we hit the node again - no cycles
    ...
    def trace_for_cycle(
        self, input_node: torch.fx.Node, outputs_to_dest_node: Dict[torch.fx.Node, Set[torch.fx.Node]]
