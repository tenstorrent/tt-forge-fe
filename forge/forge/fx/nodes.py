...
def process_clamp(node, forge_op_name):
    ...
    if min_ is None:
        ...
        return ForgeNode(OpType("relu", [max_, "max"]), inputs)
    elif max_ is None:
        ...
        for i, arg in enumerate(node.args[1]):
            if arg != i:
                if dim0 is None:
                    ...
                    assert False, "Multi axis permute needs to be added to forge"
...
def is_supported_op(torch_op_name, node: torch.fx.Node):
    if torch_op_name not in dynamo_to_forge_function:
        return False
...
def get_forge_node(torch_op_name, node):
...
def append_to_graph(graph, module, aten_module, activations, subgraph_idx, inputs_per_subgraph, outputs_per_subgraph):
    ...
    def process_function(node):
        ...
        if op_name in torch_constant_ops:
            ...
            node_to_id[node] = add_op(graph, node, node.name, forge_node, subgraph_idx)
...
def call_function_is_nop(node):
    ...
    op_name = node.target.__name__
    if op_name in dynamo_to_forge_function:
        return dynamo_to_forge_function[op_name][1] == "nop"
    else:
        return False
...
def call_function_is_reshape(node):
    ...
    op_name = node.target.__name__
    if op_name in dynamo_to_forge_function:
        return dynamo_to_forge_function[op_name][1] == "reshape"
    else:
        return False
