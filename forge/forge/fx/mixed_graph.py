...
from forge.fx.graph_utils import (
    append_to_output,
    get_output_node,
    graph_lint,
    graph_to_device,
...
from forge.fx.nodes import (
    call_function_is_nop,
    call_function_is_reshape,
    torch_constant_ops,
)
...
class MixedGraph:
    ...
    def filter_unsupported_nodes(
        self,
        device_graph: torch.fx.Graph,
        unsupported_ops: Set[torch.fx.Node],
        unsupported_outputs: Set[torch.fx.Node],
        subgraph_id: int,
    ):
        ...
        scheduled_new_outputs: List[
            Tuple[torch.fx.Node, torch.fx.Node]
        ] = []  # Tuple - source in original graph, dest in new graph
        scheduled_new_inputs: List[
            Tuple[torch.fx.Node, torch.fx.Node, torch.fx.Node]
        ] = []  # Tuple - dest in original graph, source in original graph, source in new graph
        ...
        for node in device_graph.nodes:
            ...
            if node.op == "output":
                ...
                for driving_node in node.args[0]:
                    if driving_node in unsupported_ops or driving_node in unsupported_outputs:
                        logger.trace(
                            f"Moving output: {driving_node} {hex(id(driving_node))}, to copied node: {hex(id(copied_node_mapping[driving_node]))}"
                        )
            ...
            for arg in node.all_input_nodes:
                ...
                if arg in to_copy_ops:
                    ...
                    assert (
                        arg in copied_node_mapping
                    ), f"Node {arg} not copied, but it's an argument to {node} and in 'to copy ops'"
                ...
                if arg.op == "call_function":
                    ...
                    if op_name in torch_constant_ops:
                        ...
                        new_node = fallback_graph.node_copy(
                            arg, lambda x: x
                        )  # there should be no node args to copy in a constant op
        ...
        device_graphs, fallback_graphs = self.break_up_deadlocks(
            subgraph_id,
            device_graph,
            fallback_graph,
            new_io_mapping,
        ...
        for i, device_graph in enumerate(device_graphs):
            if len(device_graph.nodes) > 0 and (
                is_nop_graph(device_graph) or is_constant_graph(device_graph) or not has_output(device_graph)
            ):
    ...
    def break_up_deadlocks(
        ...
        copied_node_mapping: Dict[torch.fx.Node, torch.fx.Node],
        moved_output_mapping: Dict[torch.fx.Node, torch.fx.Node],
    ) -> Tuple[List[torch.fx.Graph], List[torch.fx.Graph]]:
        ...
        outputs_to_dest_node: Dict[torch.fx.Node, Set[torch.fx.Node]] = defaultdict(
            set
        )  # map of output node to inputs in other graphs
        ...
        while progress:
            ...
            assert (
                len(new_graph.nodes) > 0
            ), f"Graph {new_graph} is empty after trying to disjoin multiple fallback graphs"
            ...
            for node in new_graph.nodes:
                ...
                if original_node in self.output_nodes_per_subgraph[subgraph_id]:
                    self.output_nodes_per_subgraph[subgraph_id][
                        self.output_nodes_per_subgraph[subgraph_id].index(original_node)
                    ] = node
