...
class CaptureFX:
    ...
    def append_to_graph(
        ...
        activations = [
            torch.rand(sample_input.shape).to(sample_input.dtype).to("cpu") for sample_input in sample_inputs
        ]
        device_graph_changed, graph_inputs, intermediate_tensors, output_tensors, schedule = self._append_to_graph(
            module, aten_module, activations, subgraph_id
        )
    ...
    def eval_node(self, node):
        ...
        eval_args = [
            self.id_to_intermed[self.node_to_id[arg]] if isinstance(arg, torch.fx.node.Node) else arg
            for arg in node.args
        ]
        for idx, arg in enumerate(eval_args):
            if isinstance(arg, (list, tuple)):
                eval_args[idx] = [
                    self.id_to_intermed[self.node_to_id[a]] if isinstance(a, torch.fx.node.Node) else a for a in arg
                ]
                ...
                if node.op == "placeholder":
                    ...
                    if input_source.src == TensorSource.INPUT:
                        ...
                        if uid != -1 and config.enable_pt2_fx_graph_link:
                            ...
                            add_subgraph_io_link_edge(
                                self.get_forge_graph(),
                                self.output_nodes_per_subgraph[idx][output_index],
                                0,
                                input_id,
                                0,
                            )
                            created = True
                            logger.trace(
                                f"Linking input edge from {self.output_nodes_per_subgraph[idx][output_index]} to {input_id}"
                            )
                    ...
                    self.node_to_id[node] = self.add_param(
                        param_name_map[node.target], aten_module.state_dict()[node.target], graph_idx
                    )
...
def generate_device_inputs_from_sample_inputs(
    inputs: List[torch.Tensor], schedule: Schedule
