...
def populate_conv2d_transpose_attrs(graph, nid, attrs):
    ...
    padding = [int(padding) for padding in node["attrs"]["padding"][0]]
    assert all([p == padding[0] for p in padding]), "Forge only supports same padding on all sides"
    # TVM has padding [top, left, bottom, right]
    # Convert to [left right top bottom]
    attrs.append(padding[1])
    attrs.append(padding[3])
...
def populate_broadcast_attrs(graph, nid, attrs):
    ...
    for i, (inp_dim, out_dim) in enumerate(zip(input_shape, output_shape)):
        ...
        assert input_shape == output_shape, "Forge broadcast only support 1 dim"
...
def expand_compound_op(graph, nid, attrs, forge_graph, intermediate):
    class SubGraph(ForgeModule):
        ...
        def forward(self, *act):
            ...
            if self.fn_name == "softmax":
                return forge.op.nn.Softmax("softmax", act[0], dim=self.attributes[0])

            if self.fn_name == "layernorm":
                return forge.op.nn.Layernorm(
                    "layernorm",
                    act[0],
                    act[1],
                    act[2],
    ...
    assert isinstance(output, forge.Tensor)
    ...
    while pending_tensors:
        ...
        if isinstance(tensor, forge.Parameter):
            # parameter tensor
...
def compile_tvm_for_forge(forge_graph, torchmod, inputs, compiler_cfg, graph_name, verify_cfg=None):
    ...
    for nid, node in enumerate(graph["nodes"]):
        ...
        if node["op"] == "input":
            ...
            node["tensor"] = get_f_forge_eval(OpType(op_type, attrs))(forward_inputs)
            if node["name"] != "nop":
