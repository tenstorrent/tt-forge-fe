...
class TranspilerCodeGenerator:
    ...
    def write_forward(self):
        ...
        # Build operation info list (similar to TVM's ops dictionary)
        ops_info = []
        for node in sorted_nodes:
            op_info = node.emit()
            ops_info.append(op_info)

        # Compute which inputs to delete (matches TVM's delete_unneeded_outputs logic)
        # This ensures we only delete intermediate activations, not model inputs
        ...
        for op_info in ops_info:
            ...
            # Only delete inputs that are marked for deletion (intermediate activations, not model inputs)
    ...
    def _compute_inputs_to_delete(self, ops_info, forward_args):
        ...
        Matches TVM's delete_unneeded_outputs() logic exactly.

        Only deletes intermediate activations (outputs of operations), not:
        - Model inputs (forward_args) - these are not in op_outputs, so automatically skipped
        - Parameters/constants - these are not in op_outputs, so automatically skipped

        Args:
            ops_info: List of operation info dictionaries from node.emit() (in topological order)
            forward_args: List of model input names (should never be deleted, for extra safety check)

        Returns:
            Dictionary mapping output_name -> list of input names to delete
        """
        # Track consumers of each tensor (how many times it's used)
        consumers = {}
        op_outputs = set()

        # Step 1: Initialize consumers for return values (they're always needed)
        for output_name in self.tir_graph.outputs:
            consumers[output_name] = 1

        # Step 2: Count consumers for all operation inputs
        # This matches TVM's first loop: for op in ops.values()
        for op_info in ops_info:
            output_name = op_info["output_name"]
        ...
        # Step 3: Process operations in topological order and mark inputs for deletion
        # This matches TVM's second loop: for key in sorted(ops)
        ...
        for op_info in ops_info:
            ...
            for input_name in op_info["input_names"]:
                ...
                # This automatically excludes: model inputs, parameters, constants
                # (they are not in op_outputs, so we skip them)
                if input_name not in op_outputs:
                    continue

                # Extra safety check: explicitly skip model inputs (should never be deleted)
                # This is redundant but provides extra safety
                if input_name in forward_args:
                    continue

                # Decrement consumer count (this operation is using it)
                # This matches TVM: consumers[input_name] = consumers[input_name] - 1
                assert input_name in consumers, f"Input {input_name} not found in consumers dict"
                consumers[input_name] = consumers[input_name] - 1

                # If no more consumers, mark for deletion
                # This matches TVM: if consumers[input_name] == 0: ops[key].inputs_to_delete.append(input_name)
                if consumers[input_name] == 0:
                    inputs_to_delete.append(input_name)

            # Store inputs to delete for this operation
            if inputs_to_delete:
