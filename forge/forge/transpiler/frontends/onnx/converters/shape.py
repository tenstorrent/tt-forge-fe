...
class TransposeConverter(OnnxOpConverter):
    ...
    Converts ONNX Transpose to TIR TransposeNode or IdentityNode (if permutation is identity).
    Complex permutations are decomposed into a sequence of dimension swaps for efficient
    code generation.
    """
    ...
    def _normalize_perm(cls, perm: Any, input_rank: int) -> List[int]:
        ...
        If perm is None, returns default reverse permutation [n-1, n-2, ..., 0].
        Negative indices are converted to positive by adding input_rank.

        Args:
            perm: Permutation list/attribute (may contain negative indices or None)
            input_rank: Rank of input tensor

        Returns:
            Normalized permutation list with all positive indices
        """
        if perm is None:
            # Default: reverse all dimensions
            return list(range(input_rank - 1, -1, -1))
        # Convert negative indices to positive: -1 becomes input_rank-1, etc.
    ...
    def _validate_perm(cls, perm: List[int], input_rank: int) -> None:
        ...
        - Have length equal to input_rank
        - Contain exactly the integers [0, 1, ..., input_rank-1] (no duplicates, no missing)

        Args:
            perm: Permutation list to validate
            input_rank: Rank of input tensor
    ...
    def _is_identity_perm(cls, perm: List[int]) -> bool:
        ...
        Identity permutation: [0, 1, 2, ..., n-1] (no reordering)

        Args:
            perm: Permutation list

        Returns:
            True if permutation is identity, False otherwise
        """
    ...
    @classmethod
    def _convert_permutation_to_swaps(
        cls,
        ...
        current_input_dict, _ = build_input_output_dicts(node_proto, input_tensors, output_tensors)
        ...
        for swap_idx, (dim0, dim1) in enumerate(swap_sequence):
            ...
            if is_last:
                _, node_output_dict = build_input_output_dicts(
                    node_proto, input_tensors, output_tensors, output_names=[node_proto.output[0]]
                )
            else:
                ...
                intermediate_tensor_info = TensorInfo(
                    name=intermediate_name, shape=intermediate_shape, onnx_dtype=onnx_dtype
                )
                intermediate_tensors = OrderedDict()
                intermediate_tensors[intermediate_name] = intermediate_tensor_info
                _, node_output_dict = build_input_output_dicts(
                    node_proto, input_tensors, intermediate_tensors, output_names=[intermediate_name]
                )

            nodes.append(
                TransposeNode.create(
                    name=f"{base_name}_swap_{swap_idx}",
                    inputs=current_input_dict,
    ...
    def convert(
        ...
        return cls._convert_permutation_to_swaps(node_proto, input_tensors, output_tensors, perm, node_index)
...
class CastConverter(OnnxOpConverter):
    ...
    def convert(
        ...
        - Opset v1-v12: to as attribute
        - Opset v13+: to as optional input tensor (falls back to attribute)
        """
        node_name = node_proto.name if node_proto.name else f"Cast_{node_index}"

        if opset >= 13 and len(node_proto.input) > 1:
            # v13+: to as optional input tensor
            is_valid, to_dtype, error_msg = validate_constant_input(node_proto, input_index=1, graph_proto=graph_proto)
            ...
            tir_input_tensors = (
                {data_input: input_tensors[data_input]} if data_input in input_tensors else input_tensors
            )
        else:
            # v1-v12: to as attribute
            to_dtype = attrs.get("to", None)
            inputs = list(node_proto.input)
            tir_input_tensors = input_tensors
        ...
        cast_input_dict, cast_output_dict = build_input_output_dicts(
            node_proto, tir_input_tensors, output_tensors, input_names=inputs
        )

        return [CastNode.create(name=node_name, inputs=cast_input_dict, outputs=cast_output_dict, dtype=torch_dtype)]
...
class FlattenConverter(OnnxOpConverter):
    ...
    def _calculate_flatten_shape(cls, input_shape: Tuple, axis: int, node_name: str = None) -> Tuple[int, int]:
        ...
        if axis < 0 or axis > rank:
            node_info = f" in node '{node_name}'" if node_name else ""
            raise ValueError(
                f"Flatten{node_info}: Invalid axis={axis} for input rank={rank}. " f"Axis must be in range [0, {rank}]."
            )
    ...
    def _normalize_axis(cls, axis: int, input_rank: int, opset: int, node_name: str = None) -> int:
        ...
        if input_rank <= 0:
            node_info = f" in node '{node_name}'" if node_name else ""
            raise ValueError(
                f"Flatten{node_info}: Invalid input rank={input_rank}. " f"Input tensor must have rank >= 1."
            )
    ...
    def _validate_inputs(
        cls, node_proto: NodeProto, input_tensors: OrderedDict[str, TensorInfo], node_name: str
        ...
        if not node_proto.input:
            raise ValueError(
                f"Flatten node '{node_name}': No input provided. " f"Flatten requires exactly one input tensor."
            )
    ...
    def _convert_flatten_impl(
        ...
        if not node_proto.output:
            raise ValueError(
                f"Flatten node '{node_name}': No output provided. " f"Flatten requires exactly one output tensor."
            )
        ...
        return [ReshapeNode.create(name=node_name, inputs=input_dict, outputs=output_dict, shape=output_shape)]

    @classmethod
    def _extract_and_validate_axis(cls, attrs: Dict[str, Any], input_rank: int, opset: int, node_name: str) -> int:
        """
    ...
    @classmethod
    def _get_input_rank(cls, node_proto: NodeProto, input_tensors: OrderedDict[str, TensorInfo], node_name: str) -> int:
        """
    ...
    def convert(
        ...
        - Opset v1-v9: axis range [0, R] (non-negative only)
        - Opset v11+: axis range [-r, r] (negative values supported)
        """
        node_name = node_proto.name if node_proto.name else f"Flatten_{node_index}"
        ...
        # Extract and validate axis based on opset version
