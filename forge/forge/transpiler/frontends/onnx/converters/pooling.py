...
"""
ONNX Pooling operation converters.
...
class MaxPoolConverter(OnnxOpConverter):
    ...
    def convert(
        ...
        """
        MaxPool converter - single method handles all versions using opset parameter.

        - Opset v1-v7: Basic attributes, no ceil_mode, no dilations
        - Opset v8-v9: Added Indices output and storage_order
        - Opset v10: Added ceil_mode and dilations attributes
        - Opset v11-v21: Improved formulas with explicit ceil_mode handling, auto_pad always uses ceil
        - Opset v22+: auto_pad respects ceil_mode, added bfloat16 support
        """
    ...
    def _convert_maxpool_impl(
            ...
            if input_shape is None:
                ...
                pad_output_tensors = {
                    pad_output: TensorInfo(
                        name=pad_output, shape=tuple(padded_shape), onnx_dtype=getattr(input_info, "onnx_dtype", None)
                    )
                }
        ...
        pool_input_dict, pool_output_dict = build_input_output_dicts(
            node_proto, pool_input_tensors, output_tensors, input_names=pool_inputs
        )
        ...
        if kernel_dims == 1:
            pool_node = MaxPool1dNode.create(
                name=node_name,
                inputs=pool_input_dict,
                outputs=pool_output_dict,
...
class AveragePoolConverter(OnnxOpConverter):
    ...
    def convert(
        ...
        """
        AveragePool converter - single method handles all versions using opset parameter.

        - Opset v1-v10: Basic attributes (v1: no ceil_mode/count_include_pad, v7: count_include_pad, v10: ceil_mode)
        - Opset v11-v18: Improved auto_pad formulas, auto_pad respects ceil_mode
        - Opset v19+: Added dilations attribute
        """
        # Determine opset_version for internal implementation
        if opset < 11:
            opset_version = 10  # Most permissive for v1-v10
        elif opset < 19:
            opset_version = 11
        else:
            opset_version = 19

        return cls._convert_avgpool_impl(
            node_proto, input_tensors, output_tensors, attrs, node_index, graph_proto, opset_version=opset_version
        )
