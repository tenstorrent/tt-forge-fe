...
from forge.transpiler.frontends.onnx.converters.validation import validate_constant_input, handle_validation_error, ValidationError
...
def onnx_mode_to_pytorch(onnx_mode: str) -> str:
    ...
    if onnx_mode not in mode_mapping:
        raise ValueError(
            f"Unsupported ONNX Pad mode: {onnx_mode}. "
...
def validate_pytorch_nonconstant_padding(pads: List[int], axes: Optional[List[int]], 
                                        input_rank: int, mode: str) -> None:
    """
    ...
    if input_rank not in max_padded_dims:
        raise ValueError(
            f"PyTorch F.pad with {mode} mode only supports 2D-5D tensors. "
            f"Got {input_rank}D tensor."
        )
...
def convert_onnx_pads_to_pytorch(onnx_pads: List[int], axes: Optional[List[int]] = None, 
                                  input_rank: int = None, mode: str = 'constant') -> Tuple[int, ...]:
    """
...
class PadConverter(OnnxOpConverter):
    ...
    @classmethod
    def _validate_inputs(cls, node_proto: NodeProto, input_tensors: Dict[str, TensorInfo]) -> None:
        """Validate that required inputs exist."""
        if not node_proto.input:
            raise ValidationError(
                f"Pad node '{node_proto.name or 'unknown'}' has no inputs. "
                f"At least one input (data) is required."
            )
    ...
    @classmethod
    def _extract_and_validate_pads(cls, node_proto: NodeProto, attrs: Dict[str, Any],
                                    graph_proto, input_index: int = 1) -> List[int]:
        """
        ...
        if is_valid and pads is not None:
            ...
            try:
                ...
                raise ValidationError(
                    f"Pad node '{node_proto.name or 'unknown'}' pads input contains "
                    f"non-integer values: {e}"
                )
        else:
            ...
            try:
                ...
                raise ValidationError(
                    f"Pad node '{node_proto.name or 'unknown'}' pads attribute contains "
                    f"non-integer values: {e}"
                )
        ...
        if len(pads) == 0:
            raise ValidationError(
                f"Pad node '{node_proto.name or 'unknown'}' pads cannot be empty."
            )
        
        return pads
    
    @classmethod
    def _extract_constant_value(cls, node_proto: NodeProto, attrs: Dict[str, Any],
                                graph_proto, input_index: int = 2, default: float = 0.0) -> float:
    ...
    def _extract_axes(cls, node_proto: NodeProto, graph_proto, input_index: int = 3) -> Optional[List[int]]:
        ...
        try:
            ...
            raise ValidationError(
                f"Pad node '{node_proto.name or 'unknown'}' axes input contains "
                f"non-integer values: {e}"
            )
    ...
    @classmethod
    def _get_input_rank(cls, input_tensors: Dict[str, TensorInfo], 
                        node_proto: NodeProto) -> int:
    ...
    def _impl_v1(cls, node_proto: NodeProto, input_tensors: Dict[str, TensorInfo], 
        ...
        """
        Pad opset v1: pads, mode, and value as attributes.
        """
        ...
        try:
            ...
            # Extract pads from attribute
            pads = attrs.get('paddings', [])
            if not pads:
                raise ValidationError(
                    f"Pad node '{node_name}' (opset v1) requires 'paddings' attribute."
                )
            ...
            # Validate mode
            cls._validate_mode_for_opset(onnx_mode, 1, node_name)
            
            # Convert ONNX mode to PyTorch mode
            ...
            pytorch_pads = convert_onnx_pads_to_pytorch(pads, input_rank=input_rank, mode=mode)
            
            return [PadNode.create(
                name=node_name,
                inputs=[node_proto.input[0]],  # Only data input, pads and value are attributes in v1
    ...
    @classmethod
    def _impl_v2(cls, node_proto: NodeProto, input_tensors: Dict[str, TensorInfo],
                  output_tensors: Dict[str, TensorInfo], attrs: Dict[str, Any],
                  node_index: int, graph_proto=None) -> List:
        """
        Pad opset v2-v17: pads and constant_value as inputs, mode as attribute.
        
        Note: Versions v11 and v13 only add type support (bfloat16, bool, complex, string)
        but have the same functional behavior as v2, so they are handled by this implementation.
        The get_converter method automatically selects this for opset 2-17.
        """
        node_name = node_proto.name if node_proto.name else f"Pad_{node_index}"
        ...
        try:
            ...
            return [PadNode.create(
                name=node_name,
                inputs=[node_proto.input[0]],  # Only data input, pads and constant_value are embedded
                outputs=[node_proto.output[0]],
                input_tensors=input_tensors,
                output_tensors=output_tensors,
                pad=pytorch_pads,
    ...
    def _impl_v18(cls, node_proto: NodeProto, input_tensors: Dict[str, TensorInfo],
                  ...
                  node_index: int, graph_proto=None) -> List:
        """
        Pad opset v18+: Adds axes input for selective axis padding.
        
        Note: Version v19 adds wrap mode support, but the functional behavior
        is the same (wrap mode is handled by onnx_mode_to_pytorch conversion).
        The get_converter method automatically selects this for opset 18+.
        """
        node_name = node_proto.name if node_proto.name else f"Pad_{node_index}"
        ...
        try:
            ...
            # Extract and validate mode first (needed to determine input order)
            onnx_mode = attrs.get('mode', 'constant')
            # Note: We don't know the exact opset here, but v18+ supports wrap mode
            # The validation will catch if wrap is used in v18 (shouldn't happen)
            mode = onnx_mode_to_pytorch(onnx_mode)
            
            # Extract constant_value and axes
            # Input order is ALWAYS: [data, pads, constant_value, axes?]
            # Note: We always provide constant_value as the 3rd input (even for non-constant modes)
            # to satisfy ONNX Pad input order requirements (constant_value must be type T, same as data)
            # For non-constant modes, constant_value is ignored but must be present with correct type
            value = cls._extract_constant_value(node_proto, attrs, graph_proto, input_index=2)
            if onnx_mode != 'constant':
                # For non-constant modes, constant_value is not used (but still present as input)
                value = 0.0  # Not used for non-constant modes
            
            # Axes is always at index 3 (if present), regardless of mode
            ...
            if axes is not None:
                ...
                # Validate axes are within range (will be checked in convert_onnx_pads_to_pytorch)
                for axis in axes:
            ...
            # Convert ONNX pads format to PyTorch format (with axes support)
            pytorch_pads = convert_onnx_pads_to_pytorch(pads, axes=axes, input_rank=input_rank, mode=mode)
            
            return [PadNode.create(
                name=node_name,
                inputs=[node_proto.input[0]],  # Only data input, other inputs are embedded
                outputs=[node_proto.output[0]],
                input_tensors=input_tensors,
                output_tensors=output_tensors,
                pad=pytorch_pads,
