...
from forge.transpiler.core.exceptions import DebugValidationError
...
def debug_node_output(
    onnx_model: onnx.ModelProto, inputs: list, node_outputs: dict, onnx_node, rtol: float = 1e-3, atol: float = 1e-4
):
    """
    ...
    try:
        ...
        for i, output_name in enumerate(onnx_node.output):
            ...
            if np.allclose(predicted_np, expected_np, rtol=rtol, atol=atol):
                logger.info(f"[{onnx_node.name}] ✓ {output_name} matches ONNXRuntime " f"(shape: {predicted_np.shape})")
            else:
        ...
        # Re-raise debug validation errors (shape/value mismatches)
        raise
    except Exception as e:
        # Unexpected errors during debug comparison should also raise
        error_msg = f"Unexpected error in debug comparison for node {onnx_node.name}: {e}"
        logger.error(error_msg, exc_info=True)
        raise DebugValidationError(
            error_msg, frontend_node_name=onnx_node.name if hasattr(onnx_node, "name") else None
        ) from e
