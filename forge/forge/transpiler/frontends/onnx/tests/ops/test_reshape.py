...
class TestReshape:
    ...
    def test_reshape_basic(self, opset_version, input_shape, output_shape, dtype):
        ...
        if input_shape == output_shape:
            assert tir_graph.nodes[0].op_type in ["Reshape", "Identity"], \
                f"Expected Reshape or Identity node for same shape, got {tir_graph.nodes[0].op_type}"
        else:
            assert tir_graph.nodes[0].op_type == "Reshape", \
                f"Expected Reshape node, got {tir_graph.nodes[0].op_type}"
    ...
    @pytest.mark.parametrize("input_shape, shape_with_neg1, expected_shape", [
        # -1 in first position
    ...
    @pytest.mark.parametrize("opset_version", [1, 5, 13, 14, 18])
    @pytest.mark.parametrize("input_shape, shape_with_zero, expected_shape", [
    ...
    def test_reshape_with_zero_explicit(self, opset_version, input_shape, shape_with_zero, expected_shape):
        ...
        assert tir_graph.nodes[0].op_type == "Full", \
        ...
        assert tir_output.numel() == 0, \
            f"Expected empty tensor, got {tir_output.numel()} elements"
        assert tuple(tir_output.shape) == expected_shape, \
    ...
    def test_reshape_empty_shape_scalar(self, opset_version):
        ...
        if onnx_output.shape == ():
            ...
            assert np.allclose(tir_value, onnx_output.item(), rtol=1e-5, atol=1e-6), \
    ...
    def test_reshape_allowzero_attribute(self, opset_version, allowzero):
        ...
        logger.info(
            f"✓ Reshape allowzero test passed: opset={opset_version}, allowzero={allowzero}"
        )
    ...
    def test_reshape_empty_shape_creates_flatten(self):
        ...
        assert tir_graph.nodes[0].op_type == "Reshape", \
            f"Expected Reshape node, got {tir_graph.nodes[0].op_type}"
        assert tir_graph.nodes[0].attrs.get('shape') == (-1,), \
    ...
    def test_reshape_empty_tensor_creates_fullnode(self):
        ...
        assert tir_graph.nodes[0].op_type == "Full", \
            f"Expected Full node for empty tensor, got {tir_graph.nodes[0].op_type}"
        assert tir_graph.nodes[0].attrs.get('shape') == tuple(expected_shape), \
            f"Expected shape {expected_shape}, got {tir_graph.nodes[0].attrs.get('shape')}"
        assert tir_graph.nodes[0].attrs.get('fill_value') == 0.0, \
    ...
    def test_reshape_dtype_comprehensive(self, opset_version, dtype):
        ...
        logger.info(
            f"✓ Reshape dtype test passed: opset={opset_version}, dtype={dtype}"
        )
    ...
    def test_reshape_identity_optimization(self, opset_version, input_shape):
        ...
        logger.info(
            f"✓ Reshape identity optimization test passed: opset={opset_version}, "
            f"input_shape={input_shape}"
        )
    
    @pytest.mark.parametrize("opset_version", [1, 5, 14])
    @pytest.mark.parametrize("input_shape, shape_with_copy", [
