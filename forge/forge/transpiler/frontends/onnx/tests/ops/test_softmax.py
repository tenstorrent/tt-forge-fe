...
class TestSoftmax:
    ...
    def test_softmax_basic(self, opset_version, input_shape, dtype):
        ...
        assert len(softmax_nodes) == 1, \
            f"Expected 1 SoftmaxNode, got {len(softmax_nodes)}. " \
            f"Nodes: {[n.op_type for n in tir_graph.nodes]}"
        
        softmax_node = softmax_nodes[0]
        assert len(softmax_node.inputs) == 1, \
            f"SoftmaxNode should have exactly 1 input, got {len(softmax_node.inputs)}: {softmax_node.inputs}"
        assert softmax_node.inputs[0] == "input_0", \
            f"SoftmaxNode input should be 'input_0', got {softmax_node.inputs[0]}"
        assert softmax_node.outputs[0] == "output_0", \
            f"SoftmaxNode output should be 'output_0', got {softmax_node.outputs[0]}"
        ...
        assert actual_axis == expected_axis, \
            f"SoftmaxNode axis should be {expected_axis}, got {actual_axis}"
    ...
    def test_softmax_different_axes(self, opset_version, axis):
        ...
        softmax_node = softmax_nodes[0]
        assert softmax_node.attrs.get('dim') == axis, \
    ...
    def test_softmax_all_equal_values(self, opset_version):
        ...
        input_data = {
            'input_0': np.ones(input_shape, dtype=np.float32) * 2.0  # All values are 2.0
        }
    ...
    def test_softmax_extreme_values(self, opset_version):
        ...
        assert tir_output[0, 0] > 0.99, \
            f"Large value should have high probability, got {tir_output[0, 0]}"
        # Second row: small value (-10.0) should have probability close to 0.0
        assert tir_output[1, 0] < 0.01, \
            f"Small value should have low probability, got {tir_output[1, 0]}"
    ...
    def test_softmax_single_element(self, opset_version):
        ...
        tir_output = comparison['tir_outputs']['output_0']
        assert np.allclose(tir_output, np.array([1.0]), rtol=1e-5, atol=1e-5), \
