...
class TestFlatten:
    ...
    def test_flatten_all_dimensions_and_axes(self, opset_version, input_shape, axis):
        ...
        verification = verify_tir_graph_structure(
            tir_graph,
            onnx_model,
            expected_op_types=["Reshape"]  # Flatten maps to Reshape
        )
        ...
        assert comparison['matches']['output_0'], \
    ...
    @pytest.mark.parametrize("input_shape, axis, expected_output_shape", [
        # 1D cases
        ((10,), 0, (1, 10)),
        ((10,), 1, (10, 1)),
        # 2D cases
    ...
    def test_flatten_dtypes(self, opset_version, dtype, np_dtype):
        ...
        assert len(comparison['errors']) == 0, f"Comparison errors: {comparison['errors']}"
        assert comparison['matches']['output_0'], \
            f"Output mismatch for dtype={dtype}, opset={opset_version}"
    ...
    def test_flatten_value_equality(self, opset_version):
        ...
        assert len(comparison['errors']) == 0, f"Comparison errors: {comparison['errors']}"
        assert comparison['matches']['output_0'], \
            f"Output mismatch for opset={opset_version}"
        ...
        np.testing.assert_array_equal(tir_flat, onnx_flat, 
            err_msg=f"Value mismatch: TIR output does not match ONNX output")
        
        # Verify the flattened output matches the original input when reshaped back
        input_flat = input_data.flatten()
        np.testing.assert_array_equal(tir_flat, input_flat,
            err_msg=f"Value mismatch: Flattened output does not preserve input values")
    ...
    def test_flatten_graph_structure(self):
        ...
        assert node.op_type == "Reshape", \
            f"Expected Reshape node, got {node.op_type}. Flatten should map to Reshape."
        
        assert node.name == "flatten_test", \
            f"Expected node name 'flatten_test', got {node.name}"
        
        assert node.inputs == ["input_0"], \
            f"Expected inputs ['input_0'], got {node.inputs}"
        
        assert node.outputs == ["output_0"], \
            f"Expected outputs ['output_0'], got {node.outputs}"
        
        # Verify shape attribute
        assert 'shape' in node.attrs, "Reshape node should have 'shape' attribute"
        assert node.attrs['shape'] == expected_output_shape, \
