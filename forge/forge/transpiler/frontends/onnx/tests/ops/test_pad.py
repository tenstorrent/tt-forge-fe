...
class TestPad:
    ...
    @staticmethod
    def _create_pad_model(opset_version, input_shape, pads, mode='constant', 
                          constant_value=0.0, axes=None, dtype=onnx.TensorProto.FLOAT):
    ...
    def test_opset_1_2_modes(self, opset_version, input_shape, mode):
        ...
        output_name = list(tir_graph.outputs)[0]
        assert output_name in tir_outputs, \
            f"Output '{output_name}' not found in TIR outputs. Available: {list(tir_outputs.keys())}"
        ...
        # Verify output dtype
        assert tir_output.dtype == torch.float32, \
            f"Output dtype mismatch: expected float32, got {tir_output.dtype}"
    ...
    def test_opset_1_2_constant_values(self, opset_version, constant_value):
        ...
        assert np.isclose(output_np[0, 0], constant_value), \
            f"Top-left corner should be {constant_value}, got {output_np[0, 0]}"
        assert np.isclose(output_np[0, -1], constant_value), \
            f"Top-right corner should be {constant_value}, got {output_np[0, -1]}"
        assert np.isclose(output_np[-1, 0], constant_value), \
            f"Bottom-left corner should be {constant_value}, got {output_np[-1, 0]}"
        assert np.isclose(output_np[-1, -1], constant_value), \
    ...
    def test_opset_18_plus_selective_axes_nonconstant(self, opset_version, axes, mode):
        ...
        if not all(axis in [1, 2, 3] for axis in normalized_axes):
            pytest.skip(f"For 4D tensors with {mode} mode, only dimensions [1, 2, 3] can be padded. "
        ...
        if normalized_axes != sorted(normalized_axes) or normalized_axes != expected_dims:
            pytest.skip(f"For 4D tensors with {mode} mode, must pad consecutive last dimensions. "
                       f"Requested axes: {axes} (normalized: {normalized_axes}), "
