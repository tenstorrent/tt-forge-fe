...
class TestRelu:
    ...
    def test_relu_basic(self, opset_version, input_shape, dtype):
        ...
        assert len(relu_nodes) == 1, \
            f"Expected 1 ReluNode, got {len(relu_nodes)}. " \
            f"Nodes: {[n.op_type for n in tir_graph.nodes]}"
        
        relu_node = relu_nodes[0]
        assert len(relu_node.inputs) == 1, \
            f"ReluNode should have exactly 1 input, got {len(relu_node.inputs)}: {relu_node.inputs}"
        assert relu_node.inputs[0] == "input_0", \
            f"ReluNode input should be 'input_0', got {relu_node.inputs[0]}"
        assert relu_node.outputs[0] == "output_0", \
            f"ReluNode output should be 'output_0', got {relu_node.outputs[0]}"
    ...
    def test_relu_integer_types(self, opset_version, input_shape, dtype):
        ...
        if dtype == onnx.TensorProto.INT8:
            input_data = {
                'input_0': np.random.randint(-50, 50, size=input_shape, dtype=np_dtype)
            }
        elif dtype == onnx.TensorProto.INT16:
            input_data = {
                'input_0': np.random.randint(-1000, 1000, size=input_shape, dtype=np_dtype)
            }
        else:
            input_data = {
                'input_0': np.random.randint(-10000, 10000, size=input_shape, dtype=np_dtype)
            }
    ...
    def test_relu_integer_negative_to_zero(self, opset_version):
        ...
        tir_output = comparison['tir_outputs']['output_0']
        expected = np.array([
            [0, 0, 10],    # -5 -> 0, 0 -> 0, 10 -> 10
            [0, 50, 0]     # -100 -> 0, 50 -> 50, -1 -> 0
        ], dtype=np.int32)
