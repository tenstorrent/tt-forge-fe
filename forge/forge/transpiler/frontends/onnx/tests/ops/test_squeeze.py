...
class TestSqueeze:
    ...
    def test_squeeze_multiple_axes(self, opset_version):
        ...
        squeeze_nodes = [n for n in tir_graph.nodes if n.op_type == "Squeeze"]
        assert len(squeeze_nodes) == 1, \
            f"Expected 1 Squeeze node, got {len(squeeze_nodes)}. " \
        ...
        if len(axes) > 1:
            assert isinstance(dim, (tuple, list)), \
                f"Expected tuple/list of dims for multiple axes, got {type(dim)}"
            assert len(dim) == len(axes), \
                f"Expected {len(axes)} dims, got {len(dim)}"
        else:
            assert isinstance(dim, int), \
                f"Expected int dim for single axis, got {type(dim)}"
    ...
    def test_squeeze_no_axes_input(self, opset_version):
        ...
        dim = squeeze_node.attrs.get('dim')
        assert isinstance(dim, (tuple, list)), \
            f"Expected tuple/list of dims, got {type(dim)}"
        assert set(dim) == {0, 2}, \
            f"Expected dims [0, 2], got {dim}"
