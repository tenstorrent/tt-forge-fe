...
class TestConcat:
    ...
    @pytest.mark.parametrize("input_shapes, axis, expected_shape, dtype", [
    ...
    def test_concat_basic(self, opset_version, input_shapes, axis, expected_shape, dtype):
        ...
        assert len(concat_nodes) == 1, \
            f"Expected 1 ConcatNode, got {len(concat_nodes)}. " \
        ...
        assert isinstance(concat_node.attrs['dim'], int), \
        ...
        assert concat_node.attrs['dim'] == normalized_axis, \
    ...
    def test_concat_negative_indices(self, opset_version):
        ...
        for input_shapes, axis, expected_shape in test_cases:
            ...
            concat_nodes = [n for n in tir_graph.nodes if n.op_type == "Concat"]
            assert len(concat_nodes) == 1, \
                f"Expected 1 ConcatNode for axis={axis}, got {len(concat_nodes)}"
            ...
            assert len(comparison['errors']) == 0, \
                f"Comparison errors for axis={axis}: {comparison['errors']}"
            assert all(comparison['matches'].values()), \
                f"Output mismatch for axis={axis}: {comparison}"
    ...
    def test_concat_single_input(self):
        ...
        assert len(concat_nodes) == 0, \
            f"Expected no ConcatNode for single input, got {len(concat_nodes)}"
        assert len(identity_nodes) == 1, \
            f"Expected 1 IdentityNode for single input, got {len(identity_nodes)}"
    ...
    def test_concat_v1_default_axis(self):
        ...
        concat_node = concat_nodes[0]
        assert concat_node.attrs['dim'] == 1, \
            f"Expected dim=1 (default), got {concat_node.attrs['dim']}"
    ...
    def test_concat_missing_axis_error(self, opset_version):
        ...
        # Should have 0 nodes (conversion failed)
        assert len(tir_graph.nodes) == 0, \
            f"Expected 0 nodes (conversion should fail), got {len(tir_graph.nodes)}"
    ...
    def test_concat_out_of_range_error(self, opset_version):
        ...
        for invalid_axis in invalid_axes:
            ...
            # Transpile - should fail or produce empty graph
            transpiler = ONNXToForgeTranspiler(debug=True, validate_model=True)
            tir_graph = transpiler.transpile(onnx_model)
            
            # Should have 0 nodes (conversion failed)
            assert len(tir_graph.nodes) == 0, \
                f"Expected 0 nodes for invalid axis={invalid_axis}, got {len(tir_graph.nodes)}"
    ...
    def test_concat_mismatched_ranks_error(self):
        ...
        # Should have 0 nodes (conversion failed)
        assert len(tir_graph.nodes) == 0, \
            f"Expected 0 nodes for mismatched ranks, got {len(tir_graph.nodes)}"
    ...
    def test_concat_mismatched_shapes_error(self):
        ...
        # Should have 0 nodes (conversion failed)
        assert len(tir_graph.nodes) == 0, \
            f"Expected 0 nodes for mismatched shapes, got {len(tir_graph.nodes)}"
    ...
    def test_concat_many_inputs(self):
        ...
        concat_node = concat_nodes[0]
        assert len(concat_node.inputs) == num_inputs, \
            f"Expected {num_inputs} inputs, got {len(concat_node.inputs)}"
