...
class UnsqueezeNode(TIRNode):
    ...
    Forge Unsqueeze requires dim as attribute (single int).
    """
    @staticmethod
    def create(name: str, inputs: List[str], outputs: List[str],
               input_tensors: Dict[str, TensorInfo],
    ...
    def convert_attrs_to_forge_attrs(self, attrs):
        """Convert PyTorch attrs to Forge attrs. Forge Unsqueeze requires dim as int (not tuple)."""
        forge_attrs = {}
        if 'dim' in attrs:
            dim = attrs['dim']
            if isinstance(dim, (list, tuple)):
    ...
    def eval(self, input_tensors):
        ...
        dim = self.attrs.get('dim', None)
        if dim is not None:
            # Forge Unsqueeze only supports single dim, so if we have multiple dims,
            # we need to handle it (but Forge API only supports one)
            if isinstance(dim, (list, tuple)):
                # Unsqueeze multiple dims in sorted order (ascending) to avoid index shifting
                result = x
                for d in sorted(dim):
                    result = torch.unsqueeze(result, dim=d)
                return {self.outputs[0]: result}
            else:
                return {self.outputs[0]: torch.unsqueeze(x, dim=dim)}
        else:
            # Default: unsqueeze at dim 0
            return {self.outputs[0]: torch.unsqueeze(x, dim=0)}
