# ONNX Sub Complete Opset Version Summary

Based on the [official ONNX Sub documentation](https://onnx.ai/onnx/operators/onnx__Sub.html), this document provides a comprehensive summary of all opset versions.

## Overview

The **Sub** operator performs element-wise binary subtraction between two tensors. It is one of the fundamental arithmetic operations in ONNX and supports broadcasting to handle tensors of different shapes.
...
**Example**: 
- Input A: `[10, 20, 30]`, Input B: `[1, 2, 3]` → Output: `[9, 18, 27]`
- Input A: `[[10, 20], [30, 40]]`, Input B: `[1, 2]` → Output: `[[9, 18], [29, 38]]` (broadcasting)
- Input A: `[[10, 20], [30, 40]]`, Input B: `5` → Output: `[[5, 15], [25, 35]]` (scalar broadcasting)
...
The ONNX Sub operator is similar to NumPy's subtraction with broadcasting:

```python
import numpy as np

# ONNX: Sub with same shapes
# NumPy equivalent:
a = np.array([10, 20, 30])
b = np.array([1, 2, 3])
result = a - b  # [9, 18, 27]

# ONNX: Sub with broadcasting (v7+)
# NumPy equivalent:
a = np.array([[10, 20], [30, 40]])
b = np.array([1, 2])
result = a - b  # [[9, 18], [29, 38]]

# ONNX: Sub with scalar (v7+)
# NumPy equivalent:
a = np.array([[10, 20], [30, 40]])
b = 5
result = a - b  # [[5, 15], [25, 35]]
```
...
**Note**: PyTorch subtraction always supports broadcasting (similar to ONNX v7+), and PyTorch may perform type promotion in some cases.
...
| Aspect | Sub | Add | Mul |
|--------|-----|-----|-----|
| **Operation** | Subtraction (`A - B`) | Addition (`A + B`) | Multiplication (`A * B`) |
...
**Note**: Sub, Add, and Mul operators have very similar evolution patterns across versions, with the main difference being the mathematical operation performed.

---

## References

- [ONNX Sub Operator Documentation](https://onnx.ai/onnx/operators/onnx__Sub.html)
