# ONNX Mul Complete Opset Version Summary

Based on the [official ONNX Mul documentation](https://onnx.ai/onnx/operators/onnx__Mul.html), this document provides a comprehensive summary of all opset versions.

## Overview

The **Mul** operator performs element-wise binary multiplication between two tensors. It is one of the fundamental arithmetic operations in ONNX and supports broadcasting to handle tensors of different shapes.
...
**Example**: 
- Input A: `[2, 3, 4]`, Input B: `[5, 6, 7]` → Output: `[10, 18, 28]`
- Input A: `[[2, 4], [6, 8]]`, Input B: `[2, 3]` → Output: `[[4, 12], [12, 24]]` (broadcasting)
- Input A: `[[2, 4], [6, 8]]`, Input B: `2` → Output: `[[4, 8], [12, 16]]` (scalar broadcasting)
...
The ONNX Mul operator is similar to NumPy's multiplication with broadcasting:

```python
import numpy as np

# ONNX: Mul with same shapes
# NumPy equivalent:
a = np.array([2, 3, 4])
b = np.array([5, 6, 7])
result = a * b  # [10, 18, 28]

# ONNX: Mul with broadcasting (v7+)
# NumPy equivalent:
a = np.array([[2, 4], [6, 8]])
b = np.array([2, 3])
result = a * b  # [[4, 12], [12, 24]]

# ONNX: Mul with scalar (v7+)
# NumPy equivalent:
a = np.array([[2, 4], [6, 8]])
b = 2
result = a * b  # [[4, 8], [12, 16]]
```
...
**Note**: PyTorch multiplication always supports broadcasting (similar to ONNX v7+), and PyTorch may perform type promotion in some cases.
...
| Aspect | Mul | Add | Sub |
|--------|-----|-----|-----|
| **Operation** | Multiplication (`A * B`) | Addition (`A + B`) | Subtraction (`A - B`) |
...
**Note**: Mul, Add, and Sub operators have very similar evolution patterns across versions, with the main difference being the mathematical operation performed.

---

## References

- [ONNX Mul Operator Documentation](https://onnx.ai/onnx/operators/onnx__Mul.html)
