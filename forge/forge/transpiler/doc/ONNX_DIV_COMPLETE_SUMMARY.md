# ONNX Div Complete Opset Version Summary

Based on the [official ONNX Div documentation](https://onnx.ai/onnx/operators/onnx__Div.html), this document provides a comprehensive summary of all opset versions.

## Overview

The **Div** operator performs element-wise binary division between two tensors. It is one of the fundamental arithmetic operations in ONNX and supports broadcasting to handle tensors of different shapes.
...
- **Division by zero**: Behavior is implementation-defined (typically results in inf or NaN for floating point types)

**Example**: 
- Input A: `[10, 20, 30]`, Input B: `[2, 4, 5]` → Output: `[5.0, 5.0, 6.0]`
- Input A: `[[10, 20], [30, 40]]`, Input B: `[2, 4]` → Output: `[[5.0, 5.0], [15.0, 10.0]]` (broadcasting)
- Input A: `[[10, 20], [30, 40]]`, Input B: `2` → Output: `[[5.0, 10.0], [15.0, 20.0]]` (scalar broadcasting)
...
- **Inputs**: 
  - `A` (T): First operand (dividend), should share the type with the second operand
  - `B` (T): Second operand (divisor). With broadcasting can be of smaller size than A. If broadcasting is disabled it should be of the same size
- **Outputs**:
...
**Note**: For integer division, the result type may be float even if inputs are integers (implementation-dependent).

---

### **Div v7** (since version 7)
...
- **Inputs**: 
  - `A` (T): First operand (dividend)
  - `B` (T): Second operand (divisor)
- **Outputs**:
...
2. **Type Requirements**:
   ...
   - **Note**: For integer division, result may be float (implementation-dependent)

3. **Input Requirements**:
   - Always exactly 2 inputs: `A` (dividend) and `B` (divisor)
...
5. **Division by Zero**:
   ...
   - Typically results in `inf` or `NaN` for floating point types
   - Integer division by zero may cause undefined behavior
...
2. **Type Support**:
   ...
   - Consider integer division behavior (may produce float results)
...
4. **Input Validation**:
   ...
   - Consider checking for division by zero (optional, implementation-dependent)
...
7. **Division by Zero Handling**:
   - Behavior is implementation-defined
   - Consider handling edge cases (division by zero, overflow, etc.)
   - For floating point: typically results in `inf` or `NaN`
   - For integers: behavior may be undefined

---

## Comparison with NumPy

The ONNX Div operator is similar to NumPy's division with broadcasting:

```python
import numpy as np

# ONNX: Div with same shapes
# NumPy equivalent:
a = np.array([10, 20, 30])
b = np.array([2, 4, 5])
result = a / b  # [5.0, 5.0, 6.0]

# ONNX: Div with broadcasting (v7+)
# NumPy equivalent:
a = np.array([[10, 20], [30, 40]])
b = np.array([2, 4])
result = a / b  # [[5.0, 5.0], [15.0, 10.0]]

# ONNX: Div with scalar (v7+)
# NumPy equivalent:
a = np.array([[10, 20], [30, 40]])
b = 2
result = a / b  # [[5.0, 10.0], [15.0, 20.0]]
```
...
- Division by zero behavior may differ between implementations
...
**Note**: PyTorch division always supports broadcasting (similar to ONNX v7+), and PyTorch may perform type promotion in some cases. PyTorch uses true division (float result) by default.

---

## Differences from Other Operators

| Aspect | Div | Add | Sub | Mul |
|--------|-----|-----|-----|-----|
| **Operation** | Division (`A / B`) | Addition (`A + B`) | Subtraction (`A - B`) | Multiplication (`A * B`) |
| **Inputs** | 2 inputs | 2 inputs | 2 inputs | 2 inputs |
| **Broadcasting** | v1-v6: Limited, v7+: Multidirectional | v1-v6: Limited, v7+: Multidirectional | v1-v6: Limited, v7+: Multidirectional | v1-v6: Limited, v7+: Multidirectional |
| **Attributes** | v1-v6: `broadcast`, `axis`, v7+: None | v1-v6: `broadcast`, `axis`, v7+: None | v1-v6: `broadcast`, `axis`, v7+: None | v1-v6: `broadcast`, `axis`, v7+: None |
| **Type Support** | Float + Integers (v6+), Extended (v14) | Float + Integers (v6+), Extended (v14) | Float + Integers (v6+), Extended (v14) | Float + Integers (v6+), Extended (v14) |
| **Shape Inference** | v1: No, v6+: Yes | v1: No, v6+: Yes | v1: No, v6+: Yes | v1: No, v6+: Yes |
| **Special Cases** | Division by zero | None | None | None |

**Note**: Div, Add, Sub, and Mul operators have very similar evolution patterns across versions, with the main difference being the mathematical operation performed. Div has the additional consideration of division by zero handling.

---

## References

- [ONNX Div Operator Documentation](https://onnx.ai/onnx/operators/onnx__Div.html)
