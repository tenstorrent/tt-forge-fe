...
class NLPPipelineWrapper(torch.nn.Module):
    ...
    def __init__(
        ...
        if forward_fn is None:
            self.module = forge.PyTorchModule(name, model, redirect_forward=False)
            self.ttdevice = forge.TTDevice("tt0", module=self.module, fp32_fallback=fp32_fallback)
...
def pipeline(pipeline_type: str, *args, **kwargs):
    m = kwargs["model"]
    forward_fn = None
    forge_max_length = None
    if "forward_fn" in kwargs:
        forward_fn = kwargs.pop("forward_fn")

    if "forge_max_length" in kwargs:
        forge_max_length = kwargs.pop("forge_max_length")
