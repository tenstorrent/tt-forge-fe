...
class ForgeWriter(PythonWriter):
    ...
    def write_class_definition(
        self, params, constants, class_name=None, num_submodels=0, is_submodel=False, use_random_value=False
    ):
    ...
    def write_pytest_function(
        ...
        pytest_input_shapes_and_dtypes_list: List[List[Tuple[Any]]],
        markers: Optional[List[str]] = [],
        module_metadata: Optional[Dict[str, Any]] = {},
        pytest_metadata_list: Optional[List[Dict[str, Any]]] = [],
        use_ids_function: Optional[bool] = False,
    ):
        """
        ...
        Args:
            ...
            pytest_input_shapes_and_dtypes_list (List[List[Tuple[Any]]]): A list of input shapes and corresponding data types for each module. Each tuple contains the shape and dtype to be tested.
            markers (Optional[List[str]]): A list of pytest markers that will be added above the test function. Defaults to an empty list.
            module_metadata (Optional[Dict[str, Any]]): A dictionary containing metadata about the test function. Each key-value pair represents a metadata property name and its corresponding value, which will be recorded using the `record_property` pytest fixtures. Defaults to an empty dictionary.
            pytest_metadata_list (Optional[List[Dict[str, Any]]]): A list of dictionaries containing metadata for each pytest parameter.
            use_ids_function(Optional[bool]): If set, the forge module name and shapes and dtyes will used as id for the pytest parameter.
        """
        self.wl("")
        self.wl("")
        if use_ids_function:
            ...
            self.indent += 1
            self.wl("forge_module, shapes_dtypes, _ = param")
