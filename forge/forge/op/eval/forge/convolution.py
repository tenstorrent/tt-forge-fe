...
def decompose_conv2d_sparse_first(attr, dc, inputs):
    ...
    sparse_weight_padding_mm = ast.literal_eval(os.environ.get('FORGE_PAD_SPARSE_MM_WEIGHT_MM', "{}"))
    sparse_weight_padding_concat = ast.literal_eval(os.environ.get('FORGE_PAD_SPARSE_MM_WEIGHT_CONCAT', "{}"))
...
def decompose_conv3d_sparse_first(attr, dc, inputs):
    ...
    #depthwise_env_enabled = "FORGE_ENABLE_DEPTHWISE" in os.environ and os.environ["FORGE_ENABLE_DEPTHWISE"] == "1"
    # Disallow depthwise path when training, needs BW ops implementation
...
def decompose(type, attr, dc, inputs):
    if type == "conv2d":
        ...
        elif bool(int(os.environ.get("FORGE_CONV2D_SPARSE_SECOND", "0"))):
            # Sparse second
