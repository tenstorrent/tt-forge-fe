...
def decompose_post_optimize(type, attr, dc, inputs):
    if type == "concatenate":
        ...
        max_inputs = int(os.environ.get("FORGE_MAX_CONCAT_INPUTS", "8"))
        if len(inputs) > max_inputs:
            # TODO: use max_num_inputs 8 when tenstorrent/forge#316 is resolved
        ...
        if (
            int(os.environ.get("FORGE_CONCAT_SLICE_Y", "0")) != 0
            ...
            num_split_r = int(os.environ.get("FORGE_CONCAT_SLICE_Y", "0"))
        ...
        insert_slice = (padding_removal_needed and axis == -1) or bool(int(os.environ.get("FORGE_INSERT_SLICE_FOR_CONCAT", "0")))
        if insert_slice:
