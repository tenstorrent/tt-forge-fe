...
def decompose(type, attr, dc, inputs):
    if type == "resize2d":
        ...
        if upsample:
            result = dc.op_with_named_attrs(
                "upsample2d",
                [acts],
                {"scale_factor": scale_factor, "mode": resize_method, "channel_last": channel_last},
                (scale_factor, resize_method, channel_last),
            )
            dc.fuse(result)
        else:
            raise NotImplementedError("Downsampling of resize2d is not supported yet")
    ...
    if type == "upsample2d":
        resize_method = attr[1]
        scale_factor = attr[0]
        channel_last = attr[2]
        if not channel_last:
            inputs = dc.op(TransposeTM.create(dim0=-3, dim1=-2), [inputs[0]])
            inputs = dc.op(TransposeTM.create(dim0=-2, dim1=-1), [inputs])
            result = dc.op_with_named_attrs(
                "upsample2d",
                [inputs],
                {"scale_factor": scale_factor, "mode": resize_method, "channel_last": channel_last},
