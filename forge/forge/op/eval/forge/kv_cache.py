...
class UpdateCache(PyOp):
    ...
    def eval(self, tensors):
        ...
        assert S_in == 1, "Input tensor must have sequence length S=1 for UpdateCache op"
        ...
        # Generate output cache tensor. Clone to align with tt-metal implementation.
        new_cache = cache.clone()

        for b in range(B_in):
            idx = update_index[b].item()
            assert 0 <= idx < S_cache, f"Invalid update index {idx} at batch {b}"
            new_cache[b + batch_offset, :, idx : idx + 1, :] = input[b]
...
class FillCache(PyOp):
    ...
    def eval(self, tensors):
        ...
        for b in range(B_in):
            ...
            new_cache[b + batch_offset, :, 0:S_in, :] = input[b]
