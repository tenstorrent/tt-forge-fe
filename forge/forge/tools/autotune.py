...
def main(command, num_loops, cache_path, perf_path):
    ...
    env_vars["FORGE_COMPILER_CACHE"] = cache_path
    ...
    for i in range(num_loops):
        ...
        subprocess.run(f"forge/forge/tools/perf_analysis.py --cache {cache_path}", shell=True, env=env_vars)
        subprocess.run(command + " -o " + perf_path, shell=True, env=env_vars)
    ...
    logger.info(
        f"Autotune completed {num_loops} loops, best result = {best_result} samples/sec from iteration {best_result_index} (original = {original_result})"
    )
    ...
    if best_result_index == 0:
        ...
        repro_command = f"FORGE_COMPILER_CACHE={cache_path} " + command + " -o " + perf_path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--cache", type=str, required=True, help="Path for the cache file (eg: .cache/perf_cache.ttc)"
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="perf.json",
        help="Output json file to write results to, if it exists it will be overwritten",
    )
    ...
    if "-o " in args.command or "--output " in args.command:
        logger.error(
            f"Error: output file cannot be specified in <command> directly, move it outside of '{args.command}'"
        )
        exit(1)
