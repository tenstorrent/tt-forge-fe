...
def uniquify_and_merge_netlists(
    model_paths, inter_model_connections, consumer_to_producers_map, overlay_size_per_model
):
    ...
    model_input_counts = {}
    fused_op_counter = 0
    merged_model = {"devices": [], "queues": {}, "graphs": {}, "programs": []}
            ...
            for queue in netlist_data["queues"].keys():
                ...
                if updated_queue_name in sub_graph_nodes_visited:
                    ...
                    if updated_queue_name in producer_outputs:
                        num_tiles_y = (
                            netlist_data["queues"][queue]["ublock"][0]
                            * netlist_data["queues"][queue]["mblock"][0]
                            * netlist_data["queues"][queue]["grid_size"][0]
                        )
                        num_tiles_x = (
                            netlist_data["queues"][queue]["ublock"][1]
                            * netlist_data["queues"][queue]["mblock"][1]
                            * netlist_data["queues"][queue]["grid_size"][1]
                        )
                        producer_queue_shapes[updated_queue_name] = [
                            netlist_data["queues"][queue]["entries"],
                            netlist_data["queues"][queue]["t"],
                            num_tiles_y,
                            num_tiles_x,
                        ]
                ...
                if updated_queue_name in inter_model_connections:
                    ...
                    assert consumer_queue_shape == producer_queue_shapes[inter_model_connections[updated_queue_name]], (
                        "Consumer " + queue + " shape is incompatible with the producer."
                    )
                    assert (
                        netlist_data["queues"][queue]["df"]
                        == producer_data_format[inter_model_connections[updated_queue_name]]
                    ), ("Consumer " + queue + " data format is incompatible with the producer.")
                    assert tile_dim == producer_tile_dims[inter_model_connections[updated_queue_name]], (
                        "Consumer " + queue + " tile dimensions are incompatible with the producer."
                    )
                    ...
                    updated_input_name = (
                        "HOST" if (input_name.lower() == "host") else "model_" + str(i) + "_" + input_name
                    )
                    ...
                    if op == "target_device" or op == "input_count":
                        if op == "input_count":
                            ...
                            if "model_" + str(i) in consumer_to_producers_map:
                                ...
                                for producer in consumer_to_producers_map["model_" + str(i)]:
                                    assert (
                                        netlist_data["graphs"][graph][op] == model_input_counts[producer]
                                    ), "The microbatch sizes across producers and consumers are not consistent."
                        ...
                        for input_idx in range(len(netlist_data["graphs"][graph][op]["inputs"])):
                            if netlist_data["graphs"][graph][op]["inputs"][input_idx] in unique_queue_names:
                                netlist_data["graphs"][graph][op]["inputs"][input_idx] = unique_queue_names[
                                    netlist_data["graphs"][graph][op]["inputs"][input_idx]
                                ]
                            elif netlist_data["graphs"][graph][op]["inputs"][input_idx] in unique_op_names:
                                netlist_data["graphs"][graph][op]["inputs"][input_idx] = unique_op_names[
                                    netlist_data["graphs"][graph][op]["inputs"][input_idx]
                                ]
                            else:
                        ...
                        if netlist_data["graphs"][graph][op]["type"] == "fused_op":
                            ...
                            netlist_data["graphs"][graph][op]["attributes"]["fused_op_id"] = fused_op_idx_updates[
                                local_id
                            ]
                ...
                for instrn_idx, instrn in enumerate(netlist_data["programs"][prog_idx][program_name]):
                    if type(instrn) == dict:
                        ...
                        if instrn_code == "execute":
                            netlist_data["programs"][prog_idx][program_name][instrn_idx][instrn_code][
                                "graph_name"
                            ] = unique_graph_names[
                                netlist_data["programs"][prog_idx][program_name][instrn_idx][instrn_code]["graph_name"]
                            ]
                            queue_settings = netlist_data["programs"][prog_idx][program_name][instrn_idx][instrn_code][
                                "queue_settings"
                            ]
                            netlist_data["programs"][prog_idx][program_name][instrn_idx][instrn_code][
                                "queue_settings"
                            ...
                            for queue in queue_settings:
                                ...
                                netlist_data["programs"][prog_idx][program_name][instrn_idx][instrn_code][
                                    "queue_settings"
                                ][updated_queue] = queue_settings[queue]
                        if instrn_code == "allocate_queue" or instrn_code == "deallocate_queue":
                            for queue_idx in range(
                                len(netlist_data["programs"][prog_idx][program_name][instrn_idx][instrn_code])
                            ):
                                queue_name = netlist_data["programs"][prog_idx][program_name][instrn_idx][instrn_code][
                                    queue_idx
                                ]
                                netlist_data["programs"][prog_idx][program_name][instrn_idx][instrn_code][
                                    queue_idx
                                ] = unique_queue_names[queue_name]

                merged_model["programs"].append(
                    {updated_program_name: netlist_data["programs"][prog_idx][program_name]}
                )
                ...
                for group in netlist_data["fused_ops"].keys():
                    ...
                    for sched_idx in range(len(schedules)):
                        ...
                        for op_idx, op in enumerate(schedules[sched_idx]):
                            ...
                            netlist_data["fused_ops"][group]["schedules"][sched_idx].append(
                                {updated_op_name: schedules[sched_idx][op_idx][list(op.keys())[0]]}
                            )
    ...
    for queue in sub_graph_nodes_visited:
        assert sub_graph_nodes_visited[queue], (
            "Queue " + queue + " was specified in the dependency list but was not found in any netlist."
        )
    return merged_model
...
def update_buffer_ranges(merged_model, queue, buf_info, buf_group_ranges):
    ...
    for group in buf_group_ranges[buf_info[0]]:
        if buf_info[1] >= group[0] and buf_info[1] <= group[1] + FRAGMENTATION_SIZE:
            ...
            buf_info[1] >= group[0] - get_queue_size(merged_model, queue) - FRAGMENTATION_SIZE
            and buf_info[1] + get_queue_size(merged_model, queue) <= group[1]
        ):
...
def get_queue_size(netlist, queue):
    ...
    return backend_api.get_io_size_in_bytes(
        format, is_untilized, ublock_ct, ublock_rt, mblock_m, mblock_n, t, entries, tile_height, tile_width
    )


def reallocate_queues(
    arch,
    merged_model,
    dynamic_queues,
    start_offset_to_queue_buf_per_model,
    soc_descriptor,
    switch_chans_if_capacity_hit,
    overlap_dynamic_queues,
):
    dev_cfg = backend_api.DeviceConfig(arch, soc_descriptor, "", "", "", False, [])
            ...
            for chan in start_offset_to_queue_buf_per_model[model]:
                for offset in start_offset_to_queue_buf_per_model[model][chan]:
                    ...
                    for q_buf_pair in start_offset_to_queue_buf_per_model[model][chan][offset]:
                        ...
                        dram_usage_across_groups[chan] = max(
                            dram_usage_across_groups[chan], shifted_addr + get_queue_size(merged_model, queue_name)
                        )
        ...
        if merged_model["queues"][queue]["loc"].lower() == "dram":
            for alloc in merged_model["queues"][queue]["dram"]:
                if static_queue_dram_space[alloc[0]] + queue_size > max_dram_space:
                    if switch_chans_if_capacity_hit:
                        logger.info(
                            "DRAM Channel {} capacity hit. Bytes Used: {}. Reallocating queue to a different channel",
                            alloc[0],
                            static_queue_dram_space[alloc[0]],
                        )
                ...
                assert static_queue_dram_space[alloc[0]] <= max_dram_space, (
                    "DRAM space exceeded for DRAM channel "
                    + str(alloc[0])
                    + " when trying to allocate memory for queue "
                    + queue
                    + " Bytes used: "
                    + str(static_queue_dram_space[alloc[0]])
                )
                static_queue_dram_space[alloc[0]] = backend_api.get_next_aligned_address(
                    static_queue_dram_space[alloc[0]]
                )
        else:
            for (alloc_idx, alloc) in enumerate(merged_model["queues"][queue]["host"]):
                if type(alloc) == list:
                    ...
                    if memory_consumed_per_host_channel[alloc[0]] + queue_size > dev_cfg.get_host_memory_channel_size(
                        alloc[0]
                    ):
                        if switch_chans_if_capacity_hit:
                            logger.info(
                                "Host Channel {} capacity hit. Bytes Used: {}. Reallocating queue to a different channel",
                                alloc[0],
                                memory_consumed_per_host_channel[alloc[0]],
                            )
                            for i in memory_consumed_per_host_channel:
                                if memory_consumed_per_host_channel[
                                    i
                                ] + queue_size <= dev_cfg.get_host_memory_channel_size(i):
                                    alloc[0] = i
                    alloc[1] = memory_consumed_per_host_channel[alloc[0]]
                    memory_consumed_per_host_channel[alloc[0]] += queue_size
                    assert memory_consumed_per_host_channel[alloc[0]] <= dev_cfg.get_host_memory_channel_size(
                        alloc[0]
                    ), (
                        "Host memory space exceeded for channel "
                        + str(alloc[0])
                        + " when trying to allocate memory for queue "
                        + queue
                        + " Bytes used: "
                        + str(memory_consumed_per_host_channel[alloc[0]])
                    )
                    memory_consumed_per_host_channel[alloc[0]] = backend_api.get_next_aligned_address(
                        memory_consumed_per_host_channel[alloc[0]]
                    )
                else:
                    ...
                    assert memory_consumed_per_host_channel[0] <= dev_cfg.get_host_memory_channel_size(0), (
                        "Host memory space exceeded for channel 0 when trying to allocate memory for queue "
                        + queue
                        + " Bytes used: "
                        + str(memory_consumed_per_host_channel[0])
                    )
                    memory_consumed_per_host_channel[0] = backend_api.get_next_aligned_address(
                        memory_consumed_per_host_channel[0]
                    )
    ...
    for (i, tti_path) in enumerate(unzipped_tti_paths):
        ...
        for tensor_bin in os.listdir(tensor_path):
            if os.path.isfile(os.path.join(tensor_path, tensor_bin)):
                shutil.copy(
                    os.path.join(tensor_path, tensor_bin),
...
def merge_device_metadata(unzipped_tti_paths, merged_tti_path, inter_model_connections):
    ...
    for (i, tti_path) in enumerate(unzipped_tti_paths):
        ...
        for name in device_md["compiled_graph_state"]["post_const_eval_parameters"]:
            merged_md["compiled_graph_state"]["post_const_eval_parameters"]["model_" + str(i) + "_" + name] = device_md[
                "compiled_graph_state"
            ]["post_const_eval_parameters"][name]
            tensor_bin = merged_md["compiled_graph_state"]["post_const_eval_parameters"][
                "model_" + str(i) + "_" + name
            ]["bin"].split("/")[1]
            merged_md["compiled_graph_state"]["post_const_eval_parameters"]["model_" + str(i) + "_" + name]["bin"] = (
                "tensors/" + "model_" + str(i) + "_" + tensor_bin
            )
        for name in device_md["compiled_graph_state"]["post_const_eval_constants"]:
            merged_md["compiled_graph_state"]["post_const_eval_constants"]["model_" + str(i) + "_" + name] = device_md[
                "compiled_graph_state"
            ]["post_const_eval_constants"][name]
            tensor_file = merged_md["compiled_graph_state"]["post_const_eval_constants"][
                "model_" + str(i) + "_" + name
            ]["bin"].split("/")[1]
            merged_md["compiled_graph_state"]["post_const_eval_constants"]["model_" + str(i) + "_" + name]["bin"] = (
                "tensors/" + "model_" + str(i) + "_" + tensor_file
            )
        ...
        netlist_names.append(
            os.path.join(
                tti_path, "unzipped_tti/", os.path.basename(device_md["compiled_graph_state"]["netlist_filename"])
            )
        )
        ...
        for cfg_entry in config:
            if "TT_BACKEND_" in cfg_entry:
                ...
                if cfg_entry == "TT_BACKEND_OVERLAY_MAX_EXTRA_BLOB_SIZE":
                    ...
                    assert harv_mask_bin.count("1") == existing_harv_mask_bin.count(
                        "1"
                    ), "SOC Grid size mismatch across models"
                else:
                    assert config[cfg_entry] == backend_config[cfg_entry], (
                        "Config var: " + str(cfg_entry) + " mismatches across models."
                    )
...
def merge_netlists(
    ...
    merged_netlist = uniquify_and_merge_netlists(
        netlist_paths, inter_model_connections, consumer_to_producers_map, overlay_blob_size_per_model
    )
    dynamic_queues, start_offset_to_queue_buf_per_model = get_dynamic_queue_info(merged_netlist)
    merged_model = reallocate_queues(
        arch,
        merged_netlist,
        dynamic_queues,
        start_offset_to_queue_buf_per_model,
...
def compile_backend_binaries(arch, merged_tti_path, netlist_path):
    ...
    bcfg = backend_api.BackendConfig(
        backend_api.BackendType.Silicon,
    ...
    for model in dep_list:
        for input in dep_list[model]["inputs"]:
            if type(dep_list[model]["inputs"][input]) == str:
                assert dep_list[model]["inputs"][input].lower() == "host", (
                    "If input for model "
                    + str(model)
                    + " is not host, the feeder must be specified in format [feeder_model_name, feeder_queue_name]."
                )
            else:
                assert type(dep_list[model]["inputs"][input]) == list, (
                    "The feeder for model "
                    + str(model)
                    + " must be specified in format [feeder_model_name, feeder_queue_name]."
                )
                assert dep_list[model]["inputs"][input][0] in models, (
                    "Feeder model "
                    + str(dep_list[model]["inputs"][input][0])
                    + " to consumer model "
                    + model
                    + " is not specified."
                )
        ...
        for model in models:
            if not model in ordered_models:
                if not model in dep_list:
                    logger.warning(
                        "Could not find model {} in dependency list. Assuming that this model is fed by host.", model
                    )
...
def merge_models(
    ...
    models,
    arch="wormhole_b0",
    merged_model_location="",
    switch_chans_if_capacity_hit=True,
    ...
    try:
        ...
        consumer_to_producers_map = {}
        dependency_file = (
            ""  # Explicitly set dependency file to empty, since we don't have compiler support for pipelined models
        )
        if dependency_file:
            ordered_models, inter_model_connections, consumer_to_producers_map = parse_model_deps(
                models, dependency_file
            )
        ...
        merged_netlist_path = merge_netlists(
            arch,
            netlist_names,
            merged_binary_dir,
            unzipped_tti_paths,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-mbl",
        type=str,
        help="Location of model binaries (tti files) to merge.",
        required=True,
    )
    ...
    parser.add_argument(
        ...
        "-mml",
        type=str,
        help="Filesystem location where the merged model binaries are stored.",
    )
    parser.add_argument(
        "--skip_channel_reallocation",
        "-scr",
        type=bool,
        help="Skip memory usage optimization that reallocates buffers on different DRAM channels, once channel capacity is hit.",
        default=False,
    )
    parser.add_argument(
        "--dynamic_queue_overlap_off",
        "-dqo",
        type=bool,
        help="Turn off memory usage optimization that overlaps dynamic queues",
        default=False,
    )
    args = parser.parse_args()

    merge_models(
        args.model_binaries_location,
        args.models,
        args.arch.lower(),
