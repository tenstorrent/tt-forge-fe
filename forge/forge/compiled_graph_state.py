...
class CompiledGraphState:
    ...
    def from_compiled_graph(
        ...
        for node in ordered_output_nodes:
            output_node = node.as_output()
            if output_node.is_aliased:
                aliased_outputs[output_node.name] = output_node.alias
...
class CompiledModel:
    ...
    def __init__(
        ...
        if self.bwd_compiled_graph_state is not None:
            self.gradient_inputs = [torch.tensor(0)] * len(self.bwd_compiled_graph_state.ordered_input_gradient_names)
        self.outputs = {}
