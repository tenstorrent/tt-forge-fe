...
def pytest_addoption(parser):
    # model
    parser.addoption(
        "--tmh_model", action="store", default="model_1", help="The name of the file in which the model is located."
    )
    # training
    parser.addoption("--tmh_train", action="store", default=True, help="Training or Inference.")
    # recompute
    parser.addoption("--tmh_recompute", action="store", default=True, help="Recompute or not.")
    # shape
    parser.addoption("--tmh_shape", action="store", default=[1, 16, 32, 64], help="Shape of the tensor.")
    # slice
    parser.addoption("--tmh_slice", action="store", default=4, help="Number of slices to create.")
