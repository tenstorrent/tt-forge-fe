...
shape_factors = [
    np.random.randint(
        SHAPE_DIM_MIN,
        SHAPE_DIM_MAX,
        size=[(TENSOR_SIZE if TENSOR_FIXED else np.random.randint(SHAPE_SIZE_MIN, SHAPE_SIZE_MAX)) * 2],
    )
...
@pytest.mark.parametrize(
    "old_shape, new_shape",
    zip(old_shape, new_shape),
    ids=[
        "old_shape=" + "x".join([str(item) for item in old]) + "-new_shape=" + "x".join([str(item) for item in new])
...
def test_reshape(op_test_kind, model, old_shape, new_shape):
