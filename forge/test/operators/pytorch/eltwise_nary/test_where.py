...
class ModelDirect(torch.nn.Module):
    ...
    def forward(self, x, y, z):
        treshold = 0.5
        condition = (x > treshold)
        return torch.where(condition, y, z)
    

class ModelConstEvalPass(torch.nn.Module):
    def __init__(self, shape, dtype, value_range):
    ...
    def forward(self, x, y, z):
        treshold = 0.5
        condition = (self.c1 > treshold)
        where_result = torch.where(condition, self.c2, self.c3)
