...
class TestIdsDataLoader:
    ...
    def _for_failing_reason(
        cls, operators: List[str], failing_reason: FailingReasonsDefs
    ) -> Generator[str, None, None]:
        for operator in operators:
            yield from cls.from_file(f"xfail/{operator}_{failing_reason.name.lower()}.txt")

    @classmethod
    def for_failing_reason(cls, operators: List[str], failing_reason: FailingReasonsDefs) -> List[str]:
        return list(cls._for_failing_reason(operators, failing_reason))

    @classmethod
    def build_failing_rule(cls, operators: List[str], failing_reason: FailingReasonsDefs) -> TestCollection:
        ...
        return TestCollection(
            ...
            failing_reason=failing_reason.value.description,
        )
    ...
    def _get_failing_reasons(cls, operator: str) -> Generator[FailingReasonsDefs, None, None]:
        for failing_reason in FailingReasonsDefs:
