    ...
    def verify(
        ...
        VerifyUtils.verify(
            ...
            verify_config=VerifyConfig(
                value_checker = AllCloseValueChecker()
            )
        )
...
TestParamsData.test_plan = TestPlan(
    ...
    failing_rules=[
        ...
        TestCollection(
            input_shapes=[(1, 1)],
            input_sources=[InputSource.FROM_ANOTHER_OP, InputSource.CONST_EVAL_PASS],
            failing_reason=FailingReasons.DATA_MISMATCH,
        ),
        TestCollection(
            input_shapes=[(1, 1920)],
            failing_reason=FailingReasons.FATAL_ERROR,
        ),
        TestCollection(
            input_shapes=[(11, 32, 32, 64)],
            failing_reason=FailingReasons.FATAL_ERROR,  # videti tacno reason
        ),
        TestCollection(
            input_shapes=[(12, 64, 160, 96)],
            input_sources=[InputSource.FROM_ANOTHER_OP, InputSource.FROM_DRAM_QUEUE],
            failing_reason=FailingReasons.FATAL_ERROR,  # videti tacno reason
        ),
    ],
)
