...
class ModelConstEvalPass(torch.nn.Module):
    ...
    def __init__(self, operator, opname, shape, kwargs):
        ...
        self.constant = torch.randint(0, self.kwargs["in_features"] - 1, self.shape, dtype=torch.int32)
        self.l1 = self.operator(**self.kwargs)
...
class TestParamsData:
    ...
    out_features_list = [40]  # , 50, 60,]

    @classmethod
    def generate_kwargs(cls, test_vector: TestVector):
        kwarg_list = []
        for out_features in cls.out_features_list:
            kwarg_list.append(
                {
...
class TestCollectionData:
    ...
    all = TestCollection(
        ...
        input_shapes=[input_shape for input_shape in TestCollectionCommon.all.input_shapes if len(input_shape) == 2],
