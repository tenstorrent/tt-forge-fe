...
TestParamsData.test_plan = TestPlan(
    ...
    failing_rules = [
        ...
        TestCollection(
            criteria=lambda test_vector: test_vector.get_id() in TestPlanUtils.load_test_ids_from_file(f"{os.path.dirname(__file__)}/test_reduce_ids_runtime_error.txt"),          
            failing_reason=FailingReasons.COMPILATION_FAILED,
        ),
        # Error info: PCC check failed
        TestCollection(
            criteria=lambda test_vector: test_vector.get_id() in TestPlanUtils.load_test_ids_from_file(f"{os.path.dirname(__file__)}/test_reduce_ids_pcc_error.txt"),          
            failing_reason=FailingReasons.DATA_MISMATCH,
        ),
        # Error info: 'TransposeTM' object has no attribute 'z_dim_slice' (via OpType cpp underlying class)
        TestCollection(
            criteria=lambda test_vector: test_vector.get_id() in TestPlanUtils.load_test_ids_from_file(f"{os.path.dirname(__file__)}/test_reduce_ids_attribute_error.txt"),          
            failing_reason=FailingReasons.INFERENCE_FAILED,
        ),
        # RuntimeError: TT_ASSERT @ /tt-forge-fe/forge/csrc/passes/lower_to_mlir.cpp:495: false
        TestCollection(
            criteria=lambda test_vector: test_vector.get_id() in TestPlanUtils.load_test_ids_from_file(f"{os.path.dirname(__file__)}/test_reduce_ids_df_mf_assert_error.txt"),          
            failing_reason=FailingReasons.COMPILATION_FAILED,
        ),
        # Error info: Unable to reshape a tensor in TILE_LAYOUT to non-tile height and width! Please convert the tensor to ROW_MAJOR_LAYOUT first.
        TestCollection(
            criteria=lambda test_vector: test_vector.get_id() in TestPlanUtils.load_test_ids_from_file(f"{os.path.dirname(__file__)}/test_reduce_ids_df_mf_tt_throw_error.txt"),          
            failing_reason=FailingReasons.COMPILATION_FAILED,
        ),
        # Error info: Page size must be divisible by sizeof(uint32_t) because buffers hold uint32_t values
        TestCollection(
            criteria=lambda test_vector: test_vector.get_id() in TestPlanUtils.load_test_ids_from_file(f"{os.path.dirname(__file__)}/test_reduce_ids_df_mf_tt_fatal_error.txt"),          
            failing_reason=FailingReasons.COMPILATION_FAILED,
        ),
        # RuntimeError: Unsupported data type
        TestCollection(
            criteria=lambda test_vector: test_vector.get_id() in TestPlanUtils.load_test_ids_from_file(f"{os.path.dirname(__file__)}/test_reduce_ids_df_mf_unsupported_dt_error.txt"),          
            failing_reason=FailingReasons.COMPILATION_FAILED,
        ),
        # RuntimeError: mean(): could not infer output dtype. Input dtype must be either a floating point or complex dtype. Got: Char
        TestCollection(
            criteria=lambda test_vector: test_vector.get_id() in TestPlanUtils.load_test_ids_from_file(f"{os.path.dirname(__file__)}/test_reduce_ids_df_mf_unsupported_bool_dt_error.txt"),          
            failing_reason=FailingReasons.COMPILATION_FAILED,
        ),
