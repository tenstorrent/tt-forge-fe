...
def pytest_addoption(parser):
    # model
    parser.addoption(
        "--bin_model", action="store", default="model_1", help="The name of the file in which the model is located."
    )
    # training
    parser.addoption("--bin_train", action="store", default=True, help="Training or Inference.")
    # recompute
    parser.addoption("--bin_recompute", action="store", default=True, help="Recompute or not.")
    # shape
    parser.addoption("--bin_shape", action="store", default=[1, 16, 32, 64], help="Shape of the tensor.")
    # operation
    parser.addoption(
        "--bin_op", action="store", default="Add", help="Binary element-wise operation which we want to perform."
    )
