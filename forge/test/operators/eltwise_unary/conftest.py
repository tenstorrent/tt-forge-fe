...
def pytest_addoption(parser):
    # model
    parser.addoption(
        "--un_model", action="store", default="model_4", help="The name of the file in which the model is located."
    )
    # training
    parser.addoption("--un_train", action="store", default=True, help="Training or Inference.")
    # recompute
    parser.addoption("--un_recompute", action="store", default=True, help="Recompute or not.")
    # shape
    parser.addoption("--un_shape", action="store", default=[1, 32, 64], help="Shape of the tensor.")
    # operation
    parser.addoption(
        "--un_op", action="store", default="Sqrt", help="Unary element-wise operation which we want to perform."
    )
    # kwargs
    parser.addoption(
        "--un_kwargs_json",
