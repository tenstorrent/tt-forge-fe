...
class TestCollection:
    ...
    kwargs: Optional[Union[List[OperatorParameterTypes.Kwargs], Callable[['TestVector'], List[OperatorParameterTypes.Kwargs]]]] = None
    pcc: Optional[float] = None
...
class TestPlanUtils:
    ...
    @classmethod
    def _match_failing_reason(cls, rule_failing_reason: str, vector_failing_result: TestResultFailing, check_failing_reason: bool = False):
        return not check_failing_reason or (rule_failing_reason is None or (vector_failing_result is not None and rule_failing_reason == vector_failing_result.failing_reason))

    @classmethod
    def _match_skip_reason(cls, rule_skip_reason: str, vector_failing_result: TestResultFailing, check_failing_reason: bool = False):
        return not check_failing_reason or (rule_skip_reason is None or (vector_failing_result is not None and rule_skip_reason == vector_failing_result.skip_reason))

    @classmethod
    def test_vector_in_collection(cls, test_vector: TestVector, test_collection: TestCollection, check_failing_reason: bool = False) -> bool:
        return \
            ...
            and cls._match_failing_reason(test_collection.failing_reason, test_vector.failing_result, check_failing_reason) \
    ...
    def build_test_plan_from_id_list(cls, test_ids: List[str], test_plan_failing: Optional[TestPlan] = None) -> TestPlan:
        test_plan = TestPlan(
            collections = [cls.test_vector_to_test_collection(cls.test_id_to_test_vector(test_id)) for test_id in test_ids],
