...
class TestQuery:
    ...
    def _filter_allowed(self, filter: Callable[[TestVector], bool] = None) -> Generator[TestVector, None, None]:
        for test_vector in self.test_vectors:
            if filter(test_vector):
                yield test_vector

    def _filter_skiped(self, filter: Callable[[TestVector], bool] = None) -> Generator[TestVector, None, None]:
        for test_vector in self.test_vectors:
            if not filter(test_vector):
                yield test_vector
    ...
    def filter(self, filter: Optional[Callable[[TestVector], bool]] = None) -> "TestQuery":
        """Filter test vectors based on the filter function"""
        if filter is None:
            return self
        return TestQuery(self._filter_allowed(filter))

    def skip(self, filter: Optional[Callable[[TestVector], bool]] = None) -> "TestQuery":
        """Skip test vectors based on the filter function"""
        if filter is None:
            return self
        return TestQuery(self._filter_skiped(filter))
