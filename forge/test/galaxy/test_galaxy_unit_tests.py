...
def get_two_chip_op_tests(subset, c0, c1):
    ...
    def two_chip_simple_unary_to_unary(act):
        forge.set_chip_break("unary1_A")
        forge.override_dram_queue_placement("inputs", chip_id=c0, channel=0)

        unary0 = forge.op.Exp("unary0_A", act)
        unary1 = forge.op.Gelu("unary1_A", unary0)
        return unary1

    def two_chip_eth_gather(act):
        forge.set_chip_break("unary1_B")
        forge.override_dram_queue_placement("inputs", chip_id=c0, channel=0)
        forge.override_op_size("unary0_B", (2, 2))
        forge.override_op_size("unary1_B", (1, 1))

        unary0 = forge.op.Buffer("unary0_B", act)
        unary1 = forge.op.Gelu("unary1_B", unary0)
        return unary1

    def two_chip_eth_multicast(act0, act1):
        forge.set_chip_break("unary1_C")
        forge.override_dram_queue_placement("input_0_unary0_C", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_0_unary1_C", chip_id=c1, channel=0)
        forge.override_op_size("unary0_C", (2, 1))
        forge.override_op_size("unary1_C", (1, 2))
        forge.override_op_size("matmul0_C", (2, 2))

        unary0 = forge.op.Sqrt("unary0_C", act0)
        unary1 = forge.op.Gelu("unary1_C", act1)
        matmul0 = forge.op.Matmul("matmul0_C", unary0, unary1)
        return matmul0, unary0

    def two_chip_eth_gather_multicast(act0, act1):
        forge.set_chip_break("unary1_D")
        forge.override_dram_queue_placement("input_0_unary0_D", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_0_unary1_D", chip_id=c1, channel=0)
        forge.override_op_size("unary0_D", (2, 1))
        forge.override_op_size("unary1_D", (2, 2))
        forge.override_op_size("matmul0_D", (2, 2))

        unary0 = forge.op.Gelu("unary0_D", act0)
        unary1 = forge.op.Buffer("unary1_D", act1)
        matmul0 = forge.op.Matmul("matmul0_D", unary0, unary1)
        return matmul0, unary1

    def two_chip_dram_buf_fork_c0_to_c0c1(act):
        forge.set_chip_break("unary1_E")
        forge.override_dram_queue_placement("inputs", chip_id=c0, channel=0)

        unary0 = forge.op.Gelu("unary0_E", act)
        unary1 = forge.op.Buffer("unary1_E", unary0)
        add0 = forge.op.Add("add_E", unary1, act)
        return add0

    def two_chip_l1_buf_fork_c0_to_c1c1_same_consumer(act):
        forge.set_chip_break("add0_F")
        forge.override_dram_queue_placement("inputs", chip_id=c0, channel=0)

        unary0 = forge.op.Buffer("unary0_F", act)
        add0 = forge.op.Add("add0_F", unary0, unary0)
        return add0

    def two_chip_binary_inputs_c1_tensix_c1_dram(act0, act1):
        forge.set_chip_break("unary0_G")
        forge.override_dram_queue_placement("input_0_nop0_G", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_1_add0_G", chip_id=c1, channel=0)

        nop0 = forge.op.Buffer("nop0_G", act0)
        unary0 = forge.op.Buffer("unary0_G", nop0)
        add0 = forge.op.Add("add0_G", unary0, act1)
        return add0, nop0

    def two_chip_multiply_inputs_c0_tensix_c1_dram(act0, act1):
        forge.set_chip_break("multiply0_H")
        forge.override_dram_queue_placement("input_0_nop0_H", chip_id=c0, channel=0)
        forge.override_dram_queue_placement(
            "input_1_multiply0_H", chip_id=c1, channel=0
        )

        nop0 = forge.op.Buffer("nop0_H", act0)
        multiply0 = forge.op.Multiply("multiply0_H", nop0, act1)
        return multiply0, nop0

    def two_chip_binary_inputs_c0_tensix_c1_tensix(act0, act1, act2):
        forge.set_chip_break("nop0_I")
        forge.override_dram_queue_placement("input_0_add0_I", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_1_add0_I", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_0_nop0_I", chip_id=c1, channel=0)

        add0 = forge.op.Add("add0_I", act0, act1)
        nop0 = forge.op.Buffer("nop0_I", act2)
        add1 = forge.op.Add("add1_I", add0, nop0)
        return add1, add0, nop0

    def two_chip_multiply_inputs_c0_tensix_c0_tensix(act0, act1, act2):
        forge.set_chip_break("multiply0_J")
        forge.override_dram_queue_placement("input_0_add0_J", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_1_add0_J", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_0_nop0_J", chip_id=c0, channel=0)

        add0 = forge.op.Add("add0_J", act0, act1)
        nop0 = forge.op.Buffer("nop0_J", act2)
        multiply0 = forge.op.Multiply("multiply0_J", add0, nop0)
        return multiply0, add0, nop0

    def two_chip_matmul_inputs_c0_dram_c1_tensix(act0, act1, act2):
        forge.set_chip_break("add0_K")
        forge.override_dram_queue_placement("input_0_nop0_K", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_0_nop1_K", chip_id=c0, channel=0)
        forge.override_dram_queue_placement("input_0_matmul0_K", chip_id=c0, channel=0)

        nop0 = forge.op.Buffer("nop0_K", act0)
        nop1 = forge.op.Buffer("nop1_K", act1)
        add0 = forge.op.Add("add0_K", nop0, nop1)
        matmul0 = forge.op.Matmul("matmul0_K", act2, add0)
...
def test_galaxy_dram_buf_fork(microbatch):
    ...
    def dram_buf_fork(act):
        forge.override_dram_queue_placement("input_unary0", chip_id=0, channel=0)
        for i in range(MAX_BUF_FORK):
            forge.set_chip_break(f"unary{i}")

        op_list = [forge.op.Buffer(f"unary{i}", act) for i in range(MAX_BUF_FORK)]
        return op_list
...
def test_galaxy_l1_buf_fork(microbatch):
    ...
    def l1_buf_fork(act):
        ...
        op_list = [
            forge.op.Buffer(f"unary{i}", unary0) for i in range(1, MAX_BUF_FORK)
        ]
        return op_list
...
def test_full_grid_unary_nops_multichip(microbatch, num_chips):
    ...
    def full_grid_unary_nop(act):
        ...
        for op in op_names:
            forge.override_op_size(op, (1, num_cols))

        nop = forge.op.Buffer(op_names[0], act)

        for i in range(1, len(op_names)):
            # set chip break after one chip is full of ops
            if i % num_rows == 0:
                forge.set_chip_break(op_names[i])
            nop = forge.op.Buffer(op_names[i], nop)
        return nop
...
def test_vf_chip(microbatch, scan_chip):
    ...
    def large_matmul(act0, act1):
        matmul0 = forge.op.Matmul("matmul", act0, act1)
        forge.override_op_size("matmul", (num_rows, num_cols))
        return matmul0
