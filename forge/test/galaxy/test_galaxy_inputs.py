...
def test_galaxy_large_inputs():
    ...
    def test_large_mb_bert_size_host_input(act):
        mmio_chip = 0
        forge.override_dram_queue_placement("input_0_unary0", chip_id=mmio_chip, channel=0)
...
def get_bandwidth_test(input_chip, dram_channel, verify_cfg):
    ...
    def test_unary_op_input_on_host(act):
        forge.override_dram_queue_placement("inputs", chip_id=input_chip, channel=dram_channel)
        #forge.override_op_size("unary0", (1,1))

        unary0 = forge.op.Buffer("unary0", act)
        return unary0
