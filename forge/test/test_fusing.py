...
from forge import BackendDevice
...
def check_fused_result(exp_count, *ops):
    ...
    g = forge.forgeglobal.get_devices()[0]._compile_output.lowered_graph
...
class FuseEltwise(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        a1 = forge.op.Matmul("matmul", act1, self.weights1)

        # Expecting fusing of a2 and a3
        a2 = forge.op.Add("add", act2, a1)
        a3 = forge.op.Reciprocal("reciprocal", a2)
        return a3
...
class FuseForkJoin(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        ...
        a2 = forge.op.Add("add", act2, a1)
        a3 = forge.op.Reciprocal("reciprocal", a2)
        a4 = forge.op.Sqrt("sqrt", a2)
        return forge.op.Multiply("mul", a3, a4)
...
class FuseReduce(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        ...
        a2 = forge.op.Add("add", act2, a1)
        a3 = forge.op.ReduceAvg("reduce_avg", a2, dim=-1)
        if (self.middle):
            a3 = forge.op.Reciprocal("reciprocal", a3)
        return a3

@pytest.mark.parametrize("middle", (True, False), ids=["middle", "end"])
def test_fuse_reduce(test_device, middle, test_kind):

    os.environ["FORGE_FUSE_REDUCE"] = "1"
    ...
    try:
        ...
        del os.environ["FORGE_FUSE_REDUCE"]

class FuseReduceRC(forge.ForgeModule):
    """
    ...
    def __init__(self, name):
        super().__init__(name)
        self.weights = forge.Parameter(self.shape1[-1], self.shape1[-1], requires_grad=True)

    def forward(self, act1, act2):
        a1 = forge.op.Matmul("matmul", act1, self.weights)

        # Expecting ops below to be fused into two fused ops -
        # because we cannot have reduces with both R and C dims in one fused op.
        a2 = forge.op.ReduceAvg("reduce_avg_r", a1, dim=-2)
        a3 = forge.op.Add("add", act2, a2)

        a4 = forge.op.ReduceAvg("reduce_avg_c", a3, dim=-1)
        a5 = forge.op.Multiply("multiply", a4, act2)
...
class FuseForkBroadcast(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        ...
        a2 = forge.op.Add("add", act2, a1)
        a3 = forge.op.ReduceAvg("reduce_avg", a2, dim=-1)
        a4 = forge.op.Add("add_join", a2, a3)

        if not self.second_op:
            return a4

        a5 = forge.op.Matmul("matmul2", a4, self.weights2)

        # Expecting fusing of a different op here
        a6 = forge.op.Gelu("gelu1", a5)
        a7 = forge.op.Gelu("gelu2", a6)
...
class FuseTileBroadcast(forge.ForgeModule):
    """
    ...
    def __init__(self, name, dim):
        ...
        self.weights1 = forge.Parameter(self.shape[-1], self.shape[-1], requires_grad=True)
        if dim == "r":
            self.bias1 = forge.Parameter(1, self.shape[-1], requires_grad=True)
        else:
            self.bias1 = forge.Parameter(self.shape[-2], 1, requires_grad=True)

    def forward(self, act1):
        a1 = forge.op.Matmul("matmul1", act1, self.weights1)

        # Expecting fusing of op below, which will have a tile broadcast added
        a2 = forge.op.Add("add", a1, self.bias1)
        return a2
...
def test_tile_broadcast(test_device, test_kind, dim):
    ...
    os.environ["FORGE_DISABLE_TILE_BROADCAST_CONSTEVAL"] = "1"
    try:
        relative_atol, pcc = get_relaxed_atol_pcc(True, test_device)
        ...
        del os.environ["FORGE_FUSE_REDUCE"]
        del os.environ["FORGE_NO_FUSE_MATMUL_BIAS"]
        del os.environ["FORGE_DISABLE_TILE_BROADCAST_CONSTEVAL"]


class FuseSoftmax(forge.ForgeModule):
    """
    ...
    def forward(self, act1):
        ...
        a2 = forge.op.Softmax("softmax", a1, dim=self.dim)
        return a2
...
class FuseLayernorm(forge.ForgeModule):
    """
...
class FuseMatmulBias(forge.ForgeModule):
    """
    ...
    def __init__(self, name):
        ...
        self.weights1 = forge.Parameter(self.shape[-1], self.shape[-1], requires_grad=True)
        self.weights2 = forge.Parameter(self.shape[-1], self.shape[-1], requires_grad=True)
        self.bias1 = forge.Parameter((100.0 * torch.rand(1, self.shape[-1])).detach(), requires_grad=True)
        self.bias2 = forge.Parameter((100.0 * torch.rand(1, self.shape[-1])).detach(), requires_grad=True)

    def forward(self, act1):
        a1 = forge.op.Matmul("matmul1", act1, self.weights1) + self.bias1
        a2 = forge.op.Sqrt("sqrt", a1)
        a3 = forge.op.Matmul("matmul2", a2, self.weights2) + self.bias2
        return a3
...
class FuseMatmulGelu(forge.ForgeModule):
    """
    ...
    def forward(self, act1):
        a1 = forge.op.Matmul("matmul1", act1, self.weights1, self.bias1)
        a2 = forge.op.Gelu("gelu1", a1)
        a3 = forge.op.Matmul("matmul2", a2, self.weights2)
        a4 = forge.op.Gelu("gelu2", a3)
        a5 = forge.op.Matmul("matmul3", a4, self.weights3)
        return a5
...
class FuseTwoLayernorm(forge.ForgeModule):
    """
    ...
    def __init__(self, name):
        ...
        self.weights1 = forge.Parameter(self.shape[-1], self.shape[-1], requires_grad=True)
        self.weights2 = forge.Parameter(self.shape[-1], self.shape[-1], requires_grad=True)
        self.ln_weights1 = forge.Parameter(1, self.shape[-1], requires_grad=True)
        self.ln_bias1 = forge.Parameter(1, self.shape[-1], requires_grad=True)
        self.ln_weights2 = forge.Parameter(1, self.shape[-1], requires_grad=True)
        self.ln_bias2 = forge.Parameter(1, self.shape[-1], requires_grad=True)

    def forward(self, act1):
        a1 = forge.op.Matmul("matmul1", act1, self.weights1)

        # Two layernorms with matmul in between. We want to see the fused op reused twice.
        a2 = forge.op.Layernorm("layernorm1", a1, self.ln_weights1, self.ln_bias1)
        a3 = forge.op.Matmul("matmul2", a2, self.weights2)
        a4 = forge.op.Layernorm("layernorm2", a3, self.ln_weights2, self.ln_bias2)
        return a4
...
class FuseReduceMax(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        ...
        # Expecting fusing of ops below
        a2 = forge.op.Add("add", act2, a1)
        a3 = forge.op.ReduceMax("reduce_max", a2, dim=-1)
        if (self.middle):
            a3 = forge.op.Reciprocal("reciprocal", a3)
        return a3
...
class FuseSelect(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        ...
        a1 = forge.op.Add("add0", act1, act2)
        a2 = forge.op.Select("select0", a1, self.dim, (self.index, self.length)) 
        return a2
...
class FuseBuffer(forge.ForgeModule):
    """
...
class FuseEpochAndChipBreak(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        ...
        a1 = forge.op.Add("add0", act1, act2)
        a2 = forge.op.Buffer("buffer_a", a1)
        a3 = forge.op.Buffer("buffer_b", a2)
        a4 = forge.op.Buffer("buffer_c", a2)
        a5 = forge.op.Add("add1", a3, a4)
...
class FuseSimpleTileBroadcast(forge.ForgeModule):
    """
    Module with a tile broadcast
    """
    ...
    def __init__(self, name):
        super().__init__(name)
        self.bias1 = forge.Parameter(1, 1, requires_grad=False)

    def forward(self, act1):
        a1 = forge.op.Subtract("sub", act1, self.bias1)
        return a1
...
class FuseBroadcastAsLHSOfMatmul(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        ...
        a1 = forge.op.Add("add", act1, act2)
        a2 = forge.op.ReduceAvg("reduce_avg", a1, dim=-1)
        a3 = forge.op.Add("add_join", a1, a2)

        # Have fused op as LHS argument
        a4 = forge.op.Matmul("matmul", a3, self.weights)
        return a4
...
class FuseBroadcastOutputOp(forge.ForgeModule):
    """
    ...
    def forward(self, act1, act2):
        ...
        act1_reduced = forge.op.ReduceAvg("reduce_avg_0", act1, dim=-1)
        act2_reduced = forge.op.ReduceAvg("reduce_avg_1", act2, dim=-1)
        ...
        a1 = forge.op.Add("add", act1_reduced, act2_reduced)
        a2 = forge.op.Reciprocal("reciprocal", a1)
        a3 = forge.op.Broadcast("broadcast", a2, -1, self.shape[-1])
        a4 = forge.op.Multiply("multiply", a3, act1_reduced)

        a5 = forge.op.Matmul("matmul", a4, self.weights)
        return a5
