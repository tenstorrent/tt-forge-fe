...
class PyBudify(torch.nn.Module):
    def __init__(self, pt_module, device='silicon', arch='wormhole_b0', precision='fp32', amp_level=0, micro_batch_size=1, fuse=False, num_chips=1, perf=None,
        ...
        if device != 'pytorch':
            ...
            # os.environ["FORGE_PLACER_SNAKE"] = "1" Not what we want for dual chip placement
            # os.environ["FORGE_DISABLE_INTERACTIVE_PLACER"] = "1" # Until interactive placer supports multi-chip placement overrides
        ...
        if device == 'pytorch':
            ...
            module = forge.PyTorchModule(netlist_name, self.bound_module)
            ...
            if matmuls == 'bfp8':
                ...
                forge.config.configure_mixed_precision(
                    name_regex="matmul_.*",
                    input_df={0: [forge.DataFormat.Bfp8_b, True], 1: [forge.DataFormat.Bfp8_b, True], 2: [forge.DataFormat.Bfp8_b, True]})
                ...
                forge.config.configure_mixed_precision(
                    ...
                    input_df={0: [forge.DataFormat.Bfp8_b, False], 1: [forge.DataFormat.Bfp8_b, False], 2: [forge.DataFormat.Bfp8_b, False]})
                for i in range(31):
                    forge.config.configure_mixed_precision(
            ...
            if decode_mode:
                ...
                forge.config.configure_mixed_precision(
                    ...
                    input_df={0: [forge.DataFormat.Float16_b, True], 1: [forge.DataFormat.Float16_b, True], 2: [forge.DataFormat.Float16_b, True]})
            ...
            if padded_fracture:
                ...
                factor = 2
                for layer_num in range(len(self.bound_module.layers)):
                    forge.config.insert_fracture_group([(f"matmul_{18+layer_num*offset}", -1, factor), (f"matmul_{23+layer_num*offset}", forge.k_dim, factor)])
                ...
                forge.config.configure_mixed_precision(
                    ...
                    input_df={0: [forge.DataFormat.Bfp8_b, True], 1: [forge.DataFormat.Bfp4_b, True], 2: [forge.DataFormat.Bfp8_b, True]})
                forge.config.configure_mixed_precision(
            ...
            if padded_fracture_p:
                ...
                for layer_num in range(len(self.bound_module.layers)):
                    if layer_num < len(self.bound_module.layers)//2:
                        forge.config.insert_fracture_group([(f"matmul_{18+layer_num*offset}", -1, 2), (f"matmul_{23+layer_num*offset}", forge.k_dim, 2)])
            ...
            if padded_fracture_full:
                ...
                if fracture_attn > 0:
                    ...
                    for layer_num in range(len(self.bound_module.layers)):
                        ...
                        attn_constr = self.add_sched(forge, entries, exits, ops, attn_factor, attn_constr)
                    forge.config.add_schedule_constraint(attn_constr)
                ...
                if fracture_mlp > 0:
                    ...
                    for layer_num in range(len(self.bound_module.layers)):
                        ...
                        forge.config.override_op_placement(f"concatenate_{48+mlp_offset}.dc.concatenate.2", chip_id=1, temporal_epoch_break=True)
                        ...
                        forge.config.insert_fracture_group([
                            ...
                            # mlp.dense_4h_to_h
                            (f"matmul_{23+mlp_offset}", forge.k_dim, mlp_factor),
                            ])
            ...
            forge.set_configuration_options(default_df_override=fallback,
            ...
            if self.masked_odkv:
                ...
                # forge.config.set_epoch_break("transpose_58.dc.sparse_matmul.4.lc2")
                # forge.config.set_epoch_break("matmul_64")

                # forge.config.add_schedule_constraint(['transpose_58.dc.sparse_matmul.4.lc2', 'add_59'])
                names = []
                if num_layers == 1:
                    names_start_idx = 56
                    names_end_idx = 57
                ...
                # forge.config.insert_fracture_group([(f"concatenate_50", 2, 2)])
                # forge.config.insert_fracture_group([(f"concatenate_67", 2, 2)])
    ...
    def run_async(self, *args):
        ...
        if self.odkv or self.masked_odkv:
            ...
            self.forge.run_forward(input_count=1)
    ...
    def add_sched(self, forge, entries, exits, ops, factor, constr):
        for elem in entries:
            constr.append(elem)
        for lst in ops:
        ...
        # for elem in exits:
            # constr.append(elem)
        # forge.config.override_op_placement(exits[0], temporal_epoch_break=True)
