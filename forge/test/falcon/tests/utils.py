...
def lorify_model(model, target_modules, rank, num_lora_layers=None):
    # prepare_model_for_kbit_training is not needed at the moment, sets everything to FP32 and we overwrite data formats in forge anyways
...
def create_experiment_dir(parent_dir='falcon_lora_experiments', experiment_name='untitled'):
    ...
    forge_exp_path = os.path.join(experiment_path, 'forge')
    os.makedirs(forge_exp_path, exist_ok=True)
