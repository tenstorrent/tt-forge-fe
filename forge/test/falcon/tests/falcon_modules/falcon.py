...
def run_demo_sync_finetune(model, tokenizer, dataloader, num_layers, sequence_length):
    ...
    generator_name = f"forge-greedy-alpaca_eval-ci"
...
default_args = Namespace(
    ...
    activation_cache_file='/proj_sw/large-model-cache/falcon7b/cached_activations/cached_activations_bfp8b_quantW_fp16b_compute_forge.pth',
    num_cache_layers=16,
    wandb_log_steps=20,
...
def run_finetune(cfg_file):
    ...
    parser.add_argument('--with-forge', action='store_true', help='Train a model using forge (use --device to select golden or silicon)')
    parser.add_argument('--forge-device', choices=['golden', 'silicon'], help='Forge argument only, run on golden/silicon')
    ...
    parser.add_argument('--verify', action='store_true', help='Enable forge verification code to compare tensors of intermediate forge passes')
    ...
    # Forge and TTDevice specific options
    ...
    parser.add_argument('--forge-log-level', choices=['TRACE', 'DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'], help='Forge log level')
    parser.add_argument('--tt-build-dir', type=str, help='Use this custom TT build directory')
    parser.add_argument('--perf', choices=['none', 'light', 'verbose'], help='Forge performance trace setting')
    parser.add_argument('--netlist-name', type=str, help='Netlist name')

    # Forge specific placement overwrites
    ...
    parser.add_argument('--ci-tolerance', type=float, help='Assert if CI target metric (loss/grad_norm) difference between forge and pytorch is not within this tolerance in CI mode')
    ...
    args.forge_device=args.forge_device if args.forge_device is not None else default_args.forge_device
    ...
    args.forge_log_level=args.forge_log_level if args.forge_log_level is not None else default_args.forge_log_level
    ...
    # Ensure a non-weka FORGE_BUILD_DIR for tt_build files unless specified
    forge_build_dir = os.environ.setdefault("FORGE_BUILD_DIR", f"/tmp/{getuser()}/tt_build")
    print(f"FORGE_BUILD_DIR set to {forge_build_dir}")
    ...
    if not args.netlist_name:
        assert os.getenv('FORGE_NETLIST_OVERRIDE') is None, 'FORGE_NETLIST_OVERRIDE is set, but --netlist-name is not specified, you probably wanted to use "transformer"'
    ...
    if args.wandb_log_name is None:
        with_name = {(True, True): 'both', (True, False): 'pytorch', (False, True): 'forge'}[(args.with_pytorch, args.with_forge)]
    ...
    if args.ci_tolerance is not None:
        assert args.with_pytorch and args.with_forge, 'CI mode with a target tolerance requires both --with-pytorch and --with-forge'
    ...
    assert args.with_pytorch or args.with_forge, "Need to specify at least one trainer to run"
    ...
    if args.with_forge:
        set_model_seed()
        trainers.append(ForgeTrainer(args))
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        for inputs in train_dataloader:
            ...
            if args.data_debug:
                pytorch_loss, forge_loss = losses[0], losses[1]
                is_loss_difference = abs(pytorch_loss - forge_loss) / pytorch_loss > 0.05
                pytorch_grad_norm, forge_grad_norm = trainers[0].grad_norm, trainers[1].grad_norm
                is_grad_norm_difference = abs(pytorch_grad_norm - forge_grad_norm) / pytorch_grad_norm > 0.05
                if is_loss_difference:
                    print(f"WARNING: Losses more than 5% apart: pytorch_loss={pytorch_loss}, forge_loss={forge_loss}")
                if is_grad_norm_difference:
                    print(f"WARNING: Gradient norms more than 5% apart: pytorch_grad={pytorch_grad_norm}, forge_grad={forge_grad_norm}")
            ...
            if args.ci_tolerance:
                if args.ci_target == 'loss':
                    pytorch_val, forge_val = losses[0], losses[1]
                elif args.ci_target == 'grad_norm':
                    pytorch_val, forge_val = trainers[0].grad_norm, trainers[1].grad_norm
                else:
                    assert False, "Target metric not supported"
                diff = abs(pytorch_val - forge_val)
                if diff > args.ci_tolerance:
                    error_msg = f"CI FAIL: Metric {args.ci_target} more than ci_tolerance ({args.ci_tolerance}) apart: pytorch={pytorch_val}, forge={forge_val}"
...
class ForgeTrainer():
    def __init__(self, args):
        ...
        os.environ["LOGGER_LEVEL"] = args.forge_log_level
        os.environ["LOGURU_LEVEL"] = args.forge_log_level
        # forge workarounds
        os.environ["GOLDEN_WORMHOLE_B0"] = "1"
        os.environ["WHA0_DISABLE_RELAY_BUFS"] = "1"
        os.environ["FORGE_CONVERT_PARAMS_TO_TVM"] = "0"
        ...
        os.environ['FORGE_DRAM_FLIP_FLOP'] = '1'

        # os.environ['FORGE_DRAM_PICK_CAPACITY'] = '1' # Don't need this anymore and better for perf without

        # # Testing for better handling out of valid grid issues with seqlen=1024
        # os.environ["FORGE_PADDING_PASS"] = "1"
        ...
        if args.optimizer_on_host:
            ...
            self.optimizer = forge.optimizers.AdamW(learning_rate=args.learning_rate, device_params=True)
        ...
        self.log_histograms = args.log_histograms
        self.log_opt = args.log_opt
        device = args.forge_device

        if args.tt_build_dir:
            backend_output_dir = os.path.join(forge_build_dir, args.tt_build_dir)
            os.environ["FORGE_BUILD_DIR"] = backend_output_dir 
            print("Setting backend output dir to %s" % backend_output_dir)
            forge.set_configuration_options(backend_output_dir=backend_output_dir)
        ...
        self.embeddings_module = forge.PyTorchModule("embeddings", self.model.before_decoders)
        self.transformer_module = forge.PyTorchModule(args.netlist_name, self.model.transformer)
        self.lm_head_module = forge.PyTorchModule("lm_head", self.model.after_decoders)
        self.loss_module = forge.PyTorchModule("loss", self.model.loss_fct)

        if args.precision == 'fp32':
            default_df_override = forge.DataFormat.Float32
        elif args.precision == 'fp16':
            default_df_override = forge.DataFormat.Float16
        ...
        self.apply_data_formats(lora_data_format=forge.DataFormat.Float32, num_layers=num_layers, num_lora_layers=args.num_lora_layers, precision=args.precision)
        ...
        perf_level = { None    : None,
                        ...
                        'verbose': forge.PerfTraceLevel.VERBOSE }[args.perf]

        forge.set_configuration_options(
                                        default_df_override=default_df_override,
                                         accumulate_df=forge.DataFormat.Float32,
                                         amp_level=0,
        ...
        self.cpu0 = forge.CPUDevice("cpu0", module=self.embeddings_module)

        start_time = time()
        if args.tti_load is not None:
            print(f"Load TTImage from : {args.tti_load}")
            self.tt0 = forge.TTDevice.load_image(img_path=args.tti_load)
        else:
            self.tt0 = forge.TTDevice('tt0', module=self.transformer_module,
        ...
        if args.loss_on_device:
            ...
            self.cpu1 = forge.CPUDevice("cpu1", module=self.lm_head_module)
        ...
        if args.verify:
            self.verify_cfg=forge.VerifyConfig(enabled=True,
                                                ...
                                                test_kind=self.forge.verify.config.TestKind.TRAINING,
                                                devtype=forge.BackendType.Silicon,
                                                arch=forge.BackendDevice.Wormhole_B0,
                                                pcc=99.999,
                                                verify_all=True,
    ...
    def placement_overwrites(self, num_layers, num_lora_layers):
        for i in range(10000): # Overwrite all gelu op placements
            self.forge.config.override_op_size(f'bw_in0_gelu_{i}_multiply_1', (1, 8))
        self.forge.config.override_op_size('input_1_multiply_247_splt_brcst_1_0_splt_brcst_3_0', (1, 4))
    ...
    def multichip_placement(self, num_layers, num_lora_layers, precision):
        ...
        for i in range(num_non_lora_layers):
            ...
            self.forge.config.set_epoch_break(op_name)
        ...
        if num_non_lora_layers == 0:
            ...
            for i in range(num_lora_layers):
                ...
                self.forge.config.override_op_placement(op_name, chip_id=chip_id)
            ...
            for i in range(num_non_lora_layers):
                op_name = f"multiply_{0 + non_lora_loop_offset_multiply*i}"
                self.forge.config.override_op_placement(op_name, chip_id=chip_id_0)
            ...
            for i in range(num_lora_layers):
                op_name = f"multiply_{0 + initial_offset_multiply + lora_loop_offset_multiply*i}"
                self.forge.config.override_op_placement(op_name, chip_id=chip_id_1)
        ...
        self.forge.config.override_op_size(add_name, (1, 2))
    ...
    def apply_data_formats(self, lora_data_format, num_layers, num_lora_layers, precision):
        ...
        if precision == 'very-low-mp': # all matmul inputs are bfp8_b; keep bwd/loss/lora bf16b
            self.forge.config.configure_mixed_precision(
                    name_regex="matmul_.*",
                    input_df={0: [self.forge.DataFormat.Bfp8_b, True], 1: [self.forge.DataFormat.Bfp8_b, True], 2: [self.forge.DataFormat.Bfp8_b, True]})

            if not self.args.optimizer_on_host: # Overwrites needed only for opt on device
                self.forge.config.configure_mixed_precision(
                        name_regex=".*lora_.*",
                        output_df=self.forge.DataFormat.Float16_b)
        ...
        if precision == 'almost-low-mp': # all matmul inputs are bfp8_b; keep bwd/loss/lora bf16b
            ...
            self.forge.config.configure_mixed_precision(
                    name_regex=".*lora_.*",
                    output_df=self.forge.DataFormat.Float16_b,
                    input_df={0: [self.forge.DataFormat.Float16_b, True], 1: [self.forge.DataFormat.Float16_b, True], 2: [self.forge.DataFormat.Float16_b, True]})
            ...
            self.forge.config.configure_mixed_precision(
                    ...
                    output_df=self.forge.DataFormat.Bfp8_b,
                    )
            ...
            # required for falcon-stable forge (08/2023)
    ...
    def ensure_initialized(self, batch):
        if not self.initialized:
            ...
            if self.args.tti_save is not None:
                ...
                sys.exit(0)
            self.forge.initialize_pipeline(training=True,
                                    sample_inputs=batch,
                                    sample_targets=batch,
    ...
    def forward_step(self, batch):
        ...
        assert not self.args.save_to_activation_cache, "Saving activation cache for forge model not implemented yet!"

        return self.loss

    def backward_step(self, zero_grad):
        step_start_time = time()
        self.forge.run_backward(input_count=1, zero_grad=zero_grad, _sequential=True)
        self.time_per_bwd_step = (time() - step_start_time)
        self.accumulated_batch_time_per_step += self.time_per_bwd_step
    ...
    def optimizer_step(self, do_update):
        ...
        if self.args.optimizer_on_host:
            ...
            gradients = self.forge.get_parameter_gradients(tt_device, _sequential=True)[0]
            ...
            #     self.gradients = self.forge.get_parameter_gradients(self.tt0, _sequential=True)[0]
            #     self.grad_norm = get_gradient_norm(self.model.transformer.named_parameters(), self.gradients)
            #     self.params = self.forge.get_parameter_checkpoint(self.tt0, _sequential=True)[0]

            self.forge.run_optimizer(_sequential=True)
            if self.log_histograms:
                # FIXME: not tested. get gradients and params from device for logging
                self.gradients = self.forge.get_parameter_gradients(tt_device, _sequential=True)[0]
                self.params = self.forge.get_parameters(tt_device, _sequential=True)[0]
    ...
    def save(self, filename):
        if not self.args.optimizer_on_host:
            ...
            device_params = self.forge.get_parameter_checkpoint(self.tt0, _sequential=True)[0]
