...
class PyBudify(torch.nn.Module):
    def __init__(self, pt_module, device='silicon', arch='wormhole_b0', precision='fp32', amp_config_file=None, micro_batch_size=1, fuse=False, num_chips=1, perf=None, verify=False, log_level='ERROR', tti_save=None, tti_load=None,
        ...
        if device != 'pytorch':
            ...
            # os.environ["FORGE_DISABLE_FORK_JOIN_BUF"] = "1"
            os.environ["FORGE_ENABLE_OUTPUT_QUEUES_ON_HOST"] = "0"
        ...
        if device == 'pytorch':
            ...
            amp_config.apply_amp_settings(forge, amp_config_file, num_layers)

            num_layers = 32 if num_layers is None else num_layers

            if placement_config_file is not None:
                manual_placer(forge.config, placement_config_file, loops=num_layers)
            ...
            if self.prefill_kvs:
                ...
                for layer_num in range(num_layers):
                    ...
                    forge.config.add_schedule_constraint([f'concatenate_{30+k}.dc.concatenate.0', f'index_{INDEX_START+j}.dc.select.0', f'index_{INDEX_START+j}.dc.buffer.1', f'{netlist_name}.output_hstack_{INDEX_START+1+j}_tm_nop', f'matmul_{33+k}',
                    ...
                    forge.config.override_op_size(f'{netlist_name}.output_hstack_{INDEX_START + 1 +j}_tm_nop', (1,4))
                    # Removed these since they don't play nice with fusion
                    # if layer_num < (num_layers - 1):
                    #     forge.config.add_schedule_constraint([f'add_{76+k}', f'layers.{layer_num+1}.input_layernorm.weight_s_brcst_m2_0_0.lc1'])
                # Ensure all decoders start at same op
                # forge.config.set_epoch_break(f'layers.{layer_num}.input_layernorm.weight_s_brcst_m2_0_0.lc1')
            ...
            forge.set_configuration_options(enable_auto_fusing=fuse, performance_trace=perf_level, backend_opt_level=opt_level, input_queues_on_host=False)
...
#                                                      golden_compare_callback=debug_tensors,
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.device == 'pytorch':
            ...
            if self.prefill_kvs:
                self.forge.run_generate(input_count=1, write_index=self.write_index, _sequential=True)
            else:
