...
class PyBudify(torch.nn.Module):
    def __init__(
        ...
        if device == "pytorch":
            ...
            devtype = {
                "golden": forge.BackendType.Golden,
            ...
            if bfp8_matmuls:
                forge.config.configure_mixed_precision(
                    name_regex="matmul_.*",
                    input_df={
                        0: [forge.DataFormat.Bfp8_b, True],
                        1: [forge.DataFormat.Bfp8_b, True],
                        2: [forge.DataFormat.Bfp8_b, True],
                    },
                )
            ...
            if decode_mode:
                ...
                forge.config.configure_mixed_precision(
                    ...
                    input_df={
                        ...
                        2: [forge.DataFormat.Float16_b, True],
                    },
                )

            # forge.config.configure_mixed_precision(
            #     name_regex="matmul_.*",
            #     input_df={1: [forge.DataFormat.Bfp8_b, True]})

            # forge.override_op_size('matmul_61', (1,2))
            ...
            if padded_fracture:
                ...
                forge.config.configure_mixed_precision(
                    ...
                    input_df={
                        0: [forge.DataFormat.Bfp8_b, True],
                        1: [forge.DataFormat.Bfp4_b, True],
                        2: [forge.DataFormat.Bfp8_b, True],
                    },
                )
                forge.config.configure_mixed_precision(
            ...
            perf_level = {
                ...
                "verbose": forge.PerfTraceLevel.VERBOSE,
            }[perf]
            ...
            forge_arch = {
                "grayskull": forge.BackendDevice.Grayskull,
            ...
            if verify:
                self.verify_cfg = forge.VerifyConfig(
                    verify_all=True,
                    verify_last=True,
    ...
    def ensure_initialized(self, *args):
        if not self.initialized and self.device != "pytorch":
            if self.tti_save is not None:
                ...
                sys.exit(0)
            self.forge.initialize_pipeline(
                training=False,
                sample_inputs=args,
