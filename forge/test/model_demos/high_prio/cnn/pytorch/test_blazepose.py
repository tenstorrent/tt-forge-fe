...
from forge.verify.config import TestKind, BackendDevice, BackendType
...
def test_blazepose_detector_pytorch(test_device):
    ...
    verify_module(
        forge.PyTorchModule("pt_blazepose_detector", pose_detector),
...
def test_blaze_palm_pytorch_1x1(test_device):
    ...
    verify_module(
        forge.PyTorchModule("pt_palm_detector", palm_detector),
        input_shapes=[(img2.shape,)],
        inputs=[(img2,)],
        verify_cfg=forge.VerifyConfig(
            arch=test_device.arch,
