...
def test_fuyu8b_past_cache(test_device):
    ...
    # Instantiate modules
    if "FUYU8B_FULL_LAYERS" in os.environ and os.environ["FUYU8B_FULL_LAYERS"]:
        img_decoder = forge.PyTorchModule("pt_fuyu8b_past_cache_img", FuyuModelImgDecoderWrapper(fuyu_model))  # feed inputs_embeds
        txt_decoder = forge.PyTorchModule("pt_fuyu8b_past_cache_txt", FuyuModelTxtDecoderWrapper(fuyu_model))  # feed inputs_embeds
    else:
        img_decoder = forge.PyTorchModule(f"pt_fuyu8b_past_cache_img_{num_layers}", FuyuModelImgDecoderWrapper(fuyu_model))  # feed inputs_embeds
        txt_decoder = forge.PyTorchModule(f"pt_fuyu8b_past_cache_txt_{num_layers}", FuyuModelTxtDecoderWrapper(fuyu_model))  # feed inputs_embeds
