...
def test_wider_resnet_torch(test_kind, test_device):
    ...
    compiler_cfg = _get_global_compiler_config()
    compiler_cfg.balancer_policy = "CNN"
    compiler_cfg.compile_depth = CompileDepth.GENERATE_INITIAL_GRAPH # Needs neg maxpool support tenstorrent/forge#188
...
def test_gated_spatial_conv_torch(test_kind, test_device):
    ...
    compiler_cfg = _get_global_compiler_config()
    compiler_cfg.balancer_policy = "CNN"
    compiler_cfg.compile_depth = CompileDepth.PRE_LOWERING_PASS # tenstorrent/forge#185
