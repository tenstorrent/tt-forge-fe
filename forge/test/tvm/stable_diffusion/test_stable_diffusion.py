...
def test_unet(test_device):
    ...
    os.environ["FORGE_MAX_GRAPH_CUT_RETRY"] = "2000"
    os.environ["FORGE_DECOMPOSE_SIGMOID"] = "1"
    os.environ["FORGE_RIBBON2"] = "1"
    os.environ["FORGE_PAD_SPARSE_MM_WEIGHT_MM"] = "{10:12, 20:24, 30:32, 40:48, 60:64}"
    ...
    # ttdevice = forge.TTDevice("tt0", module=mod,arch=BackendDevice.Wormhole_B0, devtype=BackendType.Silicon)
...
def test_unet_CrossAttention(test_device):
    ...
    compiler_cfg.default_df_override = forge.DataFormat.Bfp8_b
...
def test_unet_up_block(test_device):
    import os
    os.environ["FORGE_PAD_SPARSE_MM_WEIGHT_MM"] = "{10:12, 20:24, 40:48}"
    os.environ["FORGE_FORCE_CONV_MULTI_OP_FRACTURE"] = "1"
...
def test_unet_section(test_device):
    ...
    class Test(ForgeModule):
        ...
        def forward(self, x):
            softmax_210 = forge.op.Softmax("", x, dim=-1) # (1, 1, 320, 4096)
            reshape_211 = forge.op.Reshape("", softmax_210, shape=(1, 32, 10, 4096))
            reduce_avg_212 = forge.op.ReduceAvg("", reshape_211, dim=-2) # (1, 32, 1, 4096)
            reduce_avg_213 = forge.op.ReduceAvg("", reduce_avg_212, dim=-1) # (1, 32, 1, 1)
            subtract_214 = forge.op.Subtract("", reshape_211, reduce_avg_213) # (1, 32, 10, 4096)
            subtract_215 = forge.op.Subtract("", reshape_211, reduce_avg_213) # (1, 32, 10, 4096)
            multiply_216 = forge.op.Multiply("", subtract_215, subtract_215) # (1, 32, 10, 4096)
            reduce_avg_217 = forge.op.ReduceAvg("", multiply_216, dim=-2) # (1, 32, 1, 4096)
            reduce_avg_218 = forge.op.ReduceAvg("", reduce_avg_217, dim=-1) # (1, 32, 1, 1)
            add_220 = forge.op.Add("", reduce_avg_218, self.get_constant("const_760")) # (1, 32, 1, 1)
            sqrt_221 = forge.op.Sqrt("", add_220) # (1, 32, 1, 1)
            reciprocal_222 = forge.op.Reciprocal("", sqrt_221) # (1, 32, 1, 1)
            multiply_223 = forge.op.Multiply("", subtract_214, reciprocal_222) # (1, 32, 10, 4096)
            multiply_226 = forge.op.Multiply("", multiply_223, self.get_constant("const_761")) # (1, 32, 10, 4096)
            add_228 = forge.op.Add("", multiply_226, self.get_constant("const_762")) # (1, 32, 10, 4096)
            sigmoid_229 = forge.op.Sigmoid("", add_228) # (1, 32, 10, 4096)
            multiply_230 = forge.op.Multiply("", add_228, sigmoid_229) # (1, 32, 10, 4096)
            reshape_231 = forge.op.Reshape("", multiply_230, shape=(1, 1, 320, 4096))
            softmax_232 = forge.op.Softmax("", reshape_231, dim=-1) # (1, 1, 320, 4096)
            return softmax_232
...
def test_stable_diffusion_pipeline():
    ...
    # Step 1: Compile/Load Forge module
    ...
    if not os.path.exists(save_path):
        ...
        input_shape = ((1, 4, 64, 64), (1,), (1, 77, 768))
        tt_module = forge.PyTorchModule("sd_unet_script", UnetWrapper(pipe.unet),)
        device = forge.TTDevice("tt0", module=tt_module,arch=BackendDevice.Wormhole_B0, devtype=BackendType.Silicon)
