...
def test_tvm_gpt2_attention_with_past_cache(test_device):
    ...
    output_q = forge.initialize_pipeline(training=False, sample_inputs=(torch.rand(hidden_states_shape), torch.rand(attention_mask_shape), torch.zeros(layer_past_shape), torch.zeros(layer_past_shape)), _verify_cfg=VerifyConfig(
            arch=test_device.arch,
            devtype=test_device.devtype,
            verify_all=True,
        ))
    print(forge.get_parameter_checkpoint()[0]['tensor_1'])
    ...
    pk = forge.get_parameter_checkpoint()[0]['tensor_1'].value()
    ans = output_q.get(timeout = 0.5)
    print(forge.get_parameter_checkpoint()[0]['tensor_1'])
    tt0.push_to_inputs((torch.rand(hidden_states_shape), torch.rand(attention_mask_shape), ))
    forge.run_generate(input_count=1, write_index=1)
    print(forge.get_parameter_checkpoint()[0]['tensor_1'])
    tt0.push_to_inputs((torch.rand(hidden_states_shape), torch.rand(attention_mask_shape), ))
    forge.run_generate(input_count=1, write_index=2)
    print(forge.get_parameter_checkpoint()[0]['tensor_1'])
...
def test_gpt2_past_cache(test_device):
    ...
    cpu0 = forge.CPUDevice("cpu0", module=PyTorchModule("gpt2_embeddings", embeddings))
    tt1 = forge.TTDevice("tt1", 
            devtype=test_device.devtype, arch=test_device.arch, module=PyTorchModule("gpt2_blocks", blocks))
    cpu1 = forge.CPUDevice("cpu1", module=PyTorchModule("gpt2_lm_head", lm_head))
    ...
    for i in range(tokens_to_generate):
        ...
        forge.run_generate(input_count=1, write_index=write_index)
...
def test_tvm_past_cache_generate(test_device):
    ...
    output_q = forge.initialize_pipeline(training=False, sample_inputs=(torch.rand(1), torch.zeros(layer_past_shape)), _verify_cfg=VerifyConfig(
            arch=test_device.arch,
            devtype=test_device.devtype,
        ))
    print(forge.get_parameter_checkpoint())
    tt0.push_to_inputs((torch.rand(1), ))
    tt0.push_to_inputs((torch.rand(1), ))
    forge.run_generate(input_count=2, tokens_per_iter=32, token_id=0)
    print(forge.get_parameter_checkpoint())
    ans = output_q.get()
    tt0.push_to_inputs((torch.rand(1), ))
    forge.run_generate(input_count=1, tokens_per_iter=31, token_id=64)
    ans = output_q.get()
    print(forge.get_parameter_checkpoint())
    tt0.push_to_inputs((torch.rand(1), ))
    tt0.push_to_inputs((torch.rand(1), ))
...
def test_splice(test_device):
    import forge
    mod = SpliceUnit()
    pb_mod = forge.PyTorchModule('splice', mod)
