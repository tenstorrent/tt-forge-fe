...
from forge.tvm_to_python import compile_tvm_to_python
...
from forge.config import CompilerConfig, _get_global_compiler_config, CompileDepth
...
def test_reshape(input_shape):
    ...
    in1 = forge.Tensor.create_from_torch(x)
    padx = forge.op.PadTile("", in1, -1, input_shape[-2])
    pady = forge.op.PadTile("", padx, -2, input_shape[-1])
    # padded = [0, 0] + [(pady.shape[2]-1) * TILE_DIM] + [0]
    # pady = forge.op.Pad("", pady, padded)

    spm = create_reshape_flatten_sparse_picker_matrix(pady.shape[-2], TILE_DIM*pady.shape[-2])
    spm = forge.Tensor.create_from_torch(spm)
    mm = forge.op.SparseMatmul("", spm, pady)
    result = forge.op.VSlice("", mm, pady.shape[-2])

    if input_shape[-1] % TILE_DIM == 0:
        hstk = forge.op.HSlice("", result, slc)
        result = forge.op.VStack("", hstk, hstk.shape[-3] // input_shape[-3])
    else:
        spm = create_padding_insert_sparse_picker(input_shape[-1], pady.shape[-1], slc)
        spm = forge.Tensor.create_from_torch(spm)
        t = forge.op.Transpose("", result, -2, -1)
        mm = forge.op.SparseMatmul("", spm, t)
        vslc = forge.op.VSlice("", mm, slc)
        t = forge.op.Transpose("", vslc, -2, -1)
        result = forge.op.VStack("", t, t.shape[-3] // input_shape[-3])
    ...
    spm = forge.Tensor.create_from_torch(spm)
    result = forge.op.SparseMatmul("", spm, result)
    ...
    n1 = forge.op.Narrow("", result, -1, 0, newshape[-1], result.shape[-1])
    res = forge.op.Narrow("", n1, -2, 0, newshape[-2], n1.shape[-2])
...
def test_xy_flatten():
    ...
    in1 = forge.Tensor.create_from_torch(torch.rand(input_shape))
    padx = forge.op.PadTile("", in1, -1, input_shape[-1])
    pady = forge.op.PadTile("", padx, -2, input_shape[-2])
    # pady = forge.op.Pad("", in1, (0, TILE_DIM-input_shape[-2], 0, TILE_DIM-input_shape[-1]))
    torch.set_printoptions(threshold=100000, linewidth=10000)

    if input_shape[-3] > 1:
        pady = forge.op.HStack("", pady, input_shape[-3])

    padded_shape = pady.shape
    r_new = padded_shape[-1] * input_shape[-2] // (padded_shape[-1] // TILE_DIM)
    fl_spm = forge.op.eval.create_reshape_flatten_sparse_picker_matrix(pady.shape[-2], r_new, TILE_DIM)
    fl_spm = forge.Tensor.create_from_torch(fl_spm)
    mm = forge.op.SparseMatmul("", fl_spm, pady)

    if input_shape[-3] > 1:
        mm = forge.op.HSlice("", mm, input_shape[-3])
        
    rt = align_up_tile(r_new, TILE_DIM) // TILE_DIM
    vs = forge.op.VSlice("", mm, rt)
    hs = forge.op.HStack("", vs, rt)

    if input_shape[-3] > 1:
        hs = forge.op.VStack("", hs, input_shape[-3])

    if input_shape[-1] % TILE_DIM:
        tx = forge.op.Transpose("", hs, -2, -1)
        pr_spm = forge.op.eval.create_flattened_padding_removal_sparse_picker_matrix(hs.shape[-1], 0, input_shape[-1], pad_to_tile_dim(input_shape[-1], TILE_DIM))

        pr_spm = forge.Tensor.create_from_torch(pr_spm)
        pr_mm = forge.op.SparseMatmul("", pr_spm, tx)
        utx = forge.op.Transpose("", pr_mm, -2, -1)
    else:
        utx = hs

    if input_shape[-3] > 1:
        pr_spm = forge.op.eval.create_flattened_padding_removal_sparse_picker_matrix(utx.shape[-2], 0, 1, TILE_DIM)
        pr_spm = forge.Tensor.create_from_torch(pr_spm)
        res = forge.op.SparseMatmul("", pr_spm, utx)
    else:
        res = forge.op.Narrow("", utx, -2, 0, 1, utx.shape[-2])
...
#     vslc = forge.op.VSlice("", pady, pady.shape[-2])

#     fl_spm = forge.op.eval.create_reshape_flatten_sparse_picker_matrix(1, vslc.shape[-1])
#     fl_spm = forge.Tensor.create_from_torch(fl_spm)
#     mm = forge.op.SparseMatmul("", fl_spm, mm)
...
def test_zx_transpose():
    ...
    vs = forge.op.VStack("", pady, 1280)
    fspm = forge.op.eval.create_reshape_flatten_sparse_picker_matrix(1280, 40960).transpose(-1, -2)
    fspm = forge.Tensor.create_from_torch(fspm)
    mm = forge.op.SparseMatmul("", fspm, vs)
    tx = forge.op.Transpose("", mm, -2, -1)
    
    res = forge.op.Narrow("", tx, -2, 0, 1, 32)
    assert torch.all(in1.value().reshape(newshape) == res.value())
    mod = PyTorchModule("axelnet_reshape", AlexnetReshape(newshape, input_shape))
...
def test_concat():
    ...
    in1 = forge.Tensor.create_from_torch(torch.rand(input_shape_1))
    in2 = forge.Tensor.create_from_torch(torch.rand(input_shape_2))
    concat = forge.op.Concatenate("", in1, in2, axis=axis)

    if TILE_DIM == 32:
        padx1 = forge.op.PadTile("", in1, -1, input_shape_1[-1])
        pady1 = forge.op.PadTile("", padx1, -2, input_shape_1[-2])
        padx2 = forge.op.PadTile("", in2, -1, input_shape_2[-1])
        pady2 = forge.op.PadTile("", padx2, -2, input_shape_2[-2])
    else:
        pad1 = [0, 0, 0, 0]
        ...
        pady2 = forge.op.Pad("", in2, pad2)
    
    if axis == -1:
        pady1 = forge.op.Transpose("", pady1, -2, -1)

    if axis == -1:
        pady2 = forge.op.Transpose("", pady2, -2, -1)
    ...
    spm1 = forge.Tensor.create_from_torch(spm1)
    mm1 = forge.op.SparseMatmul("", spm1, vs1)
    ...
    spm2 = forge.Tensor.create_from_torch(spm2)
    mm2 = forge.op.SparseMatmul("", spm2, vs2)

    add = forge.op.Add("", mm1, mm2)
    vsl = add
    if axis == -3:
        rt = r_new//pady1.shape[-2]
        vsl = forge.op.HSlice("", add, rt)
    if axis == -1:
        vsl = forge.op.Transpose("", vsl, -2, -1)

    narrow_shape = input_shape_1[-1] if axis != -1 else input_shape_1[-1] + input_shape_2[-1]
    if narrow_shape != vsl.shape[-1]:
        vsl = forge.op.Narrow("", vsl, -1, 0, narrow_shape, vsl.shape[-1])
    narrow_shape = input_shape_1[-2] if axis != -2 else input_shape_1[-2] + input_shape_2[-2]
    if narrow_shape != vsl.shape[-2]:
        vsl = forge.op.Narrow("", vsl, -2, 0, narrow_shape, vsl.shape[-2])
...
def test_vslice(input_shape):
    ...
    result = forge.Tensor.create_from_torch(x)
    result = forge.op.PadTile("", result, -1, result.shape[-1])
    result = forge.op.PadTile("", result, -2, result.shape[-2])
    ...
    spm = forge.Tensor.create_from_torch(spm)
    result = forge.op.SparseMatmul("", spm, result)
    result = forge.op.VSlice("", result, slice_factor)

    assert align_up_tile(result.shape[-2]) == align_up_tile(attr[-2])
    result = forge.op.Narrow("", result, -1, 0, attr[-1], result.shape[-1])
    result = forge.op.Narrow("", result, -2, 0, attr[-2], result.shape[-2])
...
def test_vstack(input_shape):
    ...
    result = forge.op.VStack("", result, slice_factor)
    ...
    if orig_shape[-2] % TILE_DIM != 0:
        ...
        s_pick_multi_row = forge.Tensor.create_from_torch(s_pick_multi_row)
        result = forge.op.SparseMatmul("", s_pick_multi_row, result)
...
def test_full_flatten(input_shape): 
    ...
    if len(input_shape) > 2 and input_shape[-3] != 1:
        result = forge.op.VStack("", pady, input_shape[-3])
        if input_shape[-2] % TILE_DIM:
            ...
            spm = forge.Tensor.create_from_torch(spm)
            mm = forge.op.SparseMatmul("", spm, result)
            result = mm
    ...
    vslc = forge.op.VSlice("", result, rt)
    hstk = forge.op.HStack("", vslc, rt)
    result = hstk

    t = forge.op.Transpose("", hstk, -2, -1)
    ...
    spm = forge.Tensor.create_from_torch(spm)
    mm = forge.op.SparseMatmul("", spm, t)
    result = forge.op.Transpose("", mm, -2, -1)

    assert align_up_tile(newshape[-1]) == align_up_tile(result.shape[-1])
    assert align_up_tile(newshape[-2]) == align_up_tile(result.shape[-2])
    
    result = forge.op.Narrow("", result, -2, 0, newshape[-2], result.shape[-2])
    result = forge.op.Narrow("", result, -1, 0, newshape[-1], result.shape[-1])
...
def test_single_select(test_device, index, length):
    ...
    class Model(ForgeModule):
        ...
        def __init__(self, inp_shape):
            ...
            x = forge.Tensor.create_from_torch(torch.randn((1, 128, 8 ,8)))
            self.y = forge.op.tm.Select("", x, -3, (index, length), stride).shape[-3]
            
            self.add_constant("one")
            self.add_constant("two")
            self.add_parameter("three", forge.Parameter(torch.randn(128, 128)))
            self.add_parameter("four", forge.Parameter(torch.randn(self.y, self.y)))
        ...
        def forward(self, x):
            # x = (1, 1, 32, 64)
            x = forge.op.Matmul("", self.get_parameter("three"), x)
            x = forge.op.Reshape("", x, (1, 128, 8, 8))
            x = forge.op.Multiply("", x, self.get_constant("one"))
            x = forge.op.Add("", x, self.get_constant("two"))
            x = forge.op.Select("", x, -3, (index, length), stride)
            x = forge.op.Reshape("", x, (1, 1, self.y, 64))
            x = forge.op.Matmul("", self.get_parameter("four"), x)
            return x
            
    model = Model(input_shape)
    
    model(forge.Tensor.create_from_torch(torch.randn(input_shape)))
...
def test_multi_select(test_device):
    ...
    class Model(ForgeModule):
        ...
        def forward(self, x):
            ...
            r0 = forge.op.Reshape("", x, (1, 30, 4, 768))
            mult0 = forge.op.Multiply("", r0, self.get_constant("one"))
            add0 = forge.op.Add("", mult0, self.get_constant("two"))
            ...
            s1 = forge.op.Select("", add0, -3, (0, 10), 30)
            s2 = forge.op.Select("", add0, -3, (10, 10), 30)
            s3 = forge.op.Select("", add0, -3, (20, 10), 30)
            r1 = forge.op.Reshape("", s1, (1, 1, 10, 3072))
            r2 = forge.op.Reshape("", s2, (1, 1, 10, 3072))
            r3 = forge.op.Reshape("", s3, (1, 1, 10, 3072))
            m1 = forge.op.Softmax("", r1, dim=-1)
            m2 = forge.op.Softmax("", r2, dim=-1)
            m3 = forge.op.Softmax("", r3, dim=-1)
            ...
            s4 = forge.op.Select("", add0, -3, (0, 10), 30)
            s5 = forge.op.Select("", add0, -3, (10, 10), 30)
            s6 = forge.op.Select("", add0, -3, (20, 10), 30)
            r4 = forge.op.Reshape("", s4, (1, 10, 16, 192))
            r5 = forge.op.Reshape("", s5, (1, 10, 16, 192))
            r6 = forge.op.Reshape("", s6, (1, 10, 16, 192))
            m4 = forge.op.Softmax("", r4, dim=-1)
            m5 = forge.op.Softmax("", r5, dim=-1)
            m6 = forge.op.Softmax("", r6, dim=-1)
            
            # Multiple selects where the dim we are selecting from IS affected by commute
            # The reshapes are NOT inverses to r0. An inverse reshape shall be placed on top of each one
            # and subsequently erased during the optimize pass
            s7 = forge.op.Select("", add0, -1, (0, 256), 768)
            s8 = forge.op.Select("", add0, -1, (256, 256), 768)
            s9 = forge.op.Select("", add0, -1, (512, 256), 768)
            r7 = forge.op.Reshape("", s7, (1, 30, 16, 64))
            r8 = forge.op.Reshape("", s8, (1, 30, 16, 64))
            r9 = forge.op.Reshape("", s9, (1, 30, 16, 64))
            m7 = forge.op.Softmax("", r7, dim=-1)
            m8 = forge.op.Softmax("", r8, dim=-1)
            m9 = forge.op.Softmax("", r9, dim=-1)
            
            # Multiple selects where the dim we are selecting from IS affected by commute
            # The reshapes are all inverses to r0, should be erased
            s10 = forge.op.Select("", add0, -1, (0, 256), 768)
            s11 = forge.op.Select("", add0, -1, (256, 256), 768)
            s12 = forge.op.Select("", add0, -1, (512, 256), 768)
            r10 = forge.op.Reshape("", s10, (1, 1, 30, 1024))
            r11 = forge.op.Reshape("", s11, (1, 1, 30, 1024))
            r12 = forge.op.Reshape("", s12, (1, 1, 30, 1024))
            m10 = forge.op.Softmax("", r10, dim=-1)
            m11 = forge.op.Softmax("", r11, dim=-1)
            m12 = forge.op.Softmax("", r12, dim=-1)
...
def test_binary_stack_on_x():
    ...
    class Model(ForgeModule):
        def forward(self, x, y):
            # import pdb; pdb.set_trace()
            out1 = forge.op.BinaryStack("", x, y, -1)
...
def test_general_reshape(begin_end_shape):
    class ReshapeModel(ForgeModule):
        ...
        def forward(self, a):
            a = forge.op.Softmax("", a, dim=-1)
            a = forge.op.Reshape("", a, self.newshape)
            a = forge.op.Matmul("", a, self.get_constant("const"))
            return a
...
def test_pixel_shuffle(shape):
    ...
    class PixelShuffleModel(ForgeModule):
        def __init__(self):
            super().__init__("pixel_shuffle")
                    
        def forward(self, a):
            return forge.op.PixelShuffle("", a, r)
...
def test_reshape_with_smm_padding():
    class ReshapeModelWithPadding(ForgeModule):
        def __init__(self, name, orig_shape, newshape):
            ...
            self.add_constant("const")
            self.set_constant("const", forge.Tensor.create_from_torch(spm, constant=True))
            ...
            self.set_constant("const2", forge.Tensor.create_from_torch(spm, constant=True))
            self.newshape = newshape

        def forward(self, x):
            x = forge.op.SparseMatmul("smm0", self.get_constant("const") ,x)
            x = forge.op.Reshape("r0", x, self.newshape)
            x = forge.op.SparseMatmul("smm1", self.get_constant("const2"), x)
            return x

    import os
    os.environ["FORGE_PAD_SPARSE_MM"] = "{14:16}"
    ...
    del os.environ["FORGE_PAD_SPARSE_MM"]
