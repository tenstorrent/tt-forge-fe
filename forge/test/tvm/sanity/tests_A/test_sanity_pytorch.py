...
def test_tvm_upsample2d_channel_last(test_kind, test_device, input_shapes, scale_factors, upsample_mode, align_corners):
    ...
    if align_corners and upsample_mode != "bilinear":
            pytest.skip()
    class Upsample2d(ForgeModule):
        def __init__(self, name):
            super().__init__(name)

        def forward(self, input):
            if upsample_mode == "nearest_neighbor":
                return forge.op.Resize2d("", input, sizes=scale_factors, method=upsample_mode, channel_last=1) 
            else:
                return forge.op.Resize2d("", input, sizes=scale_factors, method=upsample_mode, align_corners=align_corners, channel_last=1) 
...
def test_tvm_conv1d(training, recompute, in_channel, out_channel, feature_size, bias, kernel_size, padding):
    ...
    forge_model_results = forge_compile(
        tt0,
        "Conv1D",
        *inputs,
        compiler_cfg=CompilerConfig(
    ...
    evaluate_framework_vs_forge(framework_model, forge_model_results, *inputs)
...
def test_tvm_nonzero():
    ...
    evaluate_framework_vs_forge(framework_model, forge_model_results, reference_input)
...
class TTLinear1(ForgeModule):
    def __init__(self, name):
        super().__init__(name)
        self.weights1 = forge.Parameter(64, 128, requires_grad=True)

    def forward(self, act1):
        return forge.op.Matmul("", act1, self.weights1)

class TTLinear2(ForgeModule):
    def __init__(self, name):
        super().__init__(name)
        self.weights2 = forge.Parameter(128, 64, requires_grad=True)

    def forward(self, act1):
        return forge.op.Matmul("", act1, self.weights2)
...
def test_tvm_tensordot(training, recompute, tensordot_inputs):
    ...
    except forge._C.UnsupportedHWOpsError:
        pass
...
def test_tvm_torch_flip(test_kind, test_device, input_shape, dim):
    ...
    model.eval()
    tt_model = forge.PyTorchModule("flip_tvm_decompose_adv_index", model)
    verify_module(
        tt_model,
        (input_shape,),
        verify_cfg=VerifyConfig(
...
def test_tvm_layernorm_cpu(test_device):
    ...
    forge_module = PyTorchModule("pt_layermorm_cpu", framework_module)
...
def test_tvm_dropout_cpu(test_device):
    ...
    framework_module = Module()
    framework_module.eval()
    forge_module = PyTorchModule("pt_dropout_cpu", framework_module)
...
def test_tvm_adv_index_bool_cpu_0(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_adv_index_bool_cpu_0", framework_module)
...
def test_tvm_adv_index_bool_cpu_1(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_adv_index_bool_cpu_1", framework_module)
...
def test_tvm_adv_index_bool_cpu_2(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_adv_index_bool_cpu_2", framework_module)
...
def test_tvm_hslice_a(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_hslice_a", framework_module)
...
def test_tvm_hslice_c(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_hslice_c", framework_module)
...
def test_tvm_non_hslice(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_hslice_d", framework_module)
...
def test_tvm_splice_with_scalar_concat(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_splice_with_scalar_concat", framework_module)
...
def test_tvm_concat_decomp_inti_smm(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_concat_decomp_smm", framework_module)
...
def test_yz_transpose(test_kind, test_device, tile_aligned):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_yz_transpose", framework_module)
...
def test_tvm_embedding(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_embedding", framework_module)
...
def test_tvm_splice_with_tm_vslice_1(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_splice_with_tm_vslice_1", framework_module)
...
def test_tvm_splice_with_tm_hslice(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_splice_with_tm_hslice", framework_module)
...
def test_tvm_splice_with_tm_hstack(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_splice_with_tm_hstack", framework_module)
...
def test_kernel_fracturing_with_grouped_conv(test_kind, test_device):
    pytest.skip("tenstorrent/forge#455")
    ...
    if test_device.is_wormhole_b0():
        os.environ["FORGE_EXTRA_L1_MARGIN"] = "60000"
...
def test_BN_no_stats(test_kind, test_device):
    ...
    forge_module = PyTorchModule("pt_BN_no_stats", framework_module)
...
def test_prelu_pytorch(test_kind, test_device):
    ...
    framework_module = Module()
    forge_module = PyTorchModule("pt_prelu", framework_module)
...
def test_override_removal_flag(test_kind, test_device):
    ...
    # Setup override veto
    os.environ["FORGE_OVERRIDES_VETO"] = "1"
    ...
    # Load Forge module
    forge_module = PyTorchModule("pt_override_removal", Module())
    ...
    del os.environ["FORGE_OVERRIDES_VETO"]
...
def test_torch_conv3d(test_device):
    ...
    forge_module = PyTorchModule("pt_conv3d", Module())
...
def test_torch_maxpool3d(test_device):
    ...
    forge_module = PyTorchModule("pt_maxpool3d", Module())
...
def test_reflection_pad(test_device):
    ...
    forge_module = PyTorchModule("reflection_pad", Module())
