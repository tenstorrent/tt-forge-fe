...
def evaluate_framework_vs_forge(
    framework_model, forge_model_results, *inputs, rtol=None, atol=None
):
    """
    Evaluates PyTorch model results agains compiled Forge model results.

    Args:
        framework_model: Framework model.
        forge_model_results (CompileResults): Forge model results.
    ...
    assert compare_tensor_to_golden("tvm", golden=pytorch_res, calculated=forge_model_results.outputs[0].value(), rtol=rtol, atol=atol)

    logger.debug("Tensors match on output of framework and forge")
