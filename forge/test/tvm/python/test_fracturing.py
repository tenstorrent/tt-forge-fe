...
def test_attn_fracture_matmul_heads(test_kind, test_device):
    ...
    module = forge.PyTorchModule('attn_matmul', mod)

    forge.set_configuration_options(default_df_override=forge.DataFormat.Float32,
    ...
    '''
    factor = 2
    forge.config.insert_fracture_group([("matmul_7", -3, factor),])
...
def test_attn_cache_loopback(test_kind, test_device):
    ...
    module = forge.PyTorchModule('attn_loopback_fractured', mod)

    df = forge.DataFormat.Float16
    forge.set_configuration_options(default_df_override=df,
        accumulate_df=df)
    ...
    # '''
    factor = 2
    forge.config.insert_fracture_group([("concatenate_5", -3, factor)])

    tt0 = forge.TTDevice("tt0", devtype=test_device.devtype, arch=test_device.arch, module=module)

    compile_inputs = (torch.randn(1, 1, d), torch.randn(1, s, d), torch.randn(1, 32, d))
    output_q = forge.initialize_pipeline(training=False, sample_inputs=compile_inputs,)
...
def test_attn_module_cache_loopback(test_kind, test_device):
    ...
    module = forge.PyTorchModule('attn_module_loopback_fractured', mod)
