...
@pytest.mark.parametrize("model_name", [x for x in tensorflow_model_name_to_forge_model.keys()])
def test_real_networks(mode, enable_tvm_constant_prop, model_name):
    if mode == "inference" and model_name in passing_tensorflow_model_name_to_forge_model_inference:
        pytest.skip()

    if (mode == "training" or mode == "recompute") and model_name in passing_tensorflow_model_name_to_forge_model_training:
        pytest.skip()
    ...
    while (http_tries < max_http_tries):
        try:
            model_config = tensorflow_model_name_to_forge_model[model_name](training, recompute)
            break
