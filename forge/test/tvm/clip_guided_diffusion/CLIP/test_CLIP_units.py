...
from forge import TTDevice, VerifyConfig, forge_compile, Tensor
...
def test_tvm_clip_text_embedding(training=False, recompute=False):
    ...
    sgd_optimizer = forge.optimizers.SGD(learning_rate=0.5, device_params=True)
    ...
    evaluate_framework_vs_forge(model, ret, input_text)
...
def test_tvm_clip_rand(training=False, recompute=False):
    ...
    evaluate_framework_vs_forge(model, ret, )
