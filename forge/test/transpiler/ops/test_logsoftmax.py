...
class TestLogSoftmax:
    ...
    def test_logsoftmax_basic(self, opset_version, input_shape, dtype):
        ...
        # Transpile
        # Disable debug mode for FLOAT16 with opsets 13, 14, 21, 23 and higher dimensional tensors (2D+)
        # due to numerical precision differences. Debug mode uses stricter tolerances than needed for these cases.
        use_debug = not (
            dtype == onnx.TensorProto.FLOAT16 and opset_version in [13, 14, 21, 23] and len(input_shape) >= 2
        )
        transpiler = ONNXToForgeTranspiler(debug=use_debug, validate_model=True)
        ...
        assert len(logsoftmax_nodes) == 1, (
            f"Expected 1 LogSoftmaxNode, got {len(logsoftmax_nodes)}. " f"Nodes: {[n.op_type for n in tir_graph.nodes]}"
        )

        logsoftmax_node = logsoftmax_nodes[0]
        assert (
            len(logsoftmax_node.inputs) == 1
        ), f"LogSoftmaxNode should have exactly 1 input, got {len(logsoftmax_node.inputs)}: {logsoftmax_node.input_names}"
        assert (
            logsoftmax_node.input_names[0] == "input_0"
        ), f"LogSoftmaxNode input should be 'input_0', got {logsoftmax_node.input_names[0]}"
        # Check original output name (before sanitization)
        assert (
            logsoftmax_node.original_outputs[0] == "output_0"
        ), f"LogSoftmaxNode output should be 'output_0', got {logsoftmax_node.original_outputs[0]}"

        # Verify axis attribute
        expected_axis = default_axis
        actual_axis = logsoftmax_node.attrs.get("dim", None)
        assert actual_axis == expected_axis, f"LogSoftmaxNode axis should be {expected_axis}, got {actual_axis}"
        ...
        if dtype == onnx.TensorProto.FLOAT16:
            if len(input_shape) == 1:
                ...
                # 2D tensors with FLOAT16 need more lenient tolerance
                rtol_val, atol_val = 1e-2, 1e-2
            elif len(input_shape) == 3:
                # 3D tensors with FLOAT16 need more lenient tolerance
                rtol_val, atol_val = 1e-2, 1e-2
    ...
    def test_logsoftmax_different_axes(self, opset_version, axis):
        ...
        logsoftmax_node = logsoftmax_nodes[0]
        assert (
            logsoftmax_node.attrs.get("dim") == axis
