...
class TestTanh:
    ...
    def test_tanh_basic(self, opset_version, input_shape, dtype):
        ...
        assert len(tanh_nodes) == 1, (
            f"Expected 1 TanhNode, got {len(tanh_nodes)}. " f"Nodes: {[n.op_type for n in tir_graph.nodes]}"
        )

        tanh_node = tanh_nodes[0]
        assert (
            len(tanh_node.inputs) == 1
        ), f"TanhNode should have exactly 1 input, got {len(tanh_node.inputs)}: {tanh_node.input_names}"
        assert (
            tanh_node.input_names[0] == "input_0"
        ), f"TanhNode input should be 'input_0', got {tanh_node.input_names[0]}"
        assert (
            tanh_node.original_outputs[0] == "output_0"
        ), f"TanhNode output should be 'output_0', got {tanh_node.original_outputs[0]}"
    ...
    def test_tanh_symmetric(self, opset_version):
        ...
        comparison_pos = compare_tir_with_onnx(tir_graph, onnx_model, {"input_0": positive_input}, rtol=1e-6, atol=1e-6)

        # Test with negative input
        comparison_neg = compare_tir_with_onnx(tir_graph, onnx_model, {"input_0": negative_input}, rtol=1e-6, atol=1e-6)

        assert len(comparison_pos["errors"]) == 0, f"Comparison errors (positive): {comparison_pos['errors']}"
        assert len(comparison_neg["errors"]) == 0, f"Comparison errors (negative): {comparison_neg['errors']}"
