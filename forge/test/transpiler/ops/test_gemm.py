...
def _create_gemm_model_v6_v7(
    opset_version, shape_a, shape_b, shape_c, alpha=1.0, beta=1.0, transA=0, transB=0, dtype=onnx.TensorProto.FLOAT
):
    """
...
def _create_gemm_model_v9_plus(
    opset_version, shape_a, shape_b, shape_c=None, alpha=1.0, beta=1.0, transA=0, transB=0, dtype=onnx.TensorProto.FLOAT
):
    """
...
class TestGemmScaling:
    ...
    def test_gemm_transpose_with_alpha_beta(self, opset, transA, transB):
        ...
        if opset < 9:
            model = _create_gemm_model_v6_v7(
                opset, shape_a, shape_b, shape_c, alpha=alpha, beta=beta, transA=transA, transB=transB
            )
        else:
            model = _create_gemm_model_v9_plus(
                opset, shape_a, shape_b, shape_c, alpha=alpha, beta=beta, transA=transA, transB=transB
            )
