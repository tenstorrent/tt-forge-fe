...
class TestTranspose:
    ...
    @pytest.mark.parametrize(
        "input_shape, perm, expected_shape",
        [
            # 2D transposes
    ...
    def test_transpose_default_perm(self, opset_version, input_shape, dtype):
        ...
        logger.info(
            f"✓ Transpose default perm test passed: opset={opset_version}, " f"input_shape={input_shape}, dtype={dtype}"
        )
    ...
    def test_transpose_node_indices(self, opset_version):
        ...
        for node in transpose_nodes:
            ...
            assert dim0 is not None and dim1 is not None, f"TransposeNode should have dim0 and dim1 attributes"
            # Should use positive indices (normalized)
            assert (
                isinstance(dim0, int) and dim0 >= 0
            ), f"dim0 should be non-negative int, got {dim0} (type: {type(dim0)})"
            assert (
                isinstance(dim1, int) and dim1 >= 0
            ), f"dim1 should be non-negative int, got {dim1} (type: {type(dim1)})"
            assert dim0 < len(input_shape), f"dim0 should be < {len(input_shape)}, got {dim0}"
            assert dim1 < len(input_shape), f"dim1 should be < {len(input_shape)}, got {dim1}"

        logger.info(f"✓ Transpose node indices test passed: opset={opset_version}")

    @pytest.mark.parametrize("opset_version", [1, 13, 21, 23, 24, 25])
    @pytest.mark.parametrize(
        "input_shape, perm, num_swaps_expected",
        [
    ...
    def test_transpose_intermediate_outputs(self, opset_version):
        ...
        assert len(transpose_nodes) == 2, (
            f"Expected 2 Transpose nodes, got {len(transpose_nodes)}. " f"Nodes: {[n.op_type for n in tir_graph.nodes]}"
        )
        ...
        for node in transpose_nodes:
            if node.output_names[0] in intermediate_outputs:
                # Check intermediate output tensor info
                if len(node.output_tensors) > 0:
                    intermediate_info = node.output_tensors[0]
                    # Shape should be (4, 3, 2) after first swap
                    assert intermediate_info.shape == (
                        4,
                        3,
                        2,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                (1, 2, 3, 4, 5, 6, 7),
                [6, 5, 4, 3, 2, 1, 0],
            ),  # 7D reverse (3 swaps) - fixed: 7D input with 7-element perm
            # Complex rotations
