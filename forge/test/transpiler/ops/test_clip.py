...
class TestClip:
    ...
    @pytest.mark.parametrize(
        "input_shape, min_val, max_val, dtype",
        [
            # Basic clipping
    ...
    def test_clip_basic(self, opset_version, input_shape, min_val, max_val, dtype):
        ...
        if opset_version < 12 and dtype in [
        ...
        if opset_version >= 11 and min_val is None and max_val is not None:
            pytest.skip(
                f"Test creates invalid ONNX model for 'only max' case in opset {opset_version} "
        ...
        if min_val is None and max_val is None:
            if opset_version >= 11:
                ...
                assert (
                    "min" in clip_node.attrs and "max" in clip_node.attrs
            ...
            clip_nodes = [n for n in tir_graph.nodes if n.op_type == "Clip"]
            assert len(clip_nodes) == 1, (
                f"Expected 1 ClipNode, got {len(clip_nodes)}. " f"Nodes: {[n.op_type for n in tir_graph.nodes]}"
            )
            ...
            assert (
                len(clip_node.inputs) == 1
            ), f"ClipNode should have exactly 1 input (data), got {len(clip_node.inputs)}: {clip_node.input_names}"
            assert (
                clip_node.input_names[0] == "input_0"
            ), f"ClipNode input should be 'input_0', got {clip_node.input_names[0]}"

            # min/max should be in attrs (extracted from input tensors in v11+)
            if min_val is not None:
                assert "min" in clip_node.attrs, f"ClipNode {clip_node.name} missing 'min' attribute"
                assert (
                    abs(clip_node.attrs["min"] - float(min_val)) < 1e-6
                ), f"ClipNode min mismatch: expected {min_val}, got {clip_node.attrs['min']}"
            if max_val is not None:
                assert "max" in clip_node.attrs, f"ClipNode {clip_node.name} missing 'max' attribute"
                assert (
                    abs(clip_node.attrs["max"] - float(max_val)) < 1e-6
    ...
    def test_clip_min_greater_than_max(self, opset_version):
        ...
        # Create test input
        input_data = {"input_0": np.random.randn(*input_shape).astype(np.float32) * 10}
    ...
    def test_clip_extreme_values(self):
        ...
        # Create test input
        input_data = {"input_0": np.random.randn(*input_shape).astype(np.float32) * 1000}
    ...
    def test_clip_integer_types(self, opset_version, dtype, min_val, max_val):
        ...
        if dtype in [onnx.TensorProto.INT8]:
            ...
            input_data = {"input_0": np.random.randint(safe_min, safe_max, size=input_shape, dtype=np_dtype)}
            ...
            input_data = {"input_0": np.random.randint(min_val - 50, max_val + 50, size=input_shape, dtype=np_dtype)}
    ...
    def test_clip_zero_tensor(self):
        ...
        if tir_output is not None:
            assert np.allclose(
                tir_output, 0.0, rtol=1e-5, atol=1e-6
