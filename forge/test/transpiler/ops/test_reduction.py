...
class TestReduceSum:
    ...
    def test_reducesum_comprehensive(self, opset_version, input_shape, dtype, axes, keepdims):
        ...
        if dtype in [onnx.TensorProto.INT32, onnx.TensorProto.INT64]:
            ...
            input_data = {"input_0": np.random.randint(1, 10, size=input_shape, dtype=np_dtype)}
    ...
    def test_reducesum_edge_cases(self, opset_version):
        ...
        for input_shape, axes, keepdims, description in edge_cases:
            ...
            assert len(comparison["errors"]) == 0, f"Comparison errors for {description}: {comparison['errors']}"
            assert all(comparison["matches"].values()), f"Output mismatch for {description}: {comparison}"
    ...
    def test_reducesum_dim_none_keepdim_true(self):
        ...
        assert tir_output.shape == expected_shape, f"Expected output shape {expected_shape}, got {tir_output.shape}"
        ...
        assert (
            torch_output.shape == expected_shape
    ...
    def test_reducesum_opset13_basic(self, input_shape, axes, keepdims):
        ...
        logger.info(f"✓ Opset 13 test passed: shape={input_shape}, axes={axes}, keepdims={keepdims}")
    ...
    def test_reducesum_opset13_noop_with_empty_axes_true(self, input_shape):
        ...
        attrs = {"keepdims": True, "noop_with_empty_axes": 1}  # True
        ...
        assert tir_output.shape == input_shape, f"Expected output shape {input_shape}, got {tir_output.shape}"
        assert np.allclose(tir_output, onnx_output, rtol=1e-5, atol=1e-6), "Output should equal input for no-op case"

        logger.info(f"✓ Opset 13 noop_with_empty_axes=True test passed: shape={input_shape}")
    ...
    def test_reducesum_opset13_noop_with_empty_axes_false(self, input_shape):
        ...
        attrs = {"keepdims": keepdims, "noop_with_empty_axes": 0}  # False (default)
        ...
        logger.info(f"✓ Opset 13 noop_with_empty_axes=False test passed: shape={input_shape}")
    ...
    def test_reducesum_opset13_axes_as_input_tensor(self, input_shape, axes_input, keepdims):
        ...
        logger.info(
            f"✓ Opset 13 axes as input test passed: shape={input_shape}, " f"axes={axes_input}, keepdims={keepdims}"
        )
...
class TestReduceMax:
    ...
    def test_reducemax_comprehensive(self, opset_version, input_shape, dtype, axes, keepdims):
        ...
        # Verify structure
        assert len(tir_graph.nodes) >= 1, f"Expected at least 1 node, got {len(tir_graph.nodes)}"
        assert tir_graph.nodes[0].op_type == "ReduceMax", f"Expected ReduceMax node, got {tir_graph.nodes[0].op_type}"
    ...
    def test_reducemax_rank_zero(self, opset_version):
        ...
        assert np.allclose(tir_output, input_data["input_0"], rtol=1e-5, atol=1e-6), "Scalar output should equal input"
    ...
    def test_reducemax_boolean_false_less_than_true(self, opset_version):
        ...
        input_data = {
            "input_0": np.array(
                [[False, True, False, True], [False, False, False, False], [True, True, True, False]], dtype=np.bool_
            )
        }
        ...
        assert np.array_equal(tir_output, expected_output), f"Expected {expected_output}, got {tir_output}"
        assert np.array_equal(
            onnx_output, expected_output
    ...
    def test_reducemax_opset18_noop_with_empty_axes(self, opset_version):
        ...
        assert tir_output.shape == input_shape, f"Expected output shape {input_shape}, got {tir_output.shape}"
        assert np.allclose(
            tir_output, input_data["input_0"], rtol=1e-5, atol=1e-6
        ), "Output should equal input for no-op case"

        logger.info(f"✓ ReduceMax noop_with_empty_axes test passed: opset={opset_version}")
...
class TestReduceMean:
    ...
    def test_reducemean_opset18_noop_with_empty_axes(self, opset_version):
        ...
        logger.info(f"✓ ReduceMean noop_with_empty_axes test passed: opset={opset_version}")
