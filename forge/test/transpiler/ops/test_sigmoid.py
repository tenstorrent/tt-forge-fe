...
class TestSigmoid:
    ...
    def test_sigmoid_basic(self, opset_version, input_shape, dtype):
        ...
        assert len(sigmoid_nodes) == 1, (
            f"Expected 1 SigmoidNode, got {len(sigmoid_nodes)}. " f"Nodes: {[n.op_type for n in tir_graph.nodes]}"
        )

        sigmoid_node = sigmoid_nodes[0]
        assert (
            len(sigmoid_node.inputs) == 1
        ), f"SigmoidNode should have exactly 1 input, got {len(sigmoid_node.inputs)}: {sigmoid_node.input_names}"
        assert (
            sigmoid_node.input_names[0] == "input_0"
        ), f"SigmoidNode input should be 'input_0', got {sigmoid_node.input_names[0]}"
        # Check original output name (before sanitization)
        assert (
            sigmoid_node.original_outputs[0] == "output_0"
        ), f"SigmoidNode output should be 'output_0', got {sigmoid_node.original_outputs[0]}"
