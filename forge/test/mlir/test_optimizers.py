...
def test_sgd_with_param_groups(shape):
    ...
    tt_optimizer = forge.optimizers.SGD(learning_rate=1, parameters=tt_param_groups)
    golden_optimizer = torch.optim.SGD(golden_param_groups, lr=1)
...
def test_adam_with_param_groups(shape):
    ...
    # Create optimizers with parameter groups
    tt_optimizer = forge.optimizers.Adam(learning_rate=0.005, bias_correction=True, parameters=tt_param_groups)
