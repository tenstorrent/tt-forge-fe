...
def test_adadelta(shape, rho, eps):
    torch.manual_seed(0)
    num_epochs = 10
    batch_size = 64
    ...
    tt_optimizer = forge.optimizers.Adadelta(rho=rho, epsilon=eps)
    golden_optimizer = torch.optim.Adadelta(golden_model.parameters(), rho=rho, eps=eps)
