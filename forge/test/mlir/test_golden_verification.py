...
def test_matmul_and_add(batch_size, outer_dim_x, outer_dim_y, inner_dim):
    class MatmulAdd(nn.Module):
        def __init__(self):
            super().__init__()
            self.const_1 = torch.rand(batch_size, outer_dim_x, outer_dim_y)
            self.const_2 = torch.rand(batch_size, outer_dim_x, outer_dim_y)

        def forward(self, x, y, z):
            mm = torch.matmul(x, y)
            add_1 = self.const_1 + self.const_2
            mm_add = mm + add_1
            return mm_add * z
