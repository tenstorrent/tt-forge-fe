...
def test_llama_prefill_on_cpu_decode_on_tt_no_cache():
    ...
    prompt = "Q: What is the largest animal?\nA:"
    inputs = tokenizer(prompt, return_tensors="pt", max_length=max_sequence_length, pad_to_max_length=True, truncation=True)
...
def test_llama_prefill_on_cpu_decode_on_tt_cache():
    ...
    class LlamaModelWrapper(torch.nn.Module):
        ...
        and past key values from the previous iteration and return logits and past key values as List of torch.Tensor
        Args:
        ...
        Returns:
            outputs (`List[torch.Tensor]`) - Logits of shape (batch_size, 1, vocab_size) will be present in the 0th index of the outputs and
            rest of the list contains past key values tensor of shape (batch_size, num_of_key_values_heads, key_values_seq_len + 1, head_dim)
        """
        ...
        def forward(self, input_id, past_key_values):
            ...
            # the past key values from List(List(Key1, Values1), ... , List(Key26, Values26)) to
            # List(Key1, Values1, ... , Key26, Values26) for comparing the Framework and TT output in similar fashion.
            outputs = [model_outputs.logits]
            for k, v in model_outputs.past_key_values:
                outputs.append(k)
                outputs.append(v)
            return outputs
    ...
    for _ in range(max_new_tokens):
        ...
        # CPU Inference
        framework_output = llama_model(model_inputs[0], model_inputs[1])
...
def test_llama_prefill_on_cpu_decode_on_tt_cache_padding():
    ...
    if apply_padding_on_past_key_values:
        ...
        # Pad past key values in key_value_seq_len(i.e -2) dimension
        ...
        for idx, (k, v) in enumerate(model_inputs[1]):
            model_inputs[1][idx][0] = torch.cat([k, torch.zeros(k.shape[-4], k.shape[-3], max_new_tokens, k.shape[-1]).to(k.dtype)], dim = -2)
            model_inputs[1][idx][1] = torch.cat([v, torch.zeros(v.shape[-4], v.shape[-3], max_new_tokens, v.shape[-1]).to(k.dtype)], dim = -2)
    ...
    for max_new_tokens_idx in range(max_new_tokens):
        ...
        if apply_padding_on_past_key_values:
            ...
            for idx in range(len(model_inputs[1])):
                ...
                model_inputs[1][idx][0][:, :, non_padding_seq_len + max_new_tokens_idx, :] = model_inputs[1][idx][0][:, :, -1, :]
                model_inputs[1][idx][0] = model_inputs[1][idx][0][:, :, :-1, :]
                model_inputs[1][idx][1][:, :, non_padding_seq_len + max_new_tokens_idx, :] = model_inputs[1][idx][1][:, :, -1, :]
