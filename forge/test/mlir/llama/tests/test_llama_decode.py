...
def unflatten_pastkv(past_key_values_list: List[torch.Tensor]) -> List[List[torch.Tensor]]:
    ...
    Note:
        If the length of `past_key_values_list` is not even, a warning is logged and an empty list is returned.
    """
    if len(past_key_values_list) % 2 == 0:
        ...
        logger.warning("past_key_values_list length should be a multiple of two.")
        return None
...
def test_llama_prefill_on_cpu_decode_on_tt_no_cache(model_path, run_on_tt_device):
    ...
    for idx in range(max_new_tokens):
        ...
        if run_on_tt_device:
            ...
            verify(tt_inputs, framework_model, compiled_model)
