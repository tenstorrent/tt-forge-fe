...
class BudaPadTMsTest1(ForgeModule):
    ...
    def forward(self, act):
        ...
        pad = forge.op.BudaPad("buda_pad", tr, self.paddings, self.value)

        return pad

class BudaPadTMsTest2(ForgeModule):
    ...
    def forward(self, act1, act2):
        ...
        pad1 = forge.op.BudaPad("buda_pad1", hsl1, self.paddings, self.value)

        # Flow of the second input
        hsl2 = forge.op.HSlice("hsl2", act2, self.hfactor)
        vsl2 = forge.op.VSlice("vsl2", hsl2, self.vfactor)
        pad2 = forge.op.BudaPad("buda_pad2", vsl2, self.paddings, self.value)
...
class BudaUnpadTMsTest1(ForgeModule):
    ...
    def forward(self, act):
        ...
        unpad = forge.op.BudaUnpad("buda_unpad", act, self.original_length, self.paddings)
...
class BudaUnpadTMsTest2(ForgeModule):
...
class PaddingTMsTest1(ForgeModule):
    ...
    def forward(self, act1, act2):
        ...
        # Flow of the first input
            # Padding
        pad1 = forge.op.BudaPad("pad1", act1, self.paddings, self.value)
        exp = forge.op.Exp("exp", pad1)
        unpad1 = forge.op.BudaUnpad("unpad1", exp, self.original_length, self.paddings)
            # TMs after padding
        vsl1 = forge.op.VSlice("vsl1", unpad1, self.vfactor)
            # Padding before joining the two flows
        pad3 = forge.op.BudaPad("pad3", vsl1, self.paddings, self.value)

        # Flow of the second input
            # Padding
        pad2 = forge.op.BudaPad("pad2", act2, self.paddings, self.value)
        relu = forge.op.Relu("relu", pad2)
        unpad2 = forge.op.BudaUnpad("unpad2", relu, self.original_length, self.paddings)
            # TMs after padding
        vsl2 = forge.op.VSlice("vsl2", unpad2, self.vfactor)
            # Padding before joining the two flows
        pad4 = forge.op.BudaPad("pad4", vsl2, self.paddings, self.value)
        ...
        unpad3 = forge.op.BudaUnpad("unpad3", add, original_length_v, self.paddings)
            # TMs after unpadding
...
class PaddingTMsTest2(ForgeModule):
    ...
    def forward(self, act1, act2):
        ...
        original_length_flow1[-1] //= self.vfactor
            ...
            # Padding
        pad1 = forge.op.BudaPad("pad1", tr1, self.paddings, self.value)
        exp = forge.op.Exp("exp", pad1)
        unpad1 = forge.op.BudaUnpad("unpad1", exp, original_length_flow1, self.paddings)
            # TMs after padding
        ...
        original_length_flow2[-1] *= self.hfactor
            ...
            # Padding
        pad2 = forge.op.BudaPad("pad2", vsl2, self.paddings, self.value)
        relu = forge.op.Relu("relu", pad2)
        unpad2 = forge.op.BudaUnpad("unpad2", relu, original_length_flow2, self.paddings)
            # TMs after padding
        ...
        original_length_flow_merged[-1] //= self.vfactor
            ...
            # Padding before joining the two flows
        pad3 = forge.op.BudaPad("pad3", tr3, self.paddings, self.value)
        pad4 = forge.op.BudaPad("pad4", tr4, self.paddings, self.value)
        add = forge.op.Add("add", pad3, pad4)
            # Unpad after joining the two flows
        unpad3 = forge.op.BudaUnpad("unpad3", add, original_length_flow_merged, self.paddings)
            # TMs after unpadding
