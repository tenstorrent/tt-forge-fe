...
class TestPaddingPassB(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        relu4 = forge.op.Relu("relu4", conv7)

        # ... head flow 1 & 2 ...
        mul1 = forge.op.Multiply("mul1", conv2, conv4)
        add1 = forge.op.Add("add1", relu2, conv6)
        mul2 = forge.op.Multiply("mul2", relu3, relu4)
        ...
        # block
        # ... block flow 1 ...
        add2 = forge.op.Add("add2", maxpool1, maxpool2)
        relu5 = forge.op.Relu("relu5", add2)
        conv8 = self.conv8(relu5)
        relu6 = forge.op.Relu("relu6", conv8)
        conv9 = self.conv9(relu6)
        relu7 = forge.op.Relu("relu7", conv9)
        conv10 = self.conv10(relu7)
        add5 = forge.op.Add("add5", relu5, conv10)

        # ... block flow 2 ...
        add3 = forge.op.Add("add3", maxpool3, maxpool4)
        mul3 = forge.op.Multiply("mul3", relu5, add3)
        exp1 = forge.op.Exp("exp1", mul3)

        conv11 = self.conv11(add3)
        relu8 = forge.op.Relu("relu8", conv11)
        exp2 = forge.op.Exp("exp2", relu8)
        conv12 = self.conv12(exp2)
        conv13 = self.conv13(conv12)
        exp3 = forge.op.Exp("exp3", conv13)
        add6 = forge.op.Add("add6", exp1, exp3)

        # ... block flow 3 ...
        add4 = forge.op.Add("add4", maxpool4, maxpool5)
        conv14 = self.conv14(add4)
        relu9 = forge.op.Relu("relu9", conv14)
        conv15 = self.conv15(relu9)
        exp4 = forge.op.Exp("exp4", conv15)
        conv16 = self.conv16(exp4)
        mul4 = forge.op.Multiply("mul4", add4, conv16)

        # ... block flow 4 ...
        mm1 = forge.op.Matmul("mm1", add5, add6)  # maybe add operation, add7
        relu10 = forge.op.Relu("relu10", mm1)
        conv17 = self.conv17(relu10)
        relu11 = forge.op.Relu("relu11", conv17)
        conv18 = self.conv18(relu11)
        relu12 = forge.op.Relu("relu12", conv18)
        conv19 = self.conv19(relu12)
        add7 = forge.op.Add("add7", relu10, conv19)

        # ... block flow 5 ...
        relu13 = forge.op.Relu("relu13", mul4)
        conv20 = self.conv20(relu13)
        relu14 = forge.op.Relu("relu14", conv20)
        conv21 = self.conv21(relu14)
        relu15 = forge.op.Relu("relu15", conv21)
        conv22 = self.conv22(relu15)
        mm2 = forge.op.Matmul("mm2", relu10, relu13)
        add8 = forge.op.Add("add8", mm2, conv22)

        # tail
        # ... tail flow 1 ...
        W1, Z1, R1, C1 = 1, 1, add7.shape[-3], add7.shape[-1] * add7.shape[-2]
        resh1 = forge.op.Reshape("resh1", add7, (W1, Z1, R1, C1))
        ...
        resh2 = forge.op.Reshape("resh2", add8, (W2, Z2, R2, C2))
...
class TestPaddingPassB_1(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        # Layer 3
        add = forge.op.Add("add", maxpool1, maxpool2)

        return add


class TestPaddingPassB_2(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        # Layer 3
        add = forge.op.Add("add", conv1, conv2)
        mul = forge.op.Multiply("mul", conv1, conv2)
        ...
        # Layer 5
        sub = forge.op.Subtract("sub", maxpool1, maxpool2)

        return sub

class TestPaddingPassB_3(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        # Layer 2
        sub = forge.op.Subtract("sub", x1, x2)
        # Layer 3
        conv = self.conv1(sub)
        # Layer 4
        add = forge.op.Add("add", conv, conv)
        # Layer 5
        maxpool = self.maxpool1(add)
        # Layer 6
        exp = forge.op.Exp("exp", maxpool)
