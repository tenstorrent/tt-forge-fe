...
class TestPaddingPassH(forge.ForgeModule):
    ...
    def forward(self, x1, x2, x3):
        ...
        # Layer 3
        mul1 = forge.op.Multiply("mul1", mm1, x2)
        mul2 = forge.op.Multiply("mul2", self.train_param1, mm2)
        mul3 = forge.op.Multiply("mul3", mm2, self.train_param3)
        mul4 = forge.op.Multiply("mul4", self.train_param2, mm3)

        # Layer 4
        mm4 = forge.op.Matmul("mm4", mul1, mul2)
        mm5 = forge.op.Matmul("mm5", mul3, mul4)

        # Layer 5
        mul5 = forge.op.Multiply("mul5", mm4, mm2)
        mul6 = forge.op.Multiply("mul6", mm2, mm5)

        # Layer 6
        mm6 = forge.op.Matmul("mm6", mul1, mul5)
        mm7 = forge.op.Matmul("mm7", mul6, mul4)
        add = forge.op.Add("add", mul5, mul6)

        # Layer 7
        mul7 = forge.op.Multiply("mul7", mm6, add)
        mul8 = forge.op.Multiply("mul8", add, mm7)

        return mul7, mul8


class TestPaddingPassH_1(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        mm1 = forge.op.Matmul("mm1", self.train_param1, x2)

        # Layer 3
        mm2 = forge.op.Matmul("mm2", mul1, mm1)
        mm3 = forge.op.Matmul("mm3", mm1, mul2)

        # Layer 4
        mm4 = forge.op.Matmul("mm4", mm2, mm3)
        
        return mm4


class TestPaddingPassH_2(forge.ForgeModule):
    ...
    def forward(self, x):
        ...
        # Layer 2
        mm = forge.op.Matmul("mm", x, self.train_param)

        # Layer 3
        exp = forge.op.Exp("exp", mm)

        return exp


class TestPaddingPassH_3(forge.ForgeModule):
    ...
    def forward(self, x):
        ...
        # Layer 2
        mm1 = forge.op.Matmul("mm1", x, self.train_param)
        mm2 = forge.op.Matmul("mm2", x, self.train_param)
        mm3 = forge.op.Matmul("mm3", x, self.train_param)

        # Layer 3
        mm4 = forge.op.Matmul("mm4", mm1, mm2)
        mm5 = forge.op.Matmul("mm5", mm2, mm3)

        # Layer 4
        mul1 = forge.op.Multiply("mul1", mm1, mm4)
        mul2 = forge.op.Multiply("mul2", mm5, mm3)

        # Layer 5
        mm6 = forge.op.Matmul("mm6", mul1, mul2)
...
def set_environment():
    ...
    if TEST_H_CHIP_PLACEMENT_SELF_CUT_TYPE_FLAG:
        ...
        if TEST_H_CHIP_PLACEMENT_SELF_CUT_TYPE_FLAG_PRODUCER:
            os.environ["FORGE_GRAPHSOLVER_SELF_CUT_TYPE"] = "ProducerUserDataEdgesFirst"
...
@pytest.mark.xfail(reason="tenstorrent/forge#1004#note_216172")
