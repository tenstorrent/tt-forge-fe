...
class TestPaddingPassG(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        add3 = forge.op.Add("add3", mul2, mul3)

        # Layer 4
        mul4 = forge.op.Multiply("mul4", add1, add2)
        mul5 = forge.op.Multiply("mul5", add1, add2)
        mul6 = forge.op.Multiply("mul6", add2, add3)

        # Layer 5
        exp1 = forge.op.Exp("exp1", mul4)
        exp2 = forge.op.Exp("exp2", mul5)
        exp3 = forge.op.Exp("exp3", mul6)

        # Layer 6
        relu1 = forge.op.Relu("relu1", exp1)
        relu2 = forge.op.Relu("relu2", exp2)
        relu3 = forge.op.Relu("relu3", exp3)

        # Layer 7
        exp4 = forge.op.Exp("exp4", relu1)
        exp5 = forge.op.Exp("exp5", relu2)
        exp6 = forge.op.Exp("exp6", relu3)

        return exp4, exp5, exp6


class TestPaddingPassG_1(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        # Layer 3
        add = forge.op.Add("add", mul1, mul2)

        # Layer 4
        exp = forge.op.Exp("exp", add)

        return exp

class TestPaddingPassG_2(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        # Layer 4
        mul2 = forge.op.Multiply("mul2", mul1, self.train_param2)
        # Layer 5
        recip = forge.op.Reciprocal("recip", mul2)

        return recip


class TestPaddingPassG_3(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        add2 = forge.op.Add("add2", self.train_param1, self.train_param2)

        # Layer 3
        mul = forge.op.Multiply("mul", add1, add2)
