...
class TestPaddingPassD(forge.ForgeModule):
    ...
    def forward(self, x1, x2, x3):
        ...
        conv1 = self.conv1(x1)
        conv2 = self.conv2(x1)
        mm1 = forge.op.Matmul("mm1", conv1, conv2) 

        # ... head flow 2 ...
        conv3 = self.conv3(x2)
        conv4 = self.conv4(x2)
        mm2 = forge.op.Matmul("mm2", conv3, conv4)

        # ... head flow 3 ...
        conv5 = self.conv5(x3)
        conv6 = self.conv6(x3)
        mm3 = forge.op.Matmul("mm3", conv5, conv6)

        # block
        # ... block flow 1 ...
        maxpool1 = self.maxpool1(mm1)
        conv7 = self.conv7(maxpool1)
        exp1 = forge.op.Exp("exp1", conv7)
        conv8 = self.conv8(exp1)
        exp2 = forge.op.Exp("exp2", conv8)
        conv9 = self.conv9(exp2)
        add13 = forge.op.Add("add13", maxpool1, conv9)

        # ... block flow 2 ...
        maxpool2 = self.maxpool2(mm2)
        conv12 = self.conv12(maxpool2)
        exp3 = forge.op.Exp("exp3", conv12)
        conv13 = self.conv13(exp3)
        exp4 = forge.op.Exp("exp4", conv13)
        conv14 = self.conv14(exp4)
        add4 = forge.op.Add("add4", maxpool2, conv14)

        # ... block flow 3 ...
        maxpool3 = self.maxpool3(mm3)
        conv17 = self.conv17(maxpool3)
        exp5 = forge.op.Exp("exp5", conv17)
        conv18 = self.conv18(exp5)
        exp6 = forge.op.Exp("exp6", conv18)
        conv19 = self.conv19(exp6)
        add6 = forge.op.Add("add6", maxpool3, conv19)

        # ... block flow 1 & 2 ...
        mul1 = forge.op.Multiply("mul1", conv7, conv12)
        sm1 = forge.op.Softmax("sm1", mul1, dim=-2, stable=True)
        conv10 = self.conv10(sm1)
        add2_ = forge.op.Add("add2_", conv8, conv13)
        add2 = forge.op.Add("add2", add2_, conv10)
        conv11 = self.conv11(add2)
        mul2_ = forge.op.Multiply("mul2_", conv9, conv14)
        mul2 = forge.op.Multiply("mul2", mul2_, conv11)

        # ... block flow 2 & 3 ...
        add1 = forge.op.Add("add1", conv12, conv17)
        sm2 = forge.op.Softmax("sm2", add1, dim=-2, stable=True)
        conv15 = self.conv15(sm2)
        add3_ = forge.op.Add("add3_", conv15, conv18)
        add3 = forge.op.Add("add3", add3_, conv13)
        conv16 = self.conv16(add3)
        add5_ = forge.op.Add("add5_", conv14, conv16)
        add5 = forge.op.Add("add5", add5_, conv19)

        # ... block flow 4 ...
        add7 = forge.op.Add("add7", add13, mul2)
        conv20 = self.conv20(add7)
        relu1 = forge.op.Relu("relu1", conv20)
        conv21 = self.conv21(relu1)
        relu2 = forge.op.Relu("relu2", conv21)
        conv22 = self.conv22(relu2)
        add10 = forge.op.Add("add10", add7, conv22)
        exp7 = forge.op.Exp("exp7", add10)
        red1 = forge.op.ReduceSum("red1", exp7, dim=-2)

        # ... block flow 5 ...
        add8 = forge.op.Add("add8", add4, add5)
        conv23 = self.conv23(add8)
        relu3 = forge.op.Relu("relu3", conv23)
        conv24 = self.conv24(relu3)
        relu4 = forge.op.Relu("relu4", conv24)
        conv25 = self.conv25(relu4)
        add11 = forge.op.Add("add11", add8, conv25)
        exp8 = forge.op.Exp("exp8", add11)
        red2 = forge.op.ReduceSum("red2", exp8, dim=-2)

        # ... block flow 6 ...
        add9 = forge.op.Add("add9", add5, add6)
        conv26 = self.conv26(add9)
        relu5 = forge.op.Relu("relu5", conv26)
        conv27 = self.conv27(relu5)
        relu6 = forge.op.Relu("relu6", conv27)
        conv28 = self.conv28(relu6)
        add12 = forge.op.Add("add12", add9, conv28)
        exp9 = forge.op.Exp("exp9", add12)
        red3 = forge.op.ReduceSum("red3", exp9, dim=-2)

        # tail
        # ... tail flow 1 ...
        add15 = forge.op.Add("add15", red1, red2)
        W1, Z1, R1, C1 = 1, 1, add15.shape[-3], add15.shape[-1] * add15.shape[-2]
        resh1 = forge.op.Reshape("resh1", add15, (W1, Z1, R1, C1))
        tr1 = forge.op.Transpose("tr1", resh1, -2, -1)
        ra1 = forge.op.ReduceAvg("ra1", tr1, -2)
        lin1 = self.linear1(ra1)
        sm3 = forge.op.Softmax("sm3", lin1, dim=-1, stable=True)

        # ... tail flow 2 ...
        add14 = forge.op.Add("add14", red2, red3)
        W2, Z2, R2, C2 = 1, 1, add14.shape[-3], add14.shape[-1] * add14.shape[-2]
        resh2 = forge.op.Reshape("resh2", add14, (W2, Z2, R2, C2))
        tr2 = forge.op.Transpose("tr2", resh2, -1, -2)
        ra2 = forge.op.ReduceAvg("ra2", tr2, -2)
        lin2 = self.linear2(ra2)
        sm4 = forge.op.Softmax("sm4", lin2, dim=-1, stable=True)
