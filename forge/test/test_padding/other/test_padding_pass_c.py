...
class TestPaddingPassC(forge.ForgeModule):
    ...
    def forward(self, x1, x2):
        ...
        conv2 = self.conv2(x1)
        conv3 = self.conv3(x1)
        exp1 = forge.op.Exp("exp1", conv1)
        exp2 = forge.op.Exp("exp2", conv2)
        exp3 = forge.op.Exp("exp3", conv3)

        # ... head flow 2 ...
        conv4 = self.conv4(x2)
        conv5 = self.conv5(x2)
        conv6 = self.conv6(x2)
        exp4 = forge.op.Exp("exp4", conv4)
        exp5 = forge.op.Exp("exp5", conv5)
        exp6 = forge.op.Exp("exp6", conv6)

        # block
        # ... block flow 1 ...
        mm1 = forge.op.Matmul("mm1", exp1, exp4)
        conv7 = self.conv7(mm1)
        relu1 = forge.op.Relu("relu1", conv7)
        conv8 = self.conv8(relu1)
        relu2 = forge.op.Relu("relu2", conv8)
        conv9 = self.conv9(relu2)
        add1 = forge.op.Add("add1", mm1, conv9)

        # ... block flow 2 ...
        mm2 = forge.op.Matmul("mm2", exp2, exp6)
        conv10 = self.conv10(mm2)
        relu3 = forge.op.Relu("relu3", conv10)
        conv11 = self.conv11(relu3)
        exp8 = forge.op.Exp("exp8", conv11)
        conv12 = self.conv12(exp8)
        mul1 = forge.op.Multiply("mul1", mm2, conv12)

        # ... block flow 3 ...
        mm3 = forge.op.Matmul("mm3", exp3, exp5)
        conv13 = self.conv13(mm3)
        exp7 = forge.op.Exp("exp7", conv13)
        conv14 = self.conv14(exp7)
        exp9 = forge.op.Exp("exp9", conv14)
        conv15 = self.conv15(exp9)
        add2 = forge.op.Add("add2", mm3, conv15)
        ...
        # ... intermediate flow 1, 2 & 3 ...
        mm4 = forge.op.Matmul("mm4", maxpool1, maxpool2)
        mm5 = forge.op.Matmul("mm5", maxpool2, maxpool3)
        mm6 = forge.op.Matmul("mm6", maxpool1, maxpool3)
        exp10 = forge.op.Exp("exp10", mm4)
        exp11 = forge.op.Exp("exp11", mm6)
        ...
        mul2 = forge.op.Multiply("mul2", conv16, conv17)
        add3 = forge.op.Add("add3", conv18, conv19)
        add4 = forge.op.Add("add4", conv20, conv22)
        mul3 = forge.op.Multiply("mul3", conv21, conv23)
        ...
        relu5 = forge.op.Relu("relu5", conv25)
        conv26 = self.conv26(relu5)
        add5 = forge.op.Add("add5", maxpool4, conv26)
        ...
        resh2 = forge.op.Reshape("resh2", maxpool9, (W2, Z2, R2, C2))
        ...
        # block
        # ... block flow 8 ...
        add9 = forge.op.Add("add9", sm1, sm2)
        conv36 = self.conv36(add9)
        relu12 = forge.op.Relu("relu12", conv36)
        conv37 = self.conv37(relu12)
        add10 = forge.op.Add("add10", add9, conv37)
