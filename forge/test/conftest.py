    ...
    def get(self, key: str) -> Any:
        ...
        if len(parts) == 1:
            if key in self.forge_properties:
                ...
                logger.info(f"There is no '{key}' in ForgePropertyStore")
                return None
        ...
        for part in parts:
            if isinstance(d, dict) and part in d:
                ...
                logger.info(f"Key '{key}' not found in ForgePropertyStore")
                return None
...
def record_forge_property(record_property):
    ...
    forge_property = ForgePropertyStore()
    
    # Add a default property.
    forge_property.add("tags.frontend", "tt-forge-fe")
    logger.info("Test Execution Start")
    
    # Provide the store to the test function.
    yield forge_property
    ...
    if execution_phase is not None:
        forge_property.add("tags.execution_phase", execution_phase)
    if execution_stage is not None:
        forge_property.add("tags.execution_stage", execution_stage)

    # After the test, record each top-level property.
    for key, value in forge_property.items():
