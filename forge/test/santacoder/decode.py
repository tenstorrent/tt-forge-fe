...
def test_santacoder(tokens, device, arch, precision, amp_level, num_chips, fuse):
    forge.config.set_configuration_options(default_df_override=forge.DataFormat.Float16)

    compiler_cfg = _get_global_compiler_config()
    compiler_cfg.amp_properties.append(forge._C.AMPNodeProperties(
        op_type="splice",
        output_df=forge._C.DataFormat.Float16,
        accumulate_df=forge._C.DataFormat.Float16,
        math_fidelity=forge.MathFidelity.HiFi3,
        intermediate_df=forge._C.DataFormat.Float16,
        input_df= {0: [forge._C.DataFormat.Float16, True], 1: [forge._C.DataFormat.Float16, True]}
    ))
    ...
    parameters = {
        ...
        'kv_cache': 'forge/test/santacoder/kv_cache.pt',
        'stop': '\n\n',
        'num_tokens': tokens,
        'output_at_end': False,
        'device': device,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--device', choices=['huggingface', 'pytorch', 'golden', 'silicon'], default='huggingface', help='huggingface: run using HF code only, pytorch: use our shim but run in PyTorch, golden/silicon: run via forge')
