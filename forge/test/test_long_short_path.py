...
def test_intra_epoch_relay_queue(mode, microbatch_size):
    def linked_list(activations):
        activations = forge.op.Buffer(f"buffer_pre", activations)
        # num_entries=microbatch_size, so if the queue is statically allocated, it still has enough memory
        activations = forge.op.DRAMQueue(f"buffering_queue", activations, num_entries=microbatch_size)
        activations = forge.op.Buffer(f"buffer_post", activations)
        return activations
...
def test_sanity(mode):
    ...
    parameters = {
        "weights1": forge.Parameter(
            torch.rand(*shape, requires_grad=True)
        ),
        "weights2": forge.Parameter(
            torch.rand(*shape, requires_grad=True)
        ),
    }

    @compile(
        compiler_cfg=forge.CompilerConfig(enable_training=training),
        verify_cfg=forge.VerifyConfig(),
    )
    def test(act1, *, weights1, weights2):
        m1 = forge.op.Matmul("matmul1", act1, weights1)
        m2 = forge.op.Matmul("matmul2", act1, weights2)
...
def test_two_branch_fork_join_branch_asymmetry(
    ...
    def two_branch_fork_join_branch_asymmetry(act1):
        ...
        for i in range(num_ops_left_branch):
            left_branch = forge.op.Buffer(f"buffer_left_{i}", left_branch)

        for i in range(num_ops_right_branch):
            right_branch = forge.op.Buffer(f"buffer_right_{i}", right_branch)

        return forge.op.Add("add", left_branch, right_branch)
...
def test_two_branch_fork_join_branch_asymmetry_with_buffering_queue(
    ...
    def two_branch_fork_join_branch_asymmetry(act1):
        ...
        # num_entries=microbatch_size, so if the queue is statically allocated, it still has enough memory
        left_branch = forge.op.DRAMQueue(f"buffering_queue", left_branch, num_entries=microbatch_size)
...
def test_three_branch_fork_join_branch_asymmetry(
    ...
    def three_branch_fork_join_branch_asymmetry(act1):
        ...
        for i in range(num_ops_first_branch):
            first_branch = forge.op.Buffer(f"branch0_buffer_{i}", first_branch)
        ...
        for i in range(num_ops_third_branch):
            third_branch = forge.op.Buffer(f"branch2_buffer_{i}", third_branch)

        partial_sum = forge.op.Add("partial_add", first_branch, second_branch)
        sum = forge.op.Add("final_add", partial_sum, third_branch)
        return sum
