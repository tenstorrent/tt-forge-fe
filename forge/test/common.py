...
import torch
import forge
from forge import is_silicon
from forge import ForgeModule, PyTorchModule, TTDevice, forge_compile, Parameter, BackendType
...
class ModuleBuilder(ForgeModule):
...
def compile(
    ...
    **compile_kwargs
    ...
    Special compile_kwargs:
        ...
        Everything else gets passed through to forge_compile
    """
...
def run(verify_cfg, module_cls=ModuleBuilder, num_inputs=1, **compile_kwargs):
    ...
    if isinstance(verify_cfg, TestDevice):
        verify_cfg = forge.VerifyConfig(devtype=verify_cfg.devtype, arch=verify_cfg.arch, devmode=verify_cfg.devmode, tti_path=verify_cfg.tti_path)
