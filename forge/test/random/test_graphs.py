...
from typing import Generator, Tuple, List, Union, Type, Callable, Any
...
class RGGRunParams:
    ...
    def from_env(cls):
        ...
        random_test_selected = os.environ.get("RANDOM_TESTS_SELECTED", "")
        random_test_selected = [int(i) for i in random_test_selected.strip().split(",") if i]
        random_test_seed = int(os.environ.get("RANDOM_TEST_SEED", 0))
        frameworks = os.environ.get("FRAMEWORKS", "").strip().split(",")
        configs = os.environ.get("CONFIGS", "DEFAULT").strip().split(",")
        algorithms = os.environ.get("ALGORITHMS", "RANDOM").strip().split(",")
        return cls(
