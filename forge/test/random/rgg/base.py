...
from .datatypes import ModelBuilder, Framework, Algorithm
...
class RandomizerCodeGenerator:
    def __init__(self, template_dir: str):
        self.template = Environment(
            loader=FileSystemLoader(template_dir), autoescape=select_autoescape(["html", "xml", "jinja2"])
        ).get_template("generated_model.jinja2")
...
class RandomizerModelProviderFromSourceCode:
    ...
    def _get_model_class_from_code(self, class_name: str, class_string: str) -> Type:
        ...
        namespace = {}
        exec(class_string, namespace)

        GeneratedTestModel = namespace[class_name]
...
class RandomizerRunner:
    ...
    def build_model(self) -> ForgeModule:
        model = self.model_provider.build_model(self.test_context)
        return model

    def verify(self, model: ForgeModule) -> None:
    ...
    def verify_model(self, model: ForgeModule) -> None:
        """
        Verify the model by building it and performing validation via Forge.
        The method is usually implemented once per framework.

        Args:
            test_context (RandomizerTestContext): The context for the randomizer test.
            model (ForgeModule): The Forge model to verify.
...
def process_test(
    ...
    Args:
        ...
        algorithm (Algorithm): The algorithm for the test.
    ...
    graph_builder = algorithm.GraphBuilderType(framework, randomizer_config)
