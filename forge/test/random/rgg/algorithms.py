...
from .datatypes import Framework
...
class RandomGraphAlgorithm(GraphBuilder):
    ...
    def build_graph(self, test_context: RandomizerTestContext):
        ...
        for node_index in range(num_of_nodes, 0, -1):
            ...
            # Logging graph building progress
            duration = math.floor(build_duration.get_duration())

            # Log building progress every n seconds
            if duration - last_build_duration >= self.buliding_progress_rate_in_sec:
                last_build_duration = duration
                logger.debug(f"Building node {num_of_nodes-node_index}/{num_of_nodes} in {duration} sec")
...
class Algorithms(Enum):
    ...
    RANDOM = Algorithm(name="Random", GraphBuilderType=RandomGraphAlgorithm)
