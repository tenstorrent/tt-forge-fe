...
from forge import ForgeModule, PyTorchModule, TTDevice, Tensor, Parameter, forge_compile, CompilerConfig, SGD 
...
class ExpModule(forge.ForgeModule):
    def __init__(self, name):
        super().__init__(name)
            
    def forward(self, act1):  
        return forge.op.Exp("exp", act1)

 
class TwoOpsModule(forge.ForgeModule):
    def __init__(self, name, r, c):
        super().__init__(name)
        self.weights1 = forge.Parameter(torch.rand(r*32, c*32), requires_grad=True)
            
    def forward(self, act1, act2):
        e1 = forge.op.Exp("exp", act1)
        m1 = forge.op.Matmul("matmul", e1, self.weights1)  
        t1 = forge.op.Transpose("transpose", m1, 1,2)
        a1 = forge.op.Add("add", t1, act2)
        return a1
 
class ThreeOpsModule(forge.ForgeModule):
    def __init__(self, name, r, c):
        super().__init__(name)
        self.weights1 = forge.Parameter(torch.rand(r*32, c*32), requires_grad=True)
            
    def forward(self, act1, act2, act3):
        e1 = forge.op.Exp("exp1", act1) 
        a1 = forge.op.Add("add1", e1, act2) 
        e2 = forge.op.Exp("exp2", act3) 
        c1 = forge.op.Concatenate("concat", a1,a1, axis=1) 
        m1 = forge.op.Matmul("matmul", e2, self.weights1)   
        a2 = forge.op.Add("add2", c1, m1)        
        return a2

class FourOpsModule(forge.ForgeModule):
    def __init__(self, name, r, c):
        super().__init__(name)
        self.weights1 = forge.Parameter(torch.rand(r*32, c*32), requires_grad=True)
    ...
    def forward(self, act1, act2, act3):
        e1 = forge.op.Exp("exp1", act1) 
        e2 = forge.op.Exp("exp2", act2) 
        a1 = forge.op.Add("add1", e1, e1)  
        c1 = forge.op.Concatenate("concat", a1, e2, axis=1) 
        m1 = forge.op.Matmul("matmul", c1, self.weights1)  
        a2 = forge.op.Add("add2", act3, m1)        
        return a2
  
class TwoOpsNoTModule(forge.ForgeModule):
    def __init__(self, name, r, c):
        super().__init__(name) 
        self.weights1 = forge.Parameter(torch.rand(r*32, c*32), requires_grad=True)
    ...
    def forward(self, act1, act2):
        e1 = forge.op.Exp("exp", act1) 
        m1 = forge.op.Matmul("matmul", e1, self.weights1)
        a1 = forge.op.Add("add", m1, act2)
        return a1

class TwoOpsModulev2(forge.ForgeModule):
    def __init__(self, name):
        super().__init__(name) 
    ...
    def forward(self, act1, act2):
        e1 = forge.op.Exp("exp1", act1)
        e2 = forge.op.Exp("exp2", act2)
        m1 = forge.op.Matmul("matmul", e1, e2)   
        return m1
...
def test_manual_op_transpose(test_device, r, c):
    if (test_device.arch == forge.BackendDevice.Wormhole_B0 or test_device.arch == forge.BackendDevice.Blackhole) and (r > 8 or c > 8):
    ...
    forge.config.override_op_size("exp", (r, c))
    forge.config.override_op_placement("exp", transpose_op=True)
...
def test_auto_op_transpose_case1(test_device): 
    ...
    forge.config.override_op_size("exp", (1, 3))   
    forge.config.override_op_size("add", (2, 1)) 
    forge.set_configuration_options(enable_auto_transposing_placement=True)
...
def test_auto_op_transpose_case2(test_device):
    ...
    forge.config.override_op_size("exp", (10, 1))   
    forge.config.override_op_size("add", (10, 1)) 
    forge.set_configuration_options(enable_auto_transposing_placement=True)
...
def test_auto_op_transpose_case3(test_device): 
    ...
    forge.config.override_op_size("exp1", (2, 7))   
    forge.config.override_op_size("add1", (2, 7)) 
    forge.config.override_op_size("exp2", (4, 1))
    forge.set_configuration_options(enable_auto_transposing_placement=True)
...
def test_auto_op_transpose_multi_rows1(test_device): 
    ...
    forge.config.override_op_size("exp1", (4, 5))   
    forge.config.override_op_size("exp2", (1, 5))
    forge.config.override_op_size("add1", (4, 5)) 
    forge.config.override_op_size("add2", (5, 2))
    forge.set_configuration_options(enable_auto_transposing_placement=True)
...
def test_auto_op_transpose_multi_rows2(test_device): 
    ...
    forge.config.override_op_size("exp1", (3, 8))   
    forge.config.override_op_size("exp2", (8, 1))
    forge.set_configuration_options(enable_auto_transposing_placement=True)
