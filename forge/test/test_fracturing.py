...
def test_fracture_multichip(test_kind, test_device, dim, factor):
    if test_device.arch == forge.BackendDevice.Wormhole_B0 or test_device.arch == forge.BackendDevice.Blackhole:
...
def test_fracture_1d_weight_stationary(test_kind, test_device):
    ...
    def fracture_1d_weight_stationary(x, Win=None, Wout=None):
        x = forge.op.Matmul("e0", x, Win)
        x = forge.op.Gelu("gelu", x)
        x = forge.op.Matmul("e1", x, Wout)
        return x
    ...
    forge.config.insert_fracture_group([("Win", -1, 2), ("Wout", -2, 2)])
...
def test_fracture_2d_weight_stationary(test_kind, test_device):
    ...
    forge.config.insert_fracture_group([("Win", [-2, -1], [2, 4]), ("Wout", [-2, -1], [4, 3])])
...
def test_fracture_slice(test_kind, test_device):
    ...
    forge.config.insert_fracture_group([("e0", -3, 12), "gelu"])
...
def test_fracture_bcast(test_kind, test_device, config):
    pytest.skip("tenstorrent/forge#1903")
    if test_kind.is_training():
        pytest.skip()
    ...
    forge.config.insert_fracture_group([("Win",) + config, "gelu"])

    fracture_bcast(x, Win=Win, bias=bias)


def test_fracture_output(test_kind, test_device):
    os.environ["FORGE_CONCAT_ON_HOST"] = "1"
...
def test_fracture_fork_join(test_kind, test_device):
    ...
    def fracture_fork_join(x, y, Win=None):
        x = forge.op.Matmul("e0", x, Win)
        h = forge.op.HSlice("", x, 16)
        m = forge.op.Multiply("multiply", h, y)
        f0 = forge.op.Gelu("gelu0", m)
        f1 = forge.op.Gelu("gelu1", h)
        j = forge.op.Multiply("join", f0, f1)
        return j
    ...
    forge.config.insert_fracture_group([("e0", -2, 2), ("join", -2, 2)])
...
def test_fracture_fork_input(test_kind, test_device):
    ...
    def fracture_fork_input(x, Win=None):
        f0 = forge.op.Gelu("gelu0", x)
        f1 = forge.op.Gelu("gelu1", x)
        return f0, f1

    x = Tensor.create_from_torch(torch.rand((1, 1, 128, 128)))

    forge.config.insert_fracture_group([("gelu0", -2, 2)])
    forge.config.insert_fracture_group([("gelu1", -1, 2)])
...
def test_mixed_factors(test_kind, test_device, factor, fork):
    ...
    forge.config.insert_fracture_group([("gelu0", -1, factor[0]), ("gelu1", -1, factor[1])])
...
def test_fracture_transpose(test_device):
    ...
    def fracture_transpose(x, y=None):
        f0 = forge.op.Gelu("gelu0", x)
        f0 = forge.op.Transpose("t0", f0, -2, -1)
        return forge.op.Add("add0", f0, y)

    x = Tensor.create_from_torch(torch.rand((1, 1, 384, 384)))
    y = Parameter(torch.rand((1, 1, 384, 384)), name="y")

    forge.config.insert_fracture_group([("gelu0", -1, 2), "add0"])
