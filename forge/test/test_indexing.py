...
def test_index(
    ...
    def index(x):
        return forge.op.Index("index", x, dim, start, stop, stride)
...
def test_index2d(
    ...
    def index2d(x):
        x = forge.op.Index("index_r", x, 0, start0, stop0, stride0)
        x = forge.op.Index("index_c", x, 1, start1, stop1, stride1)
        return x
...
def test_index3d(
    ...
    if training:
        pytest.skip("tenstorrent/forge#184")
    ...
    def index3d(x):
        x = forge.op.Index("index_z", x, has_w + 0, start0, stop0, stride0)
        x = forge.op.Index("index_r", x, has_w + 1, start1, stop1, stride1)
        x = forge.op.Index("index_c", x, has_w + 2, start2, stop2, stride2)
        return x
