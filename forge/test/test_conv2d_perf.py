...
import pytest
import forge
from forge.op.nn import Conv2dModule, MaxPool2dModule
...
def test_single_conv(test_kind, test_device, kernel, stride, input_dim, input_c, output_c):
    ...
    forge.config.override_op_size("conv2d_perf.dc.buffer.12", (1, 9))
    #forge.config.override_op_size("conv2d_perf.dc.matmul.9", (8, 1))
    #forge.config.override_t_stream_shape("conv2d_perf.dc.matmul.9", (1, 1))
    forge.config.override_op_size("conv2d_perf.dc.sparse_matmul.18.lc2", (8, 1))
    forge.config.override_t_stream_dir("conv2d_perf.dc.sparse_matmul.18.lc2", "r")
    forge.config.override_t_stream_shape("conv2d_perf.dc.sparse_matmul.18.lc2", (16, 1))
    #forge.config.override_t_stream_shape("conv2d_perf.dc.sparse_matmul.18.lc2", (1, 1))
    forge.verify.verify_module(mod, [(16, input_c, input_dim, input_dim)],
            VerifyConfig(test_kind=test_kind, devtype=test_device.devtype, arch=test_device.arch, relative_atol=relative_atol, pcc=pcc, fp32_fallback=forge.DataFormat.Bfp8_b))
...
def test_single_maxpool(test_kind, test_device, kernel, stride, input_dim, input_c):
    ...
    relative_atol, pcc = get_relaxed_atol_pcc(test_kind, test_device)
    forge.config._get_global_compiler_config().performance_trace = forge.config.PerfTraceLevel.VERBOSE
    forge.config.override_op_size("maxpool_perf.dc.sparse_matmul.5.lc2", (6, 1))
    #forge.config.override_t_stream_shape("conv2d_perf.dc.matmul.9", (1, 1))
    #forge.config.override_op_size("conv2d_perf.dc.sparse_matmul.18.lc2", (8, 1))
    #forge.config.override_t_stream_dir("conv2d_perf.dc.sparse_matmul.18.lc2", "r")
    #forge.config.override_t_stream_shape("conv2d_perf.dc.sparse_matmul.18.lc2", (16, 1))
