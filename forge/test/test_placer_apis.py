...
class ForkJoinTest(ForgeModule):
    ...
    def forward(self, act):
        nop = forge.op.Buffer("nop", act)
        m1 = forge.op.Matmul("matmul1", nop, self.weights1)
        m2 = forge.op.Matmul("matmul2", nop, self.weights2)
        m1_nop = forge.op.Buffer("matmul1_nop", m1)
        m2_nop = forge.op.Buffer("matmul2_nop", m2)
        return forge.op.Add("add", m1_nop, m2_nop)
...
def test_override_chip_id(test_device):
    ...
    module = ModuleBuilder(
        ...
        ff1_weights=forge.Tensor.create_from_torch(torch.rand(shape)),
        ff2_weights=forge.Tensor.create_from_torch(torch.rand(shape))
    )
    tt0.place_module(module)

    # apply overrides
    forge.config.override_op_placement("ff1", chip_id=3)
    forge.config.override_op_placement("gelu", chip_id=1)
    forge.config.override_op_placement("ff2", chip_id=2)

    compile_result = forge_compile(tt0, "test_override_chip_id", act1)
    placer_solution = compile_result.pass_specific_output_kwargs["placer_solution"]
...
def test_change_start_grid_location(test_kind):
    ...
    # apply overrides
    forge.config.override_op_size("ff2", (2,2))   
    forge.config.override_op_placement("ff2", start=(3,3))

    compile_result = forge_compile(tt0, "test_change_start_grid_location", act1)
    placer_solution = compile_result.pass_specific_output_kwargs["placer_solution"]
...
def test_conflicting_placement_overrides(test_kind):
    ...
    forge.config.override_op_placement("gelu", start=[2,2])
    forge.config.override_op_placement("ff2", start=[2,2])

    module = ModuleBuilder(conflicting_placement_overrides, ff1_weights=forge.Parameter(1,1,64,64), ff2_weights=forge.Parameter(1,1,64,64))
...
def test_dram_allocator_api(test_device):
    ...
    def override_dram_allocator(x, weight=None):
        mm0 = forge.op.Matmul("mm0", x, weight)
        gelu = forge.op.Gelu("gelu", mm0)
        return gelu

    x = Tensor.create_from_torch(torch.randn(shape))
    w = forge.Parameter(torch.randn(shape))
    forge.config.set_epoch_break("gelu")
    forge.config.override_dram_queue_placement("e2e_mm0_0", chip_id=0)
...
def test_predicate_overrides(test_device, transpose_op, temporal_epoch_break):
    ...
    for layer in range(num_layers):
        forge.config.override_op_size(f"mm{layer}", (1, 2))

    @compile(chip_ids=chip_ids)
    def predicate_override(x, weight=None):
        out = x
        for layer in range(num_layers):
            out = forge.op.Matmul(f"mm{layer}", out, weight)
            out = forge.op.Exp(f"exp{layer}", out)
        return out
