...
#
from forge._C.balancer import OpOverride
...
from forge._C.graph import RuntimeTensorTransformType
from forge.config import CompileDepth, _get_global_compiler_config
from forge.op.eval import compare_tensor_to_golden, does_prestriding_improve_perf
from forge.op.nn import Conv2dModule, ConvTranspose2dModule, MaxPool2dModule, AvgPool2dModule
from forge.utils import align_up_tile, round_up_div
from .common import run
from .module_utils import Conv2dTModule
from forge.op.eval.sparse_utils import calculate_conv2d_output_dimensions, calculate_conv2d_transpose_output_dimensions, conv2d_padding_to_canonical, calculate_conv2d_prestride_weights_and_padding, can_conv2d_prestride
...
def test_conv2d(
    ...
    forge.config.set_configuration_options(enable_conv_prestride=False)
    try:
        forge.verify.verify_module(mod, [(1, in_channels, original_shape[0], original_shape[1])],
        ...
        if (
            ...
            pytest.xfail("tenstorrent/forge#185")
        raise
...
def test_conv2d_fractured(
    ...
    forge.config.override_fracture_factor("conv2d_fractured.conv.dc.sparse_matmul.9.dc.sparse_matmul.1.lc2", kernel_size[0])

    os.environ["FORGE_FORCE_ALLOW_FRACTURING"] = "1"
    # TODO: Figure out how to (cleanly) confirm that fracturing happened (need to get grid_shape of conv op)
    devices = forge.verify.verify_module(mod, [(1, in_channels, original_shape[0], original_shape[1])],
        VerifyConfig(test_kind=test_kind, devtype=test_device.devtype, arch=test_device.arch, relative_atol=relative_atol, pcc=pcc))
    del os.environ["FORGE_FORCE_ALLOW_FRACTURING"]
...
def test_conv2d_multi_op_fractured(
    ...
    forge.config.override_multi_op_fracture_factor("conv2d_multi_op_fractured.conv", kernel_size[0])
    ...
    os.environ["FORGE_FORCE_DISALLOW_FRACTURING"] = "1"  # Disables "within-op" fracturing
    ...
    del os.environ["FORGE_FORCE_DISALLOW_FRACTURING"]
...
def test_conv2d_prestrided(
    ...
    os.environ["FORGE_FORCE_DISALLOW_FRACTURING"] = "1"
...
def test_conv2d_resnet_prestrided_fractured(
    ...
    compiler_cfg = _get_global_compiler_config()
    compiler_cfg.balancer_policy = "MaximizeTMinimizeGrid"
    forge.config.override_fracture_factor("conv2d_resnet_prestrided_fractured.conv.dc.conv2d.3.dc.sparse_matmul.9.dc.sparse_matmul.1.lc2", 8)
...
def test_conv2d_fractured_multi_c(
    ...
    compiler_cfg.balancer_policy = "MaximizeTMinimizeGrid"
    compiler_cfg.enable_conv_prestride = False
    # forge.config.override_fracture_factor("conv2d_fractured_multi_c.conv.dc.sparse_matmul.9.dc.sparse_matmul.1.lc2", fracture_factor)
    # forge.config.override_op_size("?", (?, fracture_factor * 2))
...
def test_max_pool2d_stream_through_queue(test_device):
    ...
    dilation = 1
    ceil_mode = False
    forge.config.override_t_stream_shape("max_pool2d.dc.sparse_matmul.5.dc.sparse_matmul.1.lc2", (5, 1))
    forge.config.override_t_stream_shape("max_pool2d.dc.reduce_max.6", (1, 1))
...
def test_max_pool2d_stream_through_queue_1x1(test_device, producer_stream_factor):
    ...
    forge.config.override_t_stream_shape("max_pool2d.dc.sparse_matmul.5.dc.sparse_matmul.1.lc2", (producer_stream_factor, 1))
    forge.config.override_t_stream_shape("max_pool2d.dc.reduce_max.6", (2, 1))
...
def test_conv2d_vgg_head(test_device):
    compiler_cfg = _get_global_compiler_config()
    compiler_cfg.balancer_policy = "MaximizeTMinimizeGrid"
    forge.config.override_t_stream_shape("conv2d.dc.sparse_matmul.9.dc.sparse_matmul.1.lc2", (28, 1))
    return test_conv2d(
        TestKind.INFERENCE,
        test_device,
        3,
