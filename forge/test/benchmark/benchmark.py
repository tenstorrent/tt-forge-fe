...
def single_thread_generative_model_run(args, first_device, last_device, inputs, targets, output_q, num_tokens_to_generate, first_current_index, pad_token_id, write_index):
    ...
    for _ in range(num_tokens_to_generate):  
        if current_token_index == 0:
            ...
            forge.run_generate(input_count=args.loop_count, write_index=write_index)
            ans = output_q.get()
        else:
        ...
        if is_text_inputs:        
            ...
            if current_token_index % TILE_DIM == 0:
                ...
                forge.run_generate(input_count=0, write_index=0)
...
def single_thread_run(args, first_device, last_device, inputs, targets, output_q, num_tokens_to_generate):
    ...
    if num_tokens_to_generate:
        for _ in range(num_tokens_to_generate):
            ...
            forge.run_generate(input_count=args.loop_count, write_index=0)
            output_q.get()
    else:
        first_device.push_to_inputs(inputs)
...
def multi_thread_run(args, first_device, last_device, inputs, targets, output_q, num_tokens_to_generate):
    ...
    output = output_q if not args.training else forge.get_loss_queue()
    ...
    if args.training:
        ...
        for _ in range(args.loop_count//args.microbatch_count):
            forge.run_forward(input_count=args.microbatch_count)
            forge.run_backward(input_count=args.microbatch_count)
    else:
        if num_tokens_to_generate:
            for _ in range(num_tokens_to_generate):
                forge.run_generate(input_count=args.loop_count, write_index=0)
    ...
    output_thread.join()
    if args.training:
        forge.sync() # wait for the last backward to finish
...
def run(
        args,
        duts: Dict[str, Union[forge.PyTorchModule, forge.ForgeModule]],
    ...
    forge.config.set_configuration_options(default_df_override=df_from_str(args.dataformat), backend_runtime_params_path=args.runtime_params_yaml, device_config=args.device_config)
    ...
    last_device = None
    if "cpu-pre" in duts:
        assert isinstance(duts["cpu-pre"], forge.PyTorchModule)
        cpu_pre = forge.CPUDevice("cpu-pre", module=duts["cpu-pre"])
    ...
    assert "tt" in duts
    if args.save_tti:
        tt = forge.TTDevice("tt0", module=duts["tt"], fp32_fallback=df_from_str(args.dataformat), num_chips=args.chips, arch=arch, devtype=devtype)
    elif args.load_tti:
        img = forge.TTDeviceImage.load_from_disk(args.load_tti)
        img.info()
        tt = forge.TTDevice.load_image(img=img)
    elif args.galaxy:
        tt = forge.TTDevice("tt0", module=duts["tt"], arch=arch, devtype=devtype, fp32_fallback=df_from_str(args.dataformat), chip_ids=[0, 11, 10, 9, 8, 7, 19, 20, 21, 22, 23, 24, 6, 5, 14, 13, 12, 16, 15, 3, 4, 26, 25, 32, 31, 30, 29, 28, 27, 1, 2, 18, 17])
    else:
        tt = forge.TTDevice("tt0", module=duts["tt"], arch=arch, devtype=devtype, fp32_fallback=df_from_str(args.dataformat), num_chips=args.chips)
    ...
    cpu_post = None
    if "cpu-post" in duts:
        assert isinstance(duts["cpu-post"], forge.PyTorchModule)
        cpu_post = forge.CPUDevice("cpu-post", module=duts["cpu-post"])
        last_device = cpu_post

    if "cpu-loss" in duts:
        assert isinstance(duts["cpu-loss"], forge.PyTorchModule)
        if cpu_post is None: # no cpu-post module
            identity = forge.PyTorchModule("identity0", torch.nn.Identity())
            cpu_post = forge.CPUDevice("cpu-post", module=identity)

        cpu_post.place_loss_module(duts["cpu-loss"])
        last_device = cpu_post

    enable_auto_fusing = "FORGE_DISABLE_FUSE_OPS" not in os.environ
    if args.perf_analysis:
        if "FORGE_OP_PERF" not in os.environ:
            os.environ["FORGE_OP_PERF"] = "1"
    ...
    if args.chips == 0:
        args.chips = len(forge.detect_available_devices())
    if args.chips == 0:
    ...
    # the one being used with api calls like forge.run_forward(..). We'll fetch
    # the arch from the first device-type available
    device_list = forge.detect_available_devices()
    ...
    output_q = forge.initialize_pipeline(training=args.training, sample_inputs=compile_inputs, microbatch_count=args.microbatch_count, _verify_cfg=forge.VerifyConfig.disabled(), sample_targets=targets)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e',   '--env', default="", help='List of environment variable settings, i.e. "FORGE_OPT1=1 FORGE_OP2=1" to run with.')
    ...
    if args.model == "bert" and args.chips > 1:
        os.environ["FORGE_MULTICHIP_BERT"] = str(args.chips)
