...
def benchmark_model(configs=[]):
    def benchmark_decorator(model_func):
        ...
        def wrapper(*args, **kwargs):
            ...
            for m in ret[0]:
                assert isinstance(m, forge.Module), err_msg
...
def df_from_str(df: str) -> forge.DataFormat:
    if (df == "Fp32"):
...
def mf_from_str(mf: str) -> forge.MathFidelity:

    if (mf == "LoFi"):
        return forge.MathFidelity.LoFi

    if (mf == "HiFi2"):
        return forge.MathFidelity.HiFi2

    if (mf == "HiFi3"):
        return forge.MathFidelity.HiFi3

    if (mf == "HiFi4"):
        return forge.MathFidelity.HiFi4

    raise RuntimeError("Unknown math fidelity: " + mf)

def trace_from_str(trace: str) -> forge.PerfTraceLevel:

    if (trace == "none"):
        return forge.PerfTraceLevel.NONE

    if (trace == "light"):
        return forge.PerfTraceLevel.LIGHT

    if (trace == "verbose"):
        return forge.PerfTraceLevel.VERBOSE
