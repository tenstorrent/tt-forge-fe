...
def bert(training: bool, config: str, microbatch: int, devtype: str, arch: str, data_type: str, math_fidelity: str, force_num_layers: Optional[int] = None):
    ...
    if config == "tiny":
        ...
        if "FORGE_MODULAR_BERT" in os.environ:
            layers = force_num_layers if force_num_layers else 24
            for i in range(layers):
                forge.config._get_global_compiler_config().place_on_new_epoch(f"matmul_{2+53*i}")

        elif "FORGE_MULTICHIP_BERT" in os.environ:
            layers = force_num_layers if force_num_layers else 24
            chips = int(os.environ["FORGE_MULTICHIP_BERT"])
            chip_breaks = []
            for i in range(chips):
                chip_breaks.append(i*(math.ceil(layers/chips)))
            for i in range(layers):
                if i in chip_breaks:
                    forge.config._get_global_compiler_config().place_on_new_chip(f"matmul_{2+53*i}")
                else:
                    forge.config._get_global_compiler_config().place_on_new_epoch(f"matmul_{2+53*i}")
            ...
            for i in range(layers):
                if (i%2 == 1):
                    forge.config._get_global_compiler_config().place_on_new_epoch(f"matmul_{41+53*i}")
    elif config == "base_tc":
    ...
    if config == "large_tc":
        ...
        forge.config._get_global_compiler_config().cpu_fallback_ops.remove("embedding")
