...
import forge
from forge import PyTorchModule, OnnxModule
...
def resnet_quant(training: bool, config: str, microbatch: int, devtype: str, arch: str, data_type: str, math_fidelity: str):
    ...
    os.environ["FORGE_DISABLE_FUSE_OPS"] = "1"
    os.environ["FORGE_RIBBON2"] = "1"
...
def resnet50_layer(training: bool, config: str, microbatch: int, devtype: str, arch: str):
    ...
    compiler_cfg = _get_global_compiler_config()
    # verify_cfg.verify_forge_codegen_vs_framework = False # hacking 7x7 to 1x1 will cause mismatches
    ...
    if layer == "head":
        if arch == "wormhole_b0":
            ...
            forge.config.override_op_size("buffer_0_input_1_conv2d_0.dc.conv2d.3.dc.conv2d.1.dc.sparse_matmul.9.dc.sparse_matmul.1.lc2", (7, 1))
            forge.config.override_t_stream_shape("buffer_0_input_1_conv2d_0.dc.conv2d.3.dc.conv2d.1.dc.sparse_matmul.9.dc.sparse_matmul.1.lc2", (2, 1))

            forge.config.override_multi_op_fracture_factor("conv2d_0.dc.conv2d.3", fracture_factor)
            # forge.config.override_multi_op_fracture_factor("conv2d_0", 7)
            for sparse_mm in fractured_conv_sparse_mms:
                forge.config.override_op_size(sparse_mm, (7, 1))
                forge.config.override_t_stream_shape(sparse_mm, (2, 1))
            for dense_mm in fractured_conv_dense_mms:
                forge.config.override_op_size(dense_mm, (7, 1))
                forge.config.override_t_stream_shape(dense_mm, (2, 1))
                forge.config.override_u_kt(dense_mm, 1)

            forge.config.set_epoch_break(["_fused_op_0"])
        else:
            forge.config.override_op_size("conv2d_0.dc.conv2d.3.dc.sparse_matmul.9.dc.sparse_matmul.1.lc2", (2, 4))
            forge.config.override_fracture_factor("conv2d_0.dc.conv2d.3.dc.sparse_matmul.9.dc.sparse_matmul.1.lc2", 4)
            forge.config.override_op_size("conv2d_0.dc.conv2d.3.dc.matmul.11", (2, 1))
            forge.config.override_op_size("max_pool2d_2.dc.sparse_matmul.5.dc.sparse_matmul.1.lc2", (2, 2))
