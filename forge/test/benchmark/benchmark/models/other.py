...
from forge.op.eval.sparse_utils import interleave_tiles, vslice, calculate_conv2d_output_dimensions, create_conv2d_sparse_picker_matrix, conv2d_padding_to_canonical
from forge.op.nn import Conv2dModule
from forge.config import _get_global_compiler_config


class ConvTModule(forge.ForgeModule):
    """
    ConvTModule
    """
    ...
    def forward(self, act):
        ...
        y = self.conv(act)
        y = forge.op.Reshape("", y, (1, 1, self.out_channels, outy * outx))
        y = forge.op.Transpose("", y, 2, 3)

        return y


class SimpleAddModule(forge.ForgeModule):
    """
    Simple add module
    """
    ...
    def __init__(self, name):
        super().__init__(name)
        self.weights = forge.Parameter(*self.shape, requires_grad=True)
        self.set_parameter("weights", torch.rand(*self.shape, requires_grad=True))

    def forward(self, x):
        return forge.op.Add("add1", x, self.weights)
