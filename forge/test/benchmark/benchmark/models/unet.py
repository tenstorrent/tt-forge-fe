...
def test_unet(
    ...
    compiler_config.mlir_config = MLIRConfig().set_enable_optimizer(True).set_enable_fusing(True)
...
def unet_benchmark(config: dict):
    """
    ...
    if not output_file:
        output_file = f"forge-benchmark-e2e-unet_{variant}_{result['run_type']}.json"
