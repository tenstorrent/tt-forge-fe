...
def hrnet(training: bool, config: str, microbatch: int, devtype: str, arch: str, data_type: str, math_fidelity: str):
    ...
    os.environ["FORGE_SUPRESS_T_FACTOR_MM"] = "46" # removing causes hang #2139
    ...
    if config == "w18":
        ...
        if available_devices:
            if available_devices[0] == BackendDevice.Grayskull:
                forge.config._internal_insert_fj_buffering_nop('add_312', ['add_341'], nop_count=2)
    else:
    ...
    modules = {"tt": forge.PyTorchModule(f"pt_hrnet_{config}", model)}
