...
def test_resnet_hf_tensorflow(training, batch_size, data_format, input_size, channel_size, loop_count, variant, task):
...
def resnet_hf_tensorflow_benchmark(config: dict):
    """
    ...
    result = test_resnet_hf_tensorflow(
        training=training,
        batch_size=batch_size,
    ...
    if not output_file:
        output_file = f"forge-benchmark-e2e-resnet50_tensorflow{result['run_type']}.json"
