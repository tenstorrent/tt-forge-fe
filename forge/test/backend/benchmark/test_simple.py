...
def run_benchmark(module, microbatch, input_shapes, cycle_range, fidelity, data_format):
    ...
    tt0 = forge.TTDevice("tt0", module=module, fp32_fallback=data_format)
    forge.set_configuration_options(
            math_fidelity=fidelity,
            performance_trace=forge.PerfTraceLevel.VERBOSE)

    loop_count = 1
    inputs = tuple(torch.rand(microbatch, *shape) for shape in input_shapes)
    tt0.push_to_inputs(inputs)

    output_q = forge.run_inference(input_count = loop_count + 1, _verify_cfg=forge.VerifyConfig.disabled())
...
class MatmulTest(forge.ForgeModule):
    def __init__(self, name, weight_shape):
        super().__init__(name)
        self.weights = forge.Parameter(torch.rand(*weight_shape))
...
def test_matmul():
    ...
    act_shape = (128, 768)
    forge.override_op_size("matmul", (1, 4))
    forge.override_op_size("matmul_output_nop_0", (1, 4))
    run_benchmark(MatmulTest("benchmark_matmul", weight_shape), 1024, (act_shape,), 
            cycle_range=(15000, 25000), fidelity=forge.MathFidelity.LoFi, data_format=forge.DataFormat.Bfp8_b)
