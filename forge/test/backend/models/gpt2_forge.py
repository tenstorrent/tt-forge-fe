...
'''
Doing GPT-2 exploration in Python script instead of notebook because the "edit Forge -> rebuild env -> restart notebook" workflow was too painful.
'''
import os

import forge
from forge import (BackendType, BackendDevice, TTDevice, CPUDevice, PyTorchModule, DataFormat)
from forge.verify import verify_module, verify_module_pipeline, VerifyConfig, TestKind
from forge.verify.utils import CPUCombiner
...
def test_gpt2_block_train_manual_loop(test_device, num_block, sequential):
    ...
    opt_tt = None
    if not opt_on_cpu:
        opt_tt = forge.optimizers.SGD(learning_rate=5e-5, device_params=True)
    ...
    cpu0.place_loss_module(forge.PyTorchModule('l1loss', torch.nn.L1Loss()))
 
    #_get_global_compiler_config().enable_tvm_dropout = False
    #_get_global_compiler_config().enable_tvm_unsupported_ops = True

    # Compile
    checkpoint_q = forge.initialize_pipeline(training=True,
    ...
    for step in range(5):
        ...
        if opt_on_cpu:
            grads = forge.get_parameter_gradients(tt0, _sequential=sequential)
            params = forge.get_parameter_checkpoint(tt0, _sequential=sequential)
            ...
            forge.update_device_parameters(tt0, params, _sequential=sequential)

        else:
            forge.run_optimizer(_sequential=sequential)
    ...
    for i, l_pb in enumerate(losses_pb):
        print(f"index={i} forge loss/pytorch loss={l_pb/loss_pt[i]}")
...
def test_gpt2_inference(devtype, dataformat):
    ...
    forge.set_configuration_options(accumulate_df=dataformat)
    compiler_cfg = _get_global_compiler_config()
