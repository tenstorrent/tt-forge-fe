...
from forge.verify import verify_module, verify_module_pipeline, VerifyConfig
...
from forge import Tensor, DataFormat, BackendType, BackendDevice
from forge.config import _get_global_compiler_config
from forge._C.placer import DRAMPlacementAlgorithm
...
def test_ff(test_kind, cfg, test_device, optimizer):
    ...
    mod = ForgeFeedForward("ff", params, config)
...
def test_pred_transform(test_kind, cfg, test_device):
    ...
    config =  { }
    mod = ForgePredictionHeadTransform("pred_transform", params, config)
...
def test_pred_decoder(test_kind, cfg, test_device):
    ...
    config =  { }
    mod = ForgePredictionHeadDecoder("pred_decoder", params, config)
...
def test_encoder(test_kind, cfg, test_device, dram_allocator):
    ...
    config =  { "num_heads": num_heads, "encoder_index": 0 }
    mod = ForgeBertEncoder("encoder", params, config)

    params["reciprocal_of_sqrt_of_head_size_0"].set_value(torch.full((1, 1, 1, 1), 1/math.sqrt(num_heads)))

    forge.config.set_configuration_options(dram_placement_algorithm=dram_allocator)
...
def test_multichip_wormhole_multi_encoder_split_concurrent(test_kind, cfg, test_device, encoder_count, num_chips):
    # Set forge config
    forge.config.set_configuration_options(default_df_override=DataFormat.Float16_b)
...
It seems as though forge/forge/verify/backend.py::_verify_training was intended to be run in sequential mode. 
...
def test_pt_encoder_ethernet_datacopy_serialization(test_kind, test_device, size, encoder_count, num_chips):
    ...
    import os
    os.environ["FORGE_ENABLE_ETH_DATACOPY_SERIALIZATION"] = "1"
    os.environ["FORGE_DISABLE_INTERACTIVE_PLACER"] = "1"
...
def test_pt_bert_qa_fallback(test_device, variant):
    ...
    # TT run
    question_answerer_tt = forge_pipeline('question-answering', model=model, tokenizer=tokenizer)
...
def test_pt_bert_qa(test_device, size):
    ...
    cpu0 = forge.CPUDevice("cpu0", module=PyTorchModule("bert_embeddings", EmbWrapper(model.bert)))
    ...
    for input in inputs:
        ...
        output_q = forge.run_inference(_verify_cfg=VerifyConfig(relative_atol=0.3), _sequential=True)
