...
class SkipConnections(forge.ForgeModule):

    def __init__(self, name, input_type: str, binary_type: str):
        super().__init__(name)
        self.weights_input = forge.Parameter(1, *shape, requires_grad=True) if input_type == "matmul" else None
    ...
    def forward(self, act1, act2):
        ...
        if self._input_type == "unary":
            input1 = forge.op.Buffer("input1", act1)
        elif self._input_type == "matmul":
            assert self.weights_input
            input1 = forge.op.Matmul("input1", act1, self.weights_input)
        else:
            input1 = act1
        
        input2 = forge.op.Buffer("input2", act2)

        stage2 = forge.op.Buffer("stage2", input2)
        stage3 = forge.op.Buffer("stage3", stage2)

        if self._binary_type == "matmul":
            output = forge.op.Matmul("binary", input1, stage3)
        else:
            output = forge.op.Add("binary", input1, stage3)
...
class VConnection(forge.ForgeModule):
    ...
    def forward(self, act):
        ...
        for i in range(self.depth):
            fwd.append( forge.op.Buffer(f"down{i}", fwd[i] ))

        bwd = fwd[-1]
        for i in range(1, self.depth):
            if self.binary_types[i-1] == "matmul":
                bwd = forge.op.Matmul(f"up{i-1}", bwd, fwd[-(i+1)])
            else:
                bwd = forge.op.Add(f"up{i-1}", bwd, fwd[-(i+1)])
...
class ForkSkipConnections(forge.ForgeModule):
    ...
    def forward(self, act):
        ...
        if self._input_type == "unary":
            input1 = forge.op.Buffer("input1", act)
        elif self._input_type == "matmul":
            assert self.weights_input
            input1 = forge.op.Matmul("input1", act, self.weights_input)
        else:
            input1 = act
        
        stage2 = forge.op.Buffer("stage2", input1)
        ...
        stage4 = forge.op.Buffer("stage4", stage3)

        if self._binary_type2 == "matmul":
            output = forge.op.Matmul("binary2", input1, stage4)
        else:
            output = forge.op.Add("binary2", input1, stage4)
...
class MultiInputRead(forge.ForgeModule):
    def forward(self, act):
        const = forge.op.Constant("const", constant=1)
        stage1 = forge.op.Add("add1", act, const)
        stage2 = forge.op.Add("add2", act, const)
        stage3 = forge.op.Add("add3", act, const)
...
import torch
class ManyParams(forge.ForgeModule):
    def __init__(self, name):
        super().__init__(name)
        self.size = 10
        self.params = [forge.Parameter(torch.normal(mean=0.0, std=0.1, size=(128, 128)), requires_grad=True) for _ in range(self.size)]

    def forward(self, act):
        for i in range(self.size):
            act = forge.op.Matmul(f"matmul_{i}", act, self.params[i])
        return act
