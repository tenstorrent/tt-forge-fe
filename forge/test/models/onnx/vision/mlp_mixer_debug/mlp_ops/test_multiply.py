...
class Multiply1(ForgeModule):
    def __init__(self, name):
        super().__init__(name)
        self.add_constant("multiply1_const_1", shape=(1,), dtype=torch.float32)

    def forward(self, multiply_input_0):
        multiply_output_1 = forge.op.Multiply("", multiply_input_0, self.get_constant("multiply1_const_1"))
        return multiply_output_1
...
forge_modules_and_shapes_dtypes_list = [
    (Multiply0, [((1, 768, 384), torch.float32), ((1, 768, 384), torch.float32)], {'model_names': ['onnx_mlp_mixer_mixer_b16_224_goog_in21k_img_cls_timm'], 'pcc': 0.99}), 
    (Multiply1, [((1, 768, 384), torch.float32)], {'model_names': ['onnx_mlp_mixer_mixer_b16_224_goog_in21k_img_cls_timm'], 'pcc': 0.99}), 
    (Multiply0, [((1, 196, 3072), torch.float32), ((1, 196, 3072), torch.float32)], {'model_names': ['onnx_mlp_mixer_mixer_b16_224_goog_in21k_img_cls_timm'], 'pcc': 0.99}), 
    (Multiply1, [((1, 196, 3072), torch.float32)], {'model_names': ['onnx_mlp_mixer_mixer_b16_224_goog_in21k_img_cls_timm'], 'pcc': 0.99}), 
]
