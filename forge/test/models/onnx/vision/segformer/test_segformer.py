...
from test.models.onnx.vision.segformer.utils.utils import get_sample_data
...
variants_img_classification = [
    pytest.param(
        ...
        marks=[
            pytest.mark.xfail(
                reason="Statically allocated circular buffers in program 5 clash with L1 buffers on core range [(x=0,y=0) - (x=7,y=7)]. L1 buffer allocated at 213120 and static circular buffer region ends at 626528"
            )
        ],
    ),
    "nvidia/mit-b1",
...
def test_segformer_image_classification_onnx(forge_property_recorder, variant, tmp_path):
    if variant != "nvidia/mit-b0":
        pytest.skip("Only running the base variant for now")
...
variants_semseg = [
    pytest.param(
        ...
        marks=[
            pytest.mark.xfail(
                reason="Statically allocated circular buffers in program 9 clash with L1 buffers on core range [(x=0,y=0) - (x=7,y=7)]. L1 buffer allocated at 213120 and static circular buffer region ends at 626528"
            )
        ],
    ),
