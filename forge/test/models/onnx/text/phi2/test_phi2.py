...
@pytest.mark.nightly
@pytest.mark.xfail(reason="AssertionError: Encountered unsupported op node type: scatter_nd, on device: tt")
...
def test_phi2_clm_onnx(forge_property_recorder, variant, tmp_path):
    ...
    compiled_model = forge.compile(
        onnx_model, inputs, forge_property_handler=forge_property_recorder, module_name=module_name, onnx_path=onnx_path
    )
