...
def test_albert_token_classification_pytorch(variant):
    class Wrapper(torch.nn.Module):
        ...
        def forward(self, ip):
            logger.info("self.classifier={}", self.model)
    ...
    sequence_output = torch.load("sequence_output.pt")

    logger.info("sequence_output={}", sequence_output)
    logger.info("sequence_output.shape={}", sequence_output.shape)
    logger.info("sequence_output.dtype={}", sequence_output.dtype)
