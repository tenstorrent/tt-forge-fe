...
def test_t5_loop_tiny_tile(test_device):
    ...
    tt_model = forge.PyTorchModule("t5_generation_loop", Wrapper(model))
...
def test_t5_past_cache_enc_dec(variant, test_device):
    ...
    encoder_module = forge.PyTorchModule("T5_encoder", T5_encoder(model))
    decoder_module_cross_attention = forge.PyTorchModule("T5_decoder_with_ca", T5_decoder(model))
    decoder_module_no_cross_attention = forge.PyTorchModule("T5_decoder_no_ca", T5_decoder(model))
    tt0 = forge.TTDevice(
        "tt0",
    ...
    print(
        f"{current_token_index} iterations took {end - start} seconds, speed: {(current_token_index) / (end - start)} iters/sec"
    )
    print(
        f"Discounting initial token, {current_token_index - 1} iterations took {end - start_1} seconds, speed: {(current_token_index - 1) / (end - start_1)} iters/sec"
    )
...
def test_t5_past_cache_forge_pipeline(variant, test_device):
    ...
    os.environ["FORGE_PAD_OUTPUT_BUFFER"] = "1"
    os.environ["TT_BACKEND_MULTI_THREADED_PUSH"] = "1"
    os.environ["FORGE_EXTRA_L1_MARGIN"] = "169536"
    os.environ["FORGE_FORCE_SEQUENTIAL"] = "1"
    ...
    if "large" in variant:
        os.environ["FORGE_EXTRA_L1_MARGIN"] = "69536"
    ...
    for i in range(num_blocks):
        inputs += (
            torch.zeros(enc_past_cache_shape),
            torch.zeros(enc_past_cache_shape),
    ...
    tt0 = forge.TTDevice(
        "tt0", devtype=test_device.devtype, arch=test_device.arch, module=forge.PyTorchModule("t5", blocks)
    )
    output_q = forge.initialize_pipeline(training=False, sample_inputs=inputs)
    ...
    text_generator = forge_pipeline("text2text-generation", model=model, tokenizer=tokenizer, forward_fn=wrap_generate)
...
def test_t5_forge_pipeline(variant, test_device):
    ...
    text_generator = forge_pipeline("text2text-generation", model=model, tokenizer=tokenizer, forge_max_length=32)
