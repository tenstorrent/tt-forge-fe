...
def test_deepseek_prefil_on_device_decode_on_cpu(variant):
    ...
    verify(inputs=[input_ids], framework_model=model_decoder, compiled_model=compiled_decoder)
    ...
    hidden_states_compiled = transformer_outputs[0].to("cpu")

    # Get hidden states for all tokens from the last "transformer layer" calculated on CPU.
    hidden_states_framework = model_decoder(input_ids)
