...
def test_check():
    ...
    print("cpu output.dtype",co_out[0].dtype)
    print("forge output.dtype",fw_out[0].dtype)
    ...
    calculated_pcc = scipy.stats.pearsonr(co_out[0].detach().numpy().reshape(-1), fw_out[0].detach().numpy().reshape(-1))[
        0
    ]
    print("calculated_pcc= ", calculated_pcc)
    
    required_pcc = 0.99999999999

    if calculated_pcc < required_pcc:
        raise Exception(
            "Calculated pcc score {} does not match with required pcc {}".format(calculated_pcc, required_pcc)
        )
