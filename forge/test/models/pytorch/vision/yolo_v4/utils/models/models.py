...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            xy, other = torch.split(io, [2, io.shape[-1] - 2], dim=-1)  # Split the first two channels
            xy = xy * 2.0 - 0.5 + self.grid  # Apply the transformation
            io = torch.cat([xy, other], dim=-1)  # Concatenate back with the remaining part
...
def get_yolo_layers(model):
    return [
        i for i, m in enumerate(model.module_list) if m.__class__.__name__ in ["YOLOLayer", "JDELayer"]
    ]  # [89, 101, 113]
