...
def nms_manual(boxes, scores, iou_threshold):
    """
    Perform Non-Maximum Suppression (NMS) without using torchvision.
    ...
    while indices.numel() > 0:
        top_idx = indices[0]  # Pick the box with the highest score
        ...
        # Compute IoU
        x1 = torch.max(top_box[:, 0], other_boxes[:, 0])
        y1 = torch.max(top_box[:, 1], other_boxes[:, 1])
        x2 = torch.min(top_box[:, 2], other_boxes[:, 2])
        y2 = torch.min(top_box[:, 3], other_boxes[:, 3])

        inter_area = (x2 - x1).clamp(0) * (y2 - y1).clamp(0)
        box_area1 = (top_box[:, 2] - top_box[:, 0]) * (top_box[:, 3] - top_box[:, 1])
        box_area2 = (other_boxes[:, 2] - other_boxes[:, 0]) * (other_boxes[:, 3] - other_boxes[:, 1])

        iou = inter_area / (box_area1 + box_area2 - inter_area + 1e-6)  # Avoid division by zero
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Replace this line (original code):

        # With this PyTorch-native implementation:
        # from torchvision.ops import nms  # Add this import at the top of the file
