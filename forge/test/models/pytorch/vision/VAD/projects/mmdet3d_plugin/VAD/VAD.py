...
class VAD(MVXTwoStageDetector):
    ...
    def forward_pts_train(
        ...
        outs = self.pts_bbox_head(
            pts_feats, img_metas, prev_bev, ego_his_trajs=ego_his_trajs, ego_lcf_feat=ego_lcf_feat
        )
        loss_inputs = [
            gt_bboxes_3d,
            gt_labels_3d,
    ...
    def forward(self, return_loss=True, **kwargs):
        ...
        for key, value in kwargs.items():
            if isinstance(value, torch.Tensor):
                kwargs[key] = value.to("cpu")
            elif isinstance(value, list):  # Handle lists of tensors
                kwargs[key] = [v.to("cpu") if isinstance(v, torch.Tensor) else v for v in value]
        if return_loss:
