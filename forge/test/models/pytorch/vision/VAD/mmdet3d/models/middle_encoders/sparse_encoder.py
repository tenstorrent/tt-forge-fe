...
class SparseEncoder(nn.Module):
    ...
    def make_encoder_layers(
        self, make_block, norm_cfg, in_channels, block_type="conv_module", conv_cfg=dict(type="SubMConv3d")
    ):
            ...
            for j, out_channels in enumerate(tuple(blocks)):
                ...
                if i != 0 and j == 0 and block_type == "conv_module":
                    ...
                    if j == len(blocks) - 1 and i != len(self.encoder_channels) - 1:
                        ...
                        blocks_list.append(
                            SparseBasicBlock(out_channels, out_channels, norm_cfg=norm_cfg, conv_cfg=conv_cfg)
                        )
                else:
                    blocks_list.append(
                        make_block(
