...
class TestCase(unittest.TestCase):
    ...
    def assertAllClose(self, a, b, rtol=1e-6, atol=1e-6):
        ...
        if is_a_dict:
            self.assertCountEqual(a.keys(), b.keys(), msg="mismatched keys, expected %s, got %s" % (a.keys(), b.keys()))
            for k in a:
...
def generate_sparse_data(
    shape, num_points, num_channels, integer=False, data_range=(-1, 1), with_dense=True, dtype=np.float32
):
    dense_shape = shape
