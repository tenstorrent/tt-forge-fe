...
class SparseBottleneck(Bottleneck, spconv.SparseModule):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, conv_cfg=None, norm_cfg=None):
        ...
        Bottleneck.__init__(
            self, inplanes, planes, stride=stride, downsample=downsample, conv_cfg=conv_cfg, norm_cfg=norm_cfg
        )
    ...
    for layer in order:
        if layer == "conv":
            if conv_type not in ["SparseInverseConv3d", "SparseInverseConv2d", "SparseInverseConv1d"]:
                layers.append(
                    build_conv_layer(
                        conv_cfg, in_channels, out_channels, kernel_size, stride=stride, padding=padding, bias=False
                    )
                )
            else:
