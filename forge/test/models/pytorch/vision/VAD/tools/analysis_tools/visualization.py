...
def lidiar_render(sample_token, data, out_path=None, out_name=None, traj_use_perstep_offset=True):
    ...
    for ann in anns:
        ...
        gt_fut_trajs, gt_fut_masks = get_gt_fut_trajs(
            nusc=nusc, anno=content, cs_record=cs_record, pose_record=pose_record, fut_ts=6
        )
        try:
            bbox_gt_list.append(
                CustomDetectionBox(
...
def get_gt_vec_maps(
    ...
    anns_results = vector_map.gen_vectorized_samples(map_location, lidar2global_translation, lidar2global_rotation)
...
def render_sample_data(
    ...
    """
    lidiar_render(
        sample_toekn, pred_data, out_path=out_path, out_name=out_name, traj_use_perstep_offset=traj_use_perstep_offset
    )
...
if __name__ == "__main__":
    ...
    for id in tqdm(range(len(sample_token_list))):
            ...
            if sensor_modality in ["lidar", "radar"]:
                ...
                if cam == "CAM_FRONT":
                    ...
                    line_segments = LineCollection(
                        plan_vecs, colors=colors, linewidths=2, linestyles="solid", cmap=cmap
                    )
        ...
        pred_img = cv2.putText(pred_img, "BEV", org, font, fontScale, color, thickness, cv2.LINE_AA)
        pred_img = cv2.putText(pred_img, plan_cmd_str, (20, 770), font, fontScale, color, thickness, cv2.LINE_AA)
