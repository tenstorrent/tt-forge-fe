...
class v116ADTRHead(DETRHead):
    ...
    def __init__(
        ...
        *args,
        ...
        loss_mot_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.8),
        map_bbox_coder=None,
        map_num_query=900,
        map_num_classes=3,
        map_num_vec=20,
        ...
        loss_plan_map_theta=dict(type="PlanMapThetaLoss", loss_weight=0.0),
        loss_tl_status_cls=dict(type="CrossEntropyLoss", use_sigmoid=False, loss_weight=1.0, class_weight=None),
        loss_tl_trigger_cls=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=1.0, class_weight=None),
        loss_stopsign_trigger_cls=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=1.0, class_weight=None),
        ego_pv_decoder=None,
        ...
        **kwargs,
        ...
        if kwargs["train_cfg"] is not None:
            assert "map_assigner" in kwargs["train_cfg"], "map assigner should be provided " "when train_cfg is set."
            ...
            assert loss_map_pts["loss_weight"] == map_assigner["pts_cost"]["weight"], (
                "The regression l1 weight for map pts loss and matcher should be" "exactly the same."
            )
    ...
    def _init_layers(self):
        ...
        mot_num_pred = (num_mot_decoder_layers + 1) if self.as_two_stage else num_mot_decoder_layers
        ...
        if self.ego_pv_decoder is not None:
            ...
            MAXNUM_PV_TOKEN = 800
            self.pv_pos_embedding = nn.Embedding(MAXNUM_PV_TOKEN, self.embed_dims)
    ...
    def init_weights(self):
        ...
        if self.interaction_pe_type is not None:
            for emb_mlp in self.pe_embed_mlps:
                xavier_init(emb_mlp, distribution="uniform", bias=0.0)
        # if self.cf_backbone is not None:
    ...
    def forward(
        ...
        if self.mot_decoder is not None:
            ...
            if self.interaction_pe_type is not None:
                ...
                mot_coords = (
                    pos2posemb2d(mot_coords, num_pos_feats=self.embed_dims // 2)
                    if self.interaction_pe_type == "sine_mlp"
                    else mot_coords
                )
            ...
            if self.mot_map_decoder is not None:
                ...
                map_query, map_pos, key_padding_mask = self.select_and_pad_pred_map(
                    ...
                    map_score,
                    map_pos,
                    map_thresh=self.mot_map_thresh,
                    dis_thresh=self.mot_dis_thresh,
                ...
                if self.interaction_pe_type is not None:
                    ...
                    mot_pos = (
                        pos2posemb2d(mot_pos, num_pos_feats=self.embed_dims // 2)
                        if self.interaction_pe_type == "sine_mlp"
                        else mot_pos
                    )
                    ...
                    map_pos = (
                        pos2posemb2d(map_pos, num_pos_feats=self.embed_dims // 2)
                        if self.interaction_pe_type == "sine_mlp"
                        else map_pos
                    )
            ...
            mot_hs = mot_hs.permute(1, 0, 2).unflatten(dim=1, sizes=(num_agent, self.mot_fut_mode))
            ca_mot_query = ca_mot_query.squeeze(0).unflatten(dim=0, sizes=(batch_size, num_agent, self.mot_fut_mode))
        ...
        if self.training:
            best_match_idx = (
                torch.linalg.norm(ego_fut_trajs[0].cumsum(dim=-2) - self.plan_anchors, dim=-1).sum(dim=-1).argmin()
            )
        ...
        self.used_plan_anchors[self.used_plan_anchors[:, 0].norm(dim=-1) < 1e-2] = 0.0
        # fix one stop traj
        self.used_plan_anchors[0] = 0.0

        _tmp = pos2posemb2d(
            self.used_plan_anchors.reshape(1, self.plan_fut_mode * self.fut_ts, -1), num_pos_feats=self.embed_dims // 2
        ).reshape(self.plan_fut_mode, self.fut_ts, -1)
        ...
        agent_query, agent_pos, agent_mask = self.select_and_pad_query(
            agent_query, agent_pos, agent_conf, score_thresh=self.ego_query_thresh, use_fix_pad=self.query_use_fix_pad
        )

        if self.interaction_pe_type is not None:
            ego_agent_pos = (
                torch.ones((batch, ego_query.shape[1], 2), device=ego_query.device) * 0.5
            )  # ego in the center
            ego_agent_pos = (
                pos2posemb2d(ego_agent_pos, num_pos_feats=self.embed_dims // 2)
                if self.interaction_pe_type == "sine_mlp"
                else ego_agent_pos
            )
            ego_agent_pos = self.pe_embed_mlps[2](ego_agent_pos)
            agent_pos = (
                pos2posemb2d(agent_pos, num_pos_feats=self.embed_dims // 2)
                if self.interaction_pe_type == "sine_mlp"
                else agent_pos
            )
        ...
        map_query, map_pos, map_mask = self.select_and_pad_query(
            map_query, min_map_pos, map_conf, score_thresh=self.ego_query_thresh, use_fix_pad=self.query_use_fix_pad
        )

        if self.interaction_pe_type is not None:
            ego_map_pos = (
                torch.ones((batch, ego_query.shape[1], 2), device=agent_query.device) * 0.5
            )  # ego in the center
            ego_map_pos = (
                pos2posemb2d(ego_map_pos, num_pos_feats=self.embed_dims // 2)
                if self.interaction_pe_type == "sine_mlp"
                else ego_map_pos
            )
        ...
        # TODO no need / 2.
        _idx = torch.floor((_tmp_target_point.clip(min=-69.0, max=69.0) - (-70.0)) / 2.0).long()
        for i in range(batch):
            _tmp_rasterized_feat[i, _idx[i, 0, 0], _idx[i, 0, 1]] = 1.0
        _tmp_rasterized_feat = _tmp_rasterized_feat.reshape(batch, 1, 140 * 140)
        ego_wp_feat = self.ego_feat_projs[3](_tmp_target_point)
        ego_wp_feat += 1.0 * self.ego_feat_projs[7](_tmp_rasterized_feat)
        ...
        if isinstance(command_id, torch.Tensor):
            ...
            for i in range(batch):
                cmdid_onehot[i, 0, command_id[i] - 1] = 1.0
            ego_cmdid_feat = self.ego_feat_projs[5](cmdid_onehot)
        else:
            cmdid_onehot = torch.zeros((batch, 1, 6), device=ego_cf_feat.device, dtype=torch.float32)
            assert command_id.max() <= 6 and command_id.min() >= 1
            assert batch == 1
            cmdid_onehot[0, 0, command_id - 1] = 1.0
            ego_cmdid_feat = self.ego_feat_projs[5](cmdid_onehot)
        ...
        outputs_ego_trajs = outputs_ego_trajs.reshape(outputs_ego_trajs.shape[0], self.plan_fut_mode, self.fut_ts, 2)
        # if self.training:
        ...
        outputs_ego_trajs = outputs_ego_trajs * 0.0 + self.used_plan_anchors[None].to(outputs_ego_trajs.device)
    ...
    def _get_target_single(self, cls_score, bbox_pred, gt_labels, gt_bboxes, gt_attr_labels, gt_bboxes_ignore=None):
        ...
        return (
            ...
            mot_targets,
            mot_weights,
            mot_masks.view(-1, self.fut_ts, 2)[..., 0],
            pos_inds,
            neg_inds,
        )
    ...
    def loss_planning(
        ...
        for i in range(batch):
            plan_col_label = self.get_plan_col_target(
                ego_fut_preds[i].detach(), ego_fut_gt[i], gt_agent_boxes[i], gt_agent_feats[i]
            )
            plan_bd_label = self.get_plan_bd_target(ego_fut_preds[i].detach(), gt_map_pts[i], gt_map_labels[i])
            plan_cl_label = self.get_plan_cl_target(ego_fut_preds[i].detach(), gt_map_pts[i], gt_map_labels[i])
        ...
        plan_expert_labels_weight = torch.stack(plan_expert_labels_weight, dim=0).to(ego_fut_preds.device)

        # plan collision classification loss
        loss_plan_cls_col = self.loss_plan_cls_col(
            ego_cls_col_preds.flatten(0, 1),
            plan_col_labels.flatten(),
        ...
        loss_plan_cls_bd = self.loss_plan_cls_bd(
            ego_cls_bd_preds.flatten(0, 1),
            plan_bd_labels.flatten(),
        ...
        plan_cl_labels_weight = (plan_cl_weight > -1.0) + (plan_cl_weight == -1.0) * 0.01
        loss_plan_cls_cl = self.loss_plan_cls_cl(
            ego_cls_cl_preds.squeeze(-1).flatten(0, 1),
            plan_cl_labels.flatten(),
        ...
        loss_plan_cls_expert = self.loss_plan_cls_expert(
            ego_cls_expert_preds.flatten(0, 1),
            plan_expert_labels.flatten(),
            plan_expert_labels_weight.flatten(),
            avg_factor=batch * self.plan_fut_mode,
        )

        loss_plan_reg = (0.0 * ego_fut_preds).sum()
        ...
        loss_plan_bound = (0.0 * ego_fut_preds).sum()
        ...
        loss_plan_agent_dis = (0.0 * ego_fut_preds).sum()
        ...
        loss_plan_map_theta = (0.0 * ego_fut_preds).sum()
    ...
    def loss_single(
        ...
        loss_mot_reg = self.loss_mot_reg(
            best_mot_preds[isnotnan], mot_targets[isnotnan], mot_weights[isnotnan], avg_factor=num_total_pos
        )

        # mot classification loss
        mot_cls_scores = mot_cls_preds.reshape(-1, self.mot_fut_mode)
        # construct weighted avg_factor to match with the official DETR repo
        mot_cls_avg_factor = num_total_pos * 1.0 + num_total_neg * self.mot_bg_cls_weight
        if self.sync_cls_avg_factor:
            mot_cls_avg_factor = reduce_mean(mot_cls_scores.new_tensor([mot_cls_avg_factor]))

        mot_cls_avg_factor = max(mot_cls_avg_factor, 1)
        loss_mot_cls = self.loss_mot_cls(mot_cls_scores, mot_labels, label_weights, avg_factor=mot_cls_avg_factor)
    ...
    def get_mot_cls_target(self, mot_preds, mot_targets, gt_fut_masks, neg_inds):
    ...
    def loss(
        ...
        if self.sync_cls_avg_factor:
            tl_trigger_cls_avg_factor = reduce_mean(tl_trigger_cls_scores.new_tensor([tl_trigger_cls_avg_factor]))
        tl_trigger_cls_avg_factor = max(tl_trigger_cls_avg_factor, 1)
        loss_tl_trigger_cls = self.loss_tl_trigger_cls(
            tl_trigger_cls_scores,
            tl_trigger_labels,
        ...
        if self.sync_cls_avg_factor:
            tl_status_cls_avg_factor = reduce_mean(tl_status_cls_scores.new_tensor([tl_status_cls_avg_factor]))
        tl_status_cls_avg_factor = max(tl_status_cls_avg_factor, 1)
        loss_tl_status_cls = self.loss_tl_status_cls(
            tl_status_cls_scores, tl_status_labels, tl_status_weights, avg_factor=tl_status_cls_avg_factor
        )
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            ret_list.append([bboxes, scores, labels, trajs, trajs_cls, map_bboxes, map_scores, map_labels, map_pts])
    ...
    def get_plan_col_target(self, ego_traj_preds, ego_traj_gts, agents_boxes_gts, agents_feats_gts):
        ...
        for i in range(self.plan_fut_mode):
            label = planning_metric.evaluate_coll(ego_traj_preds[None, i].detach(), ego_traj_gts[None, i], occupancy)
            label_list.append(label)

        return torch.cat(label_list, dim=-1).to(agents_feats_gts.device)

    def get_plan_bd_target(self, ego_traj_preds, lane_preds, lane_score_preds, lane_bound_cls_idx=1, map_thresh=0.5):
        """ "Get Trajectory mode classification target.
        Args:
        ...
        padding_zeros = torch.zeros(
            (self.plan_fut_mode, 1, 2), dtype=ego_traj_preds.dtype, device=ego_traj_preds.device
        )  # initial position
        ...
        intersect_mask = segments_intersect(ego_traj_starts, ego_traj_ends, bd_inst_starts, bd_inst_ends)
        left_deviation = ego_traj_starts.new_tensor([-0.9, 2.4])
        right_deviation = ego_traj_starts.new_tensor([+0.9, 2.4])
        forward_deviation = ego_traj_starts.new_tensor([0.0, 2.4])
        intersect_mask_left = segments_intersect(
            ego_traj_starts + left_deviation, ego_traj_ends + left_deviation, bd_inst_starts, bd_inst_ends
        )
        intersect_mask_right = segments_intersect(
            ego_traj_starts + right_deviation, ego_traj_ends + right_deviation, bd_inst_starts, bd_inst_ends
        )
        intersect_mask_forward = segments_intersect(
            ego_traj_starts + forward_deviation, ego_traj_ends + forward_deviation, bd_inst_starts, bd_inst_ends
        )
        ...
        bd_overstep_labels = torch.zeros((self.plan_fut_mode), dtype=torch.long, device=ego_traj_preds.device)
        bd_overstep_labels[intersect_mask] = 1
        bd_overstep_labels[~intersect_mask] = 0

        return bd_overstep_labels

    def get_plan_cl_target(self, ego_traj_preds, lane_preds, lane_score_preds, lane_bound_cls_idx=3, map_thresh=0.5):
        ...
        maps_interpolated = F.interpolate(
            lane_centerline_preds.permute(0, 2, 1), scale_factor=50, mode="linear", align_corners=True
        ).permute(0, 2, 1)
        ...
        dist = dist.sum(
            dim=1
        )  # dist = dist.max(dim=1)[0]  plan T dim   (max deviation)   or dist = dist.sum(dim=1)  (mean deviation)
        ...
        cl_dir_labels = 1.0 - dist_2.min(dim=-1)[0].mean(dim=-1)
        # cl_dir_labels = cos_sim.mean(dim=-1) \
        #     - dist_2.min(dim=-1)[0].mean(dim=-1)
        cl_dir_labels = torch.clamp(cl_dir_labels, min=-1.0)
    ...
    def get_plan_expert_target(
        ...
        plan_expert_labels = torch.zeros((self.plan_fut_mode), dtype=torch.long, device=ego_traj_preds.device)

        plan_expert_labels_weight = torch.zeros(
            (self.plan_fut_mode), dtype=ego_traj_preds.dtype, device=ego_traj_preds.device
        )

        if ego_fut_masks[0] == 1.0:

            neg_idx = torch.ones((self.plan_fut_mode), dtype=torch.bool, device=ego_traj_preds.device)
            #### v1
            ...
            #### v11
            traj_dis = torch.linalg.norm(ego_traj_preds[:, :, :] - ego_fut_gt.cumsum(dim=-2)[:, :, :], dim=-1).sum(
                dim=-1
            )
            plan_expert_labels[neg_idx] = 1
            plan_expert_labels_weight[neg_idx] = torch.clip(traj_dis, min=0, max=100.0) * 100.0

            plan_expert_labels[plan_col_labels == 1] = 1
            plan_expert_labels[plan_bd_labels == 1] = 1
            plan_expert_labels_weight[plan_col_labels == 1] = 100.0
            plan_expert_labels_weight[plan_bd_labels == 1] = 100.0
            ...
            self.traj_selected_cnt[pos_idx] += 1.0
            scaling_rate = self.traj_selected_cnt.sum() / self.traj_selected_cnt[pos_idx] / self.plan_fut_mode
            scaling_rate = torch.clamp(scaling_rate, 0.5, 2.0)
            plan_expert_labels_weight[pos_idx] = 100.0  # * scaling_rate
...
class PlanningMetric:
    def __init__(self, fut_ts=6):
        ...
        self.category_index = {"human": [0, 1, 2, 3], "vehicle": [0, 1, 2, 3]}
