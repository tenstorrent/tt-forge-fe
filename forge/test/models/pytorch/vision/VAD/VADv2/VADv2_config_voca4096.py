...
# camera view order
view_names = ["rgb_front", "rgb_front_right", "rgb_front_left", "rgb_rear", "rgb_rear_left", "rgb_rear_right"]
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        loss_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.8),
        loss_bbox=dict(type="L1Loss", loss_weight=0.1),
        loss_mot_reg=dict(type="L1Loss", loss_weight=0.1),
        loss_mot_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.2),
        loss_iou=dict(type="GIoULoss", loss_weight=0.0),
        loss_map_cls=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.8),
        ...
        loss_plan_cls_col=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.0),
        loss_plan_cls_bd=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.0),
        ...
        #     alpha=0.25,
        #     loss_weight=2.0),
        loss_plan_cls_expert=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=200.0),
        ...
        loss_tl_status_cls=dict(type="CrossEntropyLoss", use_sigmoid=False, loss_weight=0.8, class_weight=None),
        loss_tl_trigger_cls=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=4.0, class_weight=None),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            dict(type="RandomScaleImageMultiViewImage", scales=[0.5]),
            dict(type="PadMultiViewImage", size_divisor=32),
            dict(type="CustomDefaultFormatBundle3DFromCarla", class_names=class_names, with_label=False, with_ego=True),
            dict(
                type="CustomCollect3D",
                keys=[
                    "gt_bboxes_3d",
    ...
    ),  #  'traffic_signal', 'stop_sign_signal'
]
...
# learning policy
lr_config = dict(policy="CosineAnnealing", warmup="linear", warmup_iters=500, warmup_ratio=1.0 / 3, min_lr_ratio=1e-2)
