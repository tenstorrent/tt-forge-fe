...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
        ...
        if pts_bbox_head:
            # logger.info("inside if pts_bbox_head:")
            pts_train_cfg = train_cfg.pts if train_cfg else None
            pts_bbox_head.update(train_cfg=pts_train_cfg)
            pts_test_cfg = test_cfg.pts if test_cfg else None
            pts_bbox_head.update(test_cfg=pts_test_cfg)
            # logger.info("pts_train_cfg={}",pts_train_cfg)
            # logger.info("pts_test_cfg={}",pts_test_cfg)
            
            self.pts_bbox_head = builder.build_head(pts_bbox_head)
            # logger.info("self.pts_bbox_head={}",self.pts_bbox_head)
            # logger.info("self.pts_bbox_head.__class__={}",self.pts_bbox_head.__class__)
            # breakpoint()
