...
class PadMultiViewImage(object):
    ...
    def _pad_img(self, results):
        ...
        if self.size is not None:
            padded_img = [impad(img, shape=self.size, pad_val=self.pad_val) for img in results["img"]]
        elif self.size_divisor is not None:
            padded_img = [impad_to_multiple(img, self.size_divisor, pad_val=self.pad_val) for img in results["img"]]
...
class RandomScaleImageMultiViewImage(object):
    ...
    def __call__(self, results):
        ...
        results["img"] = [
            imresize(img, (x_size[idx], y_size[idx]), return_scale=False) for idx, img in enumerate(results["img"])
        ]
