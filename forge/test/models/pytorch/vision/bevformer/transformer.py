...
class SpatialCrossAttention(BaseModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if residual is None:
            inp_residual = query
            # slots = torch.zeros_like(query)    ## created here
            slots = query.new_zeros(query.size())
        if query_pos is not None:
            query = query + query_pos
        ...
        for i, mask_per_img in enumerate(bev_mask):
            ...
            # logger.info(f"index_query_per_img = {index_query_per_img}")
        ...
        for j in range(bs):
            for i, reference_points_per_img in enumerate(reference_points_cam):
                ...
                # Prepare queries_rebatch
                values_to_assign_query = query[j, index_query_per_img]
                padded_values_query = torch.cat(
                    [
                        values_to_assign_query,
                        torch.zeros(
                            max_len - len(index_query_per_img), self.embed_dims, device=query.device, dtype=query.dtype
                        ),
                    ],
                    dim=0,
                )
                queries_rebatch_list.append(padded_values_query.unsqueeze(0))

                # Prepare reference_points_rebatch
                values_to_assign_ref = reference_points_per_img[j, index_query_per_img]
                padded_values_ref = torch.cat(
                    [
                        values_to_assign_ref,
                        torch.zeros(
                            max_len - len(index_query_per_img),
                            reference_points_per_img.size(-2),
                            reference_points_per_img.size(-1),
                            device=reference_points_per_img.device,
                            dtype=reference_points_per_img.dtype,
                        ),
                    ],
                    dim=0,
                )
                reference_points_rebatch_list.append(padded_values_ref.unsqueeze(0))
...
class PerceptionTransformer(BaseModule):
    ...
    def get_bev_features(
        ...
        # bev_queries = bev_queries + can_bus * self.use_can_bus   ## original
        bev_queries = bev_queries + can_bus * int(self.use_can_bus)  ## added to solve pytorch jit trace failure
        feat_flatten = []
        spatial_shapes = []
