    ...
    def __init__(self, widthi_multiplier, depth_multiplier, num_classes, drop_connect_rate, dropout_rate):
        ...
        for i, stage_setting in enumerate(mb_block_settings):
            ...
            for _ in range(num_repeat - 1):
                stage.append(
                    MBConvBlock(
                        input_filters, output_filters, kernal_size, stride, expand_ratio, se_ratio, has_se=False
                    )
                )
...
def get_image_tensor(wh):
    ...
    img_tensor = tfms(Image.open("forge/test/models/files/samples/images/img.jpeg")).unsqueeze(0)
    return img_tensor
