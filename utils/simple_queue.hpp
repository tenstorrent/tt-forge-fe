...
struct SimpleQueue
{
   ...
   public:
    ...
    void assert_not_empty() { TT_ASSERT(not this->empty(), "Trying to pop from an empty SimpleQueue: " + this->name); }
    ...
    void reset_offset() { this->offset = 0; }

    void increment_offset() { this->offset++; }

    bool empty() const { return this->data_queue.empty(); }

    bool full() const { return this->data_queue.size() == this->max_size; }

    size_t size() const { return this->data_queue.size(); }

    const T& front() const { return this->data_queue.front(); }

    const T& back() const { return this->data_queue.back(); }
    ...
    const T& read(bool use_shadow_rd_ptr = false)
    {
        ...
        TT_ASSERT(
            ...
            "Trying to read queue " + this->name + " with size " + std::to_string(this->size()) +
                " out-of-bounds with index " + std::to_string(rd_ptr));
