...
def get_perf_from_es(es, build_id):
    ...
    query = {"query": {"term": {"build_id.keyword": build_id}}, "size": 100}
    ...
    return [
        {
            ...
            "index": "_".join(
                [
                    h["_source"]["args"]["model"],
                    h["_source"]["args"]["config"],
                    h["_source"]["args"]["arch"],
                    h["_source"]["args"]["dataformat"],
                ]
            ),
        }
        for h in hits
...
def compare_perf(build_ids: list):
    ...
    pivot_table = df.pivot_table(
        index=["index", "arch", "config", "dataformat", "model"],
        columns="build_id",
